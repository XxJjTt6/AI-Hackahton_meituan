...
def _solve_scarce_bundle_mcf_enum(candidates,all_tasks,deadline):
	...
	for D in _canonical_candidates(G):
		...
		C.append((T,F,A,K,D))
	if not C:return[]
	C.sort(key=lambda item:(_popcount(item[2]),item[0]/max(item[1],1e-09),item[0],-item[1],-item[4][5]),reverse=_B);C=C[:180];O=[(0,frozenset(),())];U=list(O);L=_complete_scarce_bundles_with_mcf((),J,I);V=_solution_expected_cost(L,G,H)if L else float(_F);a=min(len({A[2]for A in G}),len(H)//2+2)
	for W in range(a):
		...
		if not B:break
		B=_prune_scarce_bundle_states(B,I,J,max_states=260);U.extend(B);O=B
	Y=set()
	for(A,M,P)in U:
...
def _solve_scarce_bundle_group_mcf_enum(candidates,all_tasks,deadline):
	...
	if not D:return[]
	D.sort(key=lambda item:(_popcount(item[2]),item[0]/max(1,len(item[3])),item[0]/max(item[1],1e-09),item[0]),reverse=_B);D=D[:140];R=[(0,frozenset(),())];Z=list(R);K=_complete_scarce_bundle_groups_with_mcf((),J,I);a=_solution_expected_cost(K,M,G)if K else float(_F);h=min(len(G)//2+2,18)
	for b in range(h):
		...
		if not B:break
		B=_prune_scarce_bundle_group_states(B,I,J,max_states=180);Z.extend(B);R=B
	e=set()
	for(C,L,S)in Z:
