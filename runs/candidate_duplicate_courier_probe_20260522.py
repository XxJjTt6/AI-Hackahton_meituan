...
def solve(input_text):
	...
	P=_duplicate_courier_probe(C,B,L,d)
...
def _duplicate_courier_probe(candidates,all_tasks,task_count,courier_count):
	if task_count!=40 or courier_count!=40:return _A
	D={}
	for A in candidates:D.setdefault(A[2],[]).append(A)
	for C in sorted(D):
		E=sorted(D[C],key=lambda A:(len(A[1]),A[3],-A[4],A[5]))[:80]
		for A in E:
			S=set(A[1])
			for B in E:
				if A[0]!=B[0]and S.isdisjoint(B[1]):return[(A[0],[C]),(B[0],[C])]
	return _A
