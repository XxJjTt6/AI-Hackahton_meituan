...
def solve(input_text):
	...
	if G and time.monotonic()<A-1.2:
		R=_ssci1(C,B,[D],min(A,time.monotonic()+1.05))
...
def _ssbgme(candidates,all_tasks,deadline):
	...
	return K
def _ssci1(candidates,all_tasks,seed_solutions,deadline):
	G=candidates;B=all_tasks;H=deadline;R={(A[0],A[2]):A for A in G};S=[A for A in seed_solutions if A]
	if not S:return[]
	base=min(S,key=lambda A:_solution_expected_cost(A,G,B));sel=_result_to_selected(base,R)
	if not sel:return[]
	P={};O={}
	for K,V in sel.items():
		C=_group_expected_cost(V,len(V[0][1]))/max(1,len(V[0][1]))
		for T in V[0][1]:P[T]=C
		for A in V:O[A[2]]=K
	N={};M={}
	for A in _canonical_candidates(G):
		if len(A[1])==1:N.setdefault(A[1][0],[]).append(A)
		elif len(A[1])==2:M.setdefault(A[0],[]).append(A)
	D=[]
	for K,V in M.items():
		if time.monotonic()>H-.08:break
		T=V[0][1];V=sorted(V,key=lambda A:(_group_expected_cost([A],2),-A[4],A[5]))[:5]
		for A in V:
			C=_group_expected_cost([A],2);Q=C-sum(P.get(X,100.)for X in T);U=O.get(A[2])
			if U and U!=K and U in sel:Q+=.35*_group_expected_cost(sel[U],len(sel[U][0][1]))/max(1,len(sel[U][0][1]))
			D.append((Q,C,A))
	D=sorted(D,key=lambda A:(A[0],A[1]))[:55];best=base;bc=_solution_expected_cost(base,G,B)
	for(Q,C,A)in D:
		if time.monotonic()>H-.04:break
		Z=_csbgwm(((A,),),N,sorted(B));Y=_solution_expected_cost(Z,G,B)
		if Y<bc-1e-09:best=Z;bc=Y
	return best
