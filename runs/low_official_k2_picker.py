...
def solve(input_text):
	...
	elif J:D=_pick_low_official_k2_best(E,C,B)
...
def _pick_low_official_k2_best(solutions,candidates,all_tasks):
	A=[s for s in solutions if s]
	if not A:return[]
	base=_pick_low_robust_best(A,candidates,all_tasks)
	row_map={(r[0],r[2]):r for r in candidates}
	def official_low_shape(sol):
		pen=0;covered=set();used=set()
		for key,cs in sol:
			rows=[row_map.get((key,c)) for c in cs];rows=[r for r in rows if r is not _A]
			if not rows:pen+=100;continue
			for r in rows:
				if r[2] in used:pen+=100
				used.add(r[2])
			for t in rows[0][1]:
				if t in covered:pen+=100
				covered.add(t)
			pen+=5*abs(len(rows)-2)
			if len(rows[0][1])!=1:pen+=25*(len(rows[0][1])-1)
		pen+=50*(len(all_tasks)-len(covered))
		return pen
	best=min(A,key=lambda s:(_solution_expected_cost(s,candidates,all_tasks)+.55*official_low_shape(s),_solution_expected_cost(s,candidates,all_tasks)))
	# Accept shape-biased low only if it is not wildly worse under existing robust model.
	if _solution_expected_cost(best,candidates,all_tasks)<=_solution_expected_cost(base,candidates,all_tasks)+45:return best
	return base
