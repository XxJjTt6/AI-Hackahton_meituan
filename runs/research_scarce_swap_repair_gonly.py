...
def solve(input_text):
	...
	if G and time.monotonic()<A-.3:
		...
		if not F and time.monotonic()<A-.28:
...
def _scarce_one_swap_uncovered_repair(result,candidates,all_tasks,deadline):
	...
	if not missing:return G
	used={A[2] for rows in B.values() for A in rows}
	for u in missing:
		...
		for cand in sorted(by_task.get(u,()),key=lambda c:(_group_expected_cost([c],len(c[1])),-len(c[1]),c[5]))[:80]:
			...
			for t in sorted(lost):
				...
				opts=[row for row in by_task.get(t,()) if row[2] not in used2 and not any(x in covered2 for x in row[1] if x!=t)]
			...
			if cost<best_cost-1e-09:best=U;best_cost=cost
	return best
