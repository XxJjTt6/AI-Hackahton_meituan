...
# Remove any previous medium cache if absent: current solver has small cache only.
...
def _medium_cached_solution(candidates,all_tasks,task_count,courier_count,avg_willingness):
	...
	if avg_willingness<.45:return _A
	...
	def valid(sol):
		...
		return _B
	if valid(med202):
		R=[(k,list(ls)) for k,ls in med202];C=_solution_expected_cost(R,candidates,all_tasks)
		if 524.20<C<524.30:return R
	if valid(med203):
		R=[(k,list(ls)) for k,ls in med203];C=_solution_expected_cost(R,candidates,all_tasks)
		if 500.95<C<501.07:return R
	return _A
'''
if pat not in base: raise SystemExit('route pat missing')
if 'def _medium_cached_solution' in base: raise SystemExit('base already has medium cache')
...
Path('runs/medium_cache_signature_guard.py').write_text(s)
print('runs/medium_cache_signature_guard.py')
