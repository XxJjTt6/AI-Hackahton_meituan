...
def solve(input_text):
	...
	if J and time.monotonic()<A-1.70:D=_low_deep_window_repair_solution(D,C,B,min(A,time.monotonic()+1.2))
	if J and time.monotonic()<A-1.25:D=_low_late_acceptance_repair_solution(D,C,B,min(A,time.monotonic()+.85))
	if J and time.monotonic()<A-.55:D=_shift_couriers_between_groups(D,C,B,min(A,time.monotonic()+.24),max_moves=30)
