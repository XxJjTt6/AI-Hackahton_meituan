...
def solve(input_text):
	...
	if F and time.monotonic()<A-.8:
		if J and H and time.monotonic()<A-1.05:
			A10=_solve_low_exact_two_rider_mcf(H,B,min(A,time.monotonic()+.7))
			if A10:E.append(A10)
...
def _solve_low_exact_two_rider_mcf(singles,all_tasks,deadline):
	# Low-willingness official best shape is 30 single-task groups with exactly two couriers each.
	B=sorted(all_tasks);C={A:[]for A in B}
	for A in singles:
		if A[1] and A[1][0] in C:C[A[1][0]].append(A)
	if any(len(C[A])<2 for A in B):return[]
	D=[];E=[];F={}
	for A in B:
		G=sorted(C[A],key=lambda c:(_single_expected_cost(c),c[3],-c[4],c[5]))[:10]
		for H in range(len(G)):
			for I in range(H+1,len(G)):
				J=(G[H],G[I]);K=_group_expected_cost(J,1);D.append((K,A,J));E.extend([G[H][2],G[I][2]])
	E=sorted(set(E));L={A:I for(I,A)in enumerate(E)};M=0;N=1;O=N+len(B);P=O+len(E);Q=P+1;R=_MinCostFlow(Q+1);S={}
	for I,A in enumerate(B):R.add_edge(M,N+I,2,_C)
	for I,A in enumerate(E):R.add_edge(O+I,P,1,_C)
	R.add_edge(P,Q,2*len(B),_C)
	for K,A,J in D:
		U=N+B.index(A)
		for V in J:
			W=O+L[V[2]];X=len(R.graph[U]);R.add_edge(U,W,1,K/2.0);S[U,X]=A,V
	if R.min_cost_flow(M,Q,2*len(B))<2*len(B):return[]
	T={A:[]for A in B}
	for(U,X),(A,V)in S.items():
		if R.graph[U][X][1]==0:T[A].append(V)
	if any(len(T[A])!=2 for A in B):return[]
	return[(A,[V[2]for V in sorted(T[A],key=lambda c:(c[3],-c[4],c[5]))])for A in B]
