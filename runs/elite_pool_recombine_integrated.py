...
def solve(input_text):
	...
	if (J or f) and len(E)>1 and time.monotonic()<A-.55:
		R=_elite_pool_recombine(E,C,B,min(A,time.monotonic()+.35))
		if R:E.append(R)
...
def _elite_pool_recombine(solutions,candidates,all_tasks,deadline,max_cols=220,max_states=1000):
	H={(A[0],A[2]):A for A in candidates};T={B:A for(A,B)in enumerate(sorted(all_tasks))};C={B:A for(A,B)in enumerate(sorted({A[2]for A in candidates}))};cols=[];seen=set()
	for sol in solutions:
		if not sol:continue
		for k,ls in sol:
			rows=[];tm=0;cm=0;ok=_B
			for c in ls:
				r=H.get((k,c))
				if r is _A:ok=_D;break
				rows.append(r);cm|=1<<C[c]
			if not ok or not rows:continue
			for t in rows[0][1]:
				if t not in T:ok=_D;break
				tm|=1<<T[t]
			if not ok:continue
			key=(tm,cm,k,tuple(ls))
			if key in seen:continue
			seen.add(key);cost=_group_expected_cost(rows,len(rows[0][1]));gain=1e2*len(rows[0][1])-cost
			if gain>1e-09:cols.append((gain,cost,tm,cm,k,tuple(ls)))
	if not cols:return[]
	cols.sort(reverse=_B);cols=cols[:max_cols];states={(0,0):(_C,())}
	for idx,col in enumerate(cols):
		if time.monotonic()>deadline-.04:break
		gain,cost,tm,cm,k,ls=col;add=[]
		for (at,ac),(val,chosen) in list(states.items()):
			if at&tm or ac&cm:continue
			nk=(at|tm,ac|cm);nv=val+gain;old=states.get(nk)
			if old is _A or nv>old[0]+1e-09:add.append((nk,(nv,chosen+(idx,))))
		for nk,v in add:states[nk]=v
		if len(states)>max_states*2:states=dict(sorted(states.items(),key=lambda kv:(kv[1][0],_popcount(kv[0][0])),reverse=_B)[:max_states])
	best=max(states.items(),key=lambda kv:(kv[1][0],_popcount(kv[0][0])))[1][1]
