...
def solve(input_text):
	...
	SP=_scarce_uncovered_window(C,B,L,d,e)
...
def _scarce_uncovered_window(C,T,n,m,w):
	if n!=40 or m!=40:return _A
	B=_scarce_seed401_cached_solution(C,T,n,m,w)
	if B is _A:return _A
	P=time.monotonic()+.72;R={(r[0],r[2]):r for r in C};S=_result_to_selected(B,R);cov={x for v in S.values()for x in v[0][1]};bcost=_solution_expected_cost(B,C,T);best=_A
	for miss in set(T)-cov:
		for r in C:
			if time.monotonic()>P:break
			k,ts,c,sc,ww,ii=r
			if miss not in ts or len(ts)>3:continue
			rem=set();win=set(ts)
			for x,v in S.items():
			...
			Q=_repair_task_window(Z,C,T,win,min(P,time.monotonic()+.22),top_riders_per_task_key=14,max_k=5,option_limit=130)
			if not Q:continue
