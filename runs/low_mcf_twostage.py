...
def _solve_low_mcf_twostage(candidates,all_tasks):
	...
	# Greedy add remaining couriers by best marginal improvement, matching old probe stage2 cheaply.
	...
	while left:
		...
		for c in left:
			for t in C:
				A=row.get((t,c))
				if A is _A:continue
				old=_group_expected_cost(G[t],1);new=_group_expected_cost(G[t]+[A],1);g=old-new
				if g>gain+1e-12:gain=g;best=c,t,A
		if best is _A or gain<=1e-9:break
		c,t,A=best;G[t].append(A);left.remove(c)
