...
def solve(input_text):
	...
	if not O or F or AA:
		...
		if G and time.monotonic()<A-_E:
			...
			if w:E.append(w)
			if G and time.monotonic()<A-1.05:
				w2=_solve_scarce_topk_bundle_mcf(C,B,min(A,time.monotonic()+.45))
				if w2:
					# Official scarce can prefer leaving a hard task unassigned; do not let a
					# high-cost full-coverage TopK candidate dominate hard-scarce selection.
					if not f or _solution_covered_count(w2,C)<=max((_solution_covered_count(S,C)for S in E if S),default=0):E.append(w2)
			if G and time.monotonic()<A-_E:
