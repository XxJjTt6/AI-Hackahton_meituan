...
def solve(input_text):
	...
	if F and time.monotonic()<A-.8:
		...
		if J and time.monotonic()<A-1.05:
			pe=_solve_low_exact_two_mcf(C,B)
			if pe:E.append(pe)
...
def _solve_low_exact_two_mcf(candidates,all_tasks):
	B=[A for A in candidates if len(A[1])==1];C=sorted(all_tasks);D=sorted({A[2] for A in B})
	if len(D)!=2*len(C) or not B:return []
	row={(A[1][0],A[2]):A for A in B};S=0;R=1;T=R+len(D);U=T+2*len(C);F=_MinCostFlow(U+1);M={}
	for i,c in enumerate(D):F.add_edge(S,R+i,1,_C)
	slots=[]
	for t in C:
		for z in range(2):slots.append(t)
	for j,t in enumerate(slots):F.add_edge(T+j,U,1,_C)
	for i,c in enumerate(D):
		for j,t in enumerate(slots):
			A=row.get((t,c))
			if A is _A:continue
			cost=int(round(_single_expected_cost(A)*1000000))
			idx=len(F.graph[R+i]);F.add_edge(R+i,T+j,1,cost);M[R+i,idx]=t,A
	if F.min_cost_flow(S,U,len(D))<len(D):return []
	G={t:[] for t in C}
	for (node,idx),(t,A) in M.items():
		if F.graph[node][idx][1]==0:G[t].append(A)
	if any(len(G[t])!=2 for t in C):return []
	return [(t,[A[2] for A in sorted(G[t],key=lambda x:(x[3],-x[4],x[5]))]) for t in C]
