...
def _solve_scarce_k2_column_search(candidates,all_tasks,deadline):
	A=candidates
	if not A:return[]
	B=_search_column_window_scarce_value_shaped(A,all_tasks,deadline,top_riders_per_task_key=10,max_k=2,option_limit=60)
	return B or _search_column_window(A,all_tasks,deadline,top_riders_per_task_key=10,max_k=2,option_limit=60)
def _task_shadow_values(candidates,all_tasks):
	B={T:[]for T in all_tasks}
	for C in candidates:
		if len(C[1])>3:continue
		D=len(C[1]);E=_group_expected_cost([C],D);F=1e2*D-E
		for T in C[1]:
			if T in B:B[T].append((F,E,C[4],D))
	V={}
	for T,L in B.items():
		if not L:V[T]=30.;continue
		L.sort(reverse=_B);G=L[0][0];H=sum(1 for(g,c,w,k)in L if g>1e-9)
		V[T]=max(0.,min(35.,6./max(1,H)+max(0.,18.-G)*.06))
	return V
def _scarce_value_row_key(c,task_values,courier_counts):
	D=len(c[1]);E=_group_expected_cost([c],D);F=sum(task_values.get(T,0.)for T in c[1]);G=.06*courier_counts.get(c[2],1)
	return E-.10*F+G,E/max(1,D),-c[4],c[5]
def _search_column_window_scarce_value_shaped(candidates,all_tasks,deadline,top_riders_per_task_key,max_k,option_limit):
	O=deadline;F=candidates;Tvals=_task_shadow_values(F,all_tasks);Ccnt={}
	for C0 in F:Ccnt[C0[2]]=Ccnt.get(C0[2],0)+1
	...
	for B0 in R.values():
		...
		B0=sorted(B0,key=lambda c:_scarce_value_row_key(c,Tvals,Ccnt))[:top_riders_per_task_key]
		if not B0:continue
		S=0
		for c in B0[0][1]:S|=1<<Q[c]
		D=len(B0[0][1])
		for d in range(1,min(max_k,len(B0))+1):
			for K in itertools.combinations(B0,d):
				L=0;Bad=_D
				for G in K:
					U=1<<b[G[2]]
					if L&U:Bad=_B;break
	...
	def h(decided_task_mask,current_reduced):
		...
		while A:C0=A&-A;D0=C0.bit_length()-1;B+=g[D0];A^=C0
		return B
	def i(decided_task_mask,courier_mask):
		E0=decided_task_mask;A=M&~E0;B=_A;C0=[]
		while A:
			F0=A&-A;G0=F0.bit_length()-1;D0=[A0 for A0 in I[G0]if not A0[2]&E0 and not A0[3]&courier_mask]
			if B is _A or len(D0)<len(C0):
				B=G0;C0=D0
				if not D0:break
			A^=F0
		return B,C0
	def N(decided_task_mask,courier_mask,current_reduced):
		F0=courier_mask;B0=decided_task_mask;A0=current_reduced;nonlocal E,C
		if time.monotonic()>O-.02:return
		if h(B0,A0)>=C-1e-09:return
		if B0==M:
			if A0<C-1e-09:C=A0;E=list(J)
			return
		G0,H0=i(B0,F0)
		if G0 is _A:
			if A0<C-1e-09:C=A0;E=list(J)
			return
		for D0 in H0[:option_limit]:J.append(D0);N(B0|D0[2],F0|D0[3],A0+D0[0]);J.pop()
		N(B0|1<<G0,F0,A0)
	N(0,0,_C);a=[]
	for A in sorted(E,key=lambda c:(min(A[5]for A in c[4]),c[4][0][0],tuple(A[2]for A in c[4]))):B0=sorted(A[4],key=lambda c:(c[3],-c[4],c[5]));a.append((B0[0][0],[A[2]for A in B0]))
	return a
