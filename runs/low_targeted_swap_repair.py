...
def solve(input_text):
	...
	if J and len(D)<=18 and time.monotonic()<A-.48:
		SW=_low_targeted_swap_solution(D,C,B,min(A,time.monotonic()+.24))
		if len(SW)==len(D) and _solution_expected_cost(SW,C,B)<_solution_expected_cost(D,C,B)-1e-09:D=SW
...
def _low_targeted_swap_solution(result,candidates,all_tasks,deadline):
	lut={(A[0],A[2]):A for A in candidates};base=_solution_expected_cost(result,candidates,all_tasks);best=result;bestc=base;groups=[]
	for idx,(k,cs) in enumerate(result):
		r=[lut.get((k,c)) for c in cs];r=[A for A in r if A is not _A]
		if not r:continue
		groups.append((_group_expected_cost(r,len(k.split(',')))/max(1,len(k.split(','))),idx,k,cs))
	high=sorted(groups,reverse=_B)[:5];low=sorted(groups)[:7]
	for _,i,k1,cs1 in high:
		if time.monotonic()>deadline-.04:break
		for _,j,k2,cs2 in low:
			if i==j:continue
			a_i,b_j=i,j;A1,A2=k1,k2;C1,C2=cs1,cs2
			if a_i>b_j:a_i,b_j=b_j,a_i;A1,A2=A2,A1;C1,C2=C2,C1
			for a in C1:
				for b in C2:
					n1=[b if x==a else x for x in C1];n2=[a if x==b else x for x in C2]
					cand=result[:a_i]+[(A1,n1)]+result[a_i+1:b_j]+[(A2,n2)]+result[b_j+1:]
					c=_solution_expected_cost(cand,candidates,all_tasks)
