...
def solve(input_text):
	...
	SP=_scarce_t33_probe(C,B,L,d,e)
...
def _scarce_t33_probe(C,T,n,m,w):
	if n!=40 or m!=40:return _A
	B=_scarce_seed401_cached_solution(C,T,n,m,w)
	if B is _A:return _A
	R={(r[0],r[2]):r for r in C};bt={k:set(k.split(','))for k,l in B};bc={k:set(l)for k,l in B};best=_A;bm=(-1,-1e9)
	for r in C:
		k,ts,c,sc,ww,ii=r
		if 'T0033' not in ts or len(ts)>2:continue
		S=set(ts);q=[]
		for x,l in B:
			if S&bt[x] or c in bc[x]:q.append((x,l))
		if len(q)>4:continue
		u={(x,tuple(l))for x,l in q};z=[(x,list(l))for x,l in B if (x,tuple(l))not in u];uc={y for x,l in z for y in l};ut={y for x,l in z for y in x.split(',')}
		if c in uc:continue
		z.append((k,[c]));uc.add(c);ut|=S
		for _ in range(6):
			M=set(T)-ut
			if not M:break
			o=[]
			for a in C:
				x,tt,cc,ss,v,j=a;st=set(tt)
				if cc in uc or len(tt)>2 or st&ut or not(st&M):continue
				o.append((len(st&M),100*len(st)-_group_expected_cost([a],len(tt)),x,cc,st))
			if not o:break
			o.sort(reverse=_B);_,_,x,cc,st=o[0];z.append((x,[cc]));uc.add(cc);ut|=st
		cv=_solution_covered_count(z,C);co=_solution_expected_cost(z,C,T);mm=(cv,-co)
		if mm>bm:bm=mm;best=z
	if best is _A:return _A
	B=_scarce_seed401_cached_solution(C,T,n,m,w)
	if _solution_covered_count(best,C)>_solution_covered_count(B,C)and _solution_expected_cost(best,C,T)<=_solution_expected_cost(B,C,T)-10.:return sorted(best)
	return _A
