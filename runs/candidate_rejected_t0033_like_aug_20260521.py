...
def _augment_scarce_cache(base,candidates,all_tasks):
	...
	L={};M0=set(all_tasks)-{B for A in S.values()for B in A[0][1]}
	for A in candidates:
		if len(A[1])==1:L.setdefault(A[1][0],[]).append(A)
	U={C for A,B in base for C in B}
	for i,(k,ls)in enumerate(base):
		Q=[A for A in candidates if A[0]==k and(A[2]not in U or A[2]in ls)]
		if Q:
			A=min(Q,key=lambda A:_group_expected_cost([A],len(A[1])));sol=base[:i]+[(k,[A[2]])]+base[i+1:];C=_solution_expected_cost(sol,candidates,all_tasks)
			if C<bc-1e-09:best=sol;bc=C
	V=sorted((A for A in candidates if set(A[1])&M0),key=lambda A:_group_expected_cost([A],len(A[1])))
	for A in V[:12]:
		T=set(A[1]);U={A[2]};cur=[];M=set()
		for k,ls in base:
			r=R.get((k,ls[0]));ts=set(r[1])if r else set()
			if T&ts or U&set(ls):M|=ts
			else:cur.append((k,ls))
		used={C for k,ls in cur for C in ls}|U;cur.append((A[0],[A[2]]));M=(M|T)-T
		for t in sorted(M):
			Q=[r for r in L.get(t,[])if r[2]not in used]
			if not Q:break
			r=min(Q,key=lambda X:_group_expected_cost([X],1));used.add(r[2]);cur.append((r[0],[r[2]]))
		else:
			C=_solution_expected_cost(cur,candidates,all_tasks)
			if _solution_covered_count(cur,candidates)>=base_cov and C<bc-1e-09:best=cur;bc=C
