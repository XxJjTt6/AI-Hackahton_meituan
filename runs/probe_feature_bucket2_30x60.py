...
def solve(input_text):
	...
	FB=_feature_bucket2_30x60(C,B,L,d,e)
	if FB is not _A:return FB
...
def _feature_bucket2_30x60(candidates,all_tasks,task_count,courier_count,avg_willingness):
	...
	if avg_willingness>=.29:return _A
	avg_score=sum(r[3] for r in candidates)/len(candidates)
	by={}
	for r in candidates:
		if len(r[1])==1:by.setdefault(r[1][0],[]).append(r)
	if len(by)<30:return _A
	# Three feasible 60-courier patterns by avg_score bucket.
	if avg_score<30:
		triple={'T0029','T0004','T0027','T0012','T0016'}; single={'T0025','T0008','T0023','T0019','T0005'}
	elif avg_score<35:
		triple={'T0025','T0008','T0023','T0019','T0005'}; single={'T0029','T0004','T0027','T0012','T0016'}
	else:
		triple={f'T{i:04d}' for i in range(20,30)}; single={f'T{i:04d}' for i in range(10)}
	used=set();out=[]
	for t in sorted(all_tasks):
		k=3 if t in triple else 1 if t in single else 2
		opts=sorted([r for r in by.get(t,[]) if r[2] not in used],key=lambda r:_group_expected_cost([r],1))
		pick=[]
		for r in opts:
			if r[2] not in used:
				pick.append(r);used.add(r[2])
				if len(pick)>=k:break
		if len(pick)<k:return _A
