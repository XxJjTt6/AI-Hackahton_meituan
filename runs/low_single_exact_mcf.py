...
def solve(input_text):
	...
	if J and time.monotonic()<A-.45:D=_low_single_shape_reassign(D,C,B,min(A,time.monotonic()+.32))
...
def _low_single_shape_reassign(result,candidates,all_tasks,deadline):
	A=result;B=candidates;C=all_tasks
	D={(A[0],A[2]):A for A in B if len(A[1])==1}
	E=_result_to_selected(A,D)
	if not E:return A
	if any(len(F[0][1])!=1 for F in E.values() if F):return A
	G=_selected_cost(E,C)
	for H in range(3):
		if time.monotonic()>deadline-.08:break
		I=_reassign_selected_once(E,D);J=_selected_cost(I,C)
		if J<G-1e-09:E=I;G=J
		else:break
	K=_format_selected(E)
	return K if _solution_expected_cost(K,B,C)<_solution_expected_cost(A,B,C)-1e-09 else A
