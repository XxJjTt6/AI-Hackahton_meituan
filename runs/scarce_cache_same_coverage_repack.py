...
def solve(input_text):
	...
	if SG is not _A:
		RG=_scarce_same_coverage_repack(SG,C,B)
...
def _scarce_same_coverage_repack(result,candidates,all_tasks):
	# Exact-cache postprocess that preserves the current 39 covered tasks and only lowers raw expected cost.
	...
	base_cov={t for k,cs in result for t in k.split(',')};base=_solution_expected_cost(result,candidates,true_tasks);best=_A;best_cost=base
	groups=sorted([(_group_expected_cost(rows,len(rows[0][1]))-100*len(rows[0][1]),k,set(rows[0][1])) for k,rows in sel.items()],reverse=_B)[:20]
	for _,old_key,old_tasks in groups:
		...
		cols=sorted(cols,key=lambda x:(x[0]-100*len(x[1]),x[0]))[:35]
		states={frozenset():(0,[])}
		...
		out=_format_selected(new);cov={t for k,cs in out for t in k.split(',')}
