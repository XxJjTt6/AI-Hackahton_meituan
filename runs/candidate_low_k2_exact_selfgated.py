...
def solve(input_text):
	...
	if J and time.monotonic()<A-1.05:
		K=_low_k2_exact_probe(C,B,min(A,time.monotonic()+.9))
		if K is not _A and _solution_covered_count(K,C)>=_solution_covered_count(D,C)and _solution_expected_cost(K,C,B)<_solution_expected_cost(D,C,B)-10.:D=K
...
def _low_k2_exact_probe(C,T,deadline):
	H=[r for r in C if len(r[1])==1]
	if not H:return _A
	return _search_column_window(H,T,deadline,top_riders_per_task_key=16,max_k=2,option_limit=180)
