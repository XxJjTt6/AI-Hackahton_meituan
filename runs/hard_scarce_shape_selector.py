...
def solve(input_text):
	...
	if f:D=_pick_hard_scarce_shape_guard(E,C,B);D=_drop_unprofitable_groups(D,C,B)
...
def _pick_hard_scarce_shape_guard(solutions,candidates,all_tasks):
	A=[s for s in solutions if s]
	if not A:return[]
	row_map={(r[0],r[2]):r for r in candidates}
	def metrics(sol):
		covered=set();used=set();pairs=singles=multi=bad=0
		for key,cs in sol:
			rows=[row_map.get((key,c)) for c in cs];rows=[r for r in rows if r is not _A]
			if not rows:bad+=10;continue
			for r in rows:
				if r[2] in used:bad+=20
				used.add(r[2])
			for t in rows[0][1]:
				if t in covered:bad+=20
				covered.add(t)
			if len(rows)>1:multi+=len(rows)-1
			if len(rows[0][1])==2:pairs+=1
			elif len(rows[0][1])==1:singles+=1
			else:bad+=len(rows[0][1])
		return covered,pairs,singles,multi,bad
	def key(sol):
		covered,pairs,singles,multi,bad=metrics(sol); n=len(all_tasks)
		shape=0
		shape+=18*abs(len(covered)-(n-1))
		shape+=3*abs(pairs-19)
		shape+=2*abs(singles-1)
		shape+=8*multi+20*bad
		# Strongly discourage b803 failure mode: low coverage many singles.
		if len(covered)<n-3:shape+=1000+100*(n-len(covered))
		# Strongly discourage bce failure mode: full coverage low-prob pairs.
		if len(covered)==n and pairs>=20:shape+=140
		return _hard_scarce_shadow_cost(sol,candidates,all_tasks)+shape,_solution_expected_cost(sol,candidates,all_tasks)
