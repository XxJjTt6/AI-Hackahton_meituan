...
def _scarce_t0033_two_replace_probe(result,candidates,all_tasks):
	...
	for rem in remove_sets[:21]:
		...
		for r in candidates:
			...
			if saving>-30:cols.append((cost,ts,{r[2]},[r]))
		bykey={}
		for cost,ts,cs,rs in cols:bykey.setdefault(rs[0][0],[]).append(rs[0])
		for rs in bykey.values():
			rs=sorted(rs,key=lambda r:_group_expected_cost([r],len(r[1])))[:5]
			for a in range(len(rs)):
				for b in range(a+1,len(rs)):
					...
					if saving>-30:cols.append((cost,ts,{rs[a][2],rs[b][2]},[rs[a],rs[b]]))
		cols=sorted(cols,key=lambda x:(x[0]-100*len(x[1]),x[0]))[:45]
