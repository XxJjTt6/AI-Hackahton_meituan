...
def solve(input_text):
	...
	if G and time.monotonic()<A-.3:
		...
		if L>=35 and time.monotonic()<A-.16:
			A2=_guard_known_scarce_t0028_pattern(D,C,B,min(A,time.monotonic()+.12))
			if _solution_expected_cost(A2,C,B)<_solution_expected_cost(D,C,B)-1e-09:D=A2
...
def _guard_known_scarce_t0028_pattern(result,candidates,all_tasks,deadline):
	# Official bad signature: a scarce-like solution misses one extra task after assigning
	# one courier to a pair that excludes the missing task. Generic version: if exactly
	# two tasks are missing, try replacing one selected 2-task group with a 2-task group
	# from the same courier that covers one missing task, accepting only total-cost win.
	K={(A[0],A[2]):A for A in candidates};S=_result_to_selected(result,K)
	if not S:return result
	covered={T for rows in S.values() if rows for T in rows[0][1]};missing=set(all_tasks)-covered
	if len(missing)!=2:return result
	best=result;best_cost=_solution_expected_cost(result,candidates,all_tasks);base_cov=_solution_covered_count(result,candidates)
	by_courier={}
	for C in candidates:by_courier.setdefault(C[2],[]).append(C)
	for key,rows in list(S.items()):
		if time.monotonic()>deadline-.04:break
		if not rows or len(rows[0][1])!=2:continue
		couriers={R[2] for R in rows}
		for courier in couriers:
			for cand in by_courier.get(courier,()):
				if time.monotonic()>deadline-.04:break
				if len(cand[1])!=2 or not (set(cand[1])&missing):continue
				# Candidate must keep at least one currently covered task or it is too disruptive.
				if not (set(cand[1])&covered):continue
				NS={k:list(v) for k,v in S.items() if k!=key};NS[cand[0]]=[cand]
				R=_format_selected(NS);cov=_solution_covered_count(R,candidates)
				if cov<base_cov:continue
				cost=_solution_expected_cost(R,candidates,all_tasks)
				if cost<best_cost-1e-09:best=R;best_cost=cost
	return best
