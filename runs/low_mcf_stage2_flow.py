...
def solve(input_text):
	...
	if F and time.monotonic()<A-.8:
		if J and time.monotonic()<A-1.2:
			...
			if p:E.append(p)
		if J and time.monotonic()<A-.9:
			pm=_solve_low_mcf_twostage(C,B)
			if pm:E.append(pm)
...
def _solve_low_mcf_twostage(candidates,all_tasks):
	B=[A for A in candidates if len(A[1])==1];C=sorted(all_tasks);D=sorted({A[2] for A in B});row={(A[1][0],A[2]):A for A in B}
	if len(D)<len(C) or not B:return []
	S=0;R=1;T=R+len(D);U=T+len(C);F=_MinCostFlow(U+1);M={}
	for i,c in enumerate(D):F.add_edge(S,R+i,1,0)
	for j,t in enumerate(C):F.add_edge(T+j,U,1,0)
	for i,c in enumerate(D):
		for j,t in enumerate(C):
			A=row.get((t,c))
			if A is _A:continue
			cost=int(round(_group_expected_cost([A],1)*1000000));idx=len(F.graph[R+i]);F.add_edge(R+i,T+j,1,cost);M[R+i,idx]=t,A
	if F.min_cost_flow(S,U,len(C))<len(C):return []
	G={t:[] for t in C};used=set()
	for (node,idx),(t,A) in M.items():
		if F.graph[node][idx][1]==0:G[t].append(A);used.add(A[2])
	left=[c for c in D if c not in used]
	# Stage2 exact assignment of remaining riders to optional task slots with negative marginal costs.
	if left:
		S2=0;R2=1;T2=R2+len(left);U2=T2+len(C);F2=_MinCostFlow(U2+1);M2={}
		for i,c in enumerate(left):F2.add_edge(S2,R2+i,1,0)
		for j,t in enumerate(C):F2.add_edge(T2+j,U2,1,0)
		for i,c in enumerate(left):
			for j,t in enumerate(C):
				A=row.get((t,c))
				if A is _A:continue
				old=_group_expected_cost(G[t],1);newc=_group_expected_cost(G[t]+[A],1);delta=newc-old
				if delta>=-1e-9:continue
				cost=int(round(delta*1000000));idx=len(F2.graph[R2+i]);F2.add_edge(R2+i,T2+j,1,cost);M2[R2+i,idx]=t,A
		# Push one unit at a time while negative paths exist by asking for all possible flow; positive edges absent.
		F2.min_cost_flow(S2,U2,min(len(left),len(C)))
		for (node,idx),(t,A) in M2.items():
			if F2.graph[node][idx][1]==0:G[t].append(A)
	return [(t,[A[2] for A in sorted(G[t],key=lambda x:(x[3],-x[4],x[5]))]) for t in C if G[t]]
