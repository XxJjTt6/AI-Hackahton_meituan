...
def _scarce_full_column_repack(result,candidates,all_tasks):
	...
	P=time.monotonic()+.75;base=_solution_expected_cost(result,candidates,all_tasks);fixed_shapes={tuple(v[0][1]) for v in sel.values()}
	focus=set()
	for rows in sel.values():
		if _group_expected_cost(rows,len(rows[0][1]))-100*len(rows[0][1])>-125:focus.update(rows[0][1])
	if 'T0033' in all_tasks:focus.add('T0033')
	cols=[];bykey={}
	for r in candidates:
		if time.monotonic()>P:return _A
		ts=set(r[1])
	...
	for cost,ts,cs,rs in cols:
		...
		states=new
		if len(states)>5e4:states=dict(sorted(states.items(),key=lambda kv:(kv[1][0]-100*bin(kv[0][0]).count('1')))[:25000])
	...
	for (tm,um),(val,path) in states.items():
		...
		if cost<best_cost-10:best=out;best_cost=cost
	return best
...
def _solve_scarce_elite_column_recombine(candidates,all_tasks,seed_solutions,deadline):
	...
	F=list(I.values())
	if not F:return[]
	F=_scarce_prune_elite_columns(F,1150);Z=_scarce_beam_pack_columns(F,P,5200)
	if not Z:return[]
	N=[]
