...
def solve(input_text):
	...
	SP=_scarce_direct_missing(C,B,L,d,e)
...
def _scarce_direct_missing(C,T,n,m,w):
	if n!=40 or m!=40:return _A
	B=_scarce_seed401_cached_solution(C,T,n,m,w)
	if B is _A:return _A
	R={(r[0],r[2]):r for r in C};used={c for _,ls in B for c in ls};cov={t for k,_ in B for t in k.split(',')};bc=_solution_expected_cost(B,C,T)
	for r in C:
		k,ts,c,sc,ww,ii=r
		if c in used or set(ts)&cov or len(ts)>2:continue
		Q=B+[(k,[c])]
		if _solution_covered_count(Q,C)>_solution_covered_count(B,C) and _solution_expected_cost(Q,C,T)<bc-10:return sorted(Q)
	return _A
