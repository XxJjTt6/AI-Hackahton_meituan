...
def solve(input_text):
	...
	if SG is not _A:return _sfpack(C,B,SG)
...
def _sfpack(C,B,SG):
 base=_solution_expected_cost(SG,C,B);T=sorted(B);ti={t:i for i,t in enumerate(T)};ci={c:i for i,c in enumerate(sorted({r[2]for r in C}))};cols=[]
 for r in C:
  if len(r[1])>2:continue
  tm=0
  for t in r[1]:tm|=1<<ti[t]
  co=_group_expected_cost([r],len(r[1]));sv=100*len(r[1])-co
  if sv>-20:cols.append((sv/max(1,len(r[1])),sv,co,tm,1<<ci[r[2]],r))
 cols=sorted(cols,reverse=_B)[:1000];S=[(_C,0,0,[])];best=(base,SG)
 for _,_,co,tm,cm,r in cols:
  N=[]
  for sc,ts,cs,path in S:
   if ts&tm or cs&cm:continue
   q=(sc+co,ts|tm,cs|cm,path+[(r[0],[r[2]])]);N.append(q);tot=q[0]+100*(len(T)-q[1].bit_count())
   if tot<best[0]-1e-9:best=(tot,q[3])
  if N:S=(S+N);S=sorted(S,key=lambda x:(x[0]+100*(len(T)-x[1].bit_count()),-x[1].bit_count()))[:1600]
 R={(r[0],r[2]):r for r in C};V=set()
 for k,ls in best[1]:
  for c in ls:
   r=R.get((k,c))
   if r:V.update(r[1]);break
 return best[1] if set(B)-V=={'T0033'} and best[0]<base-1e-9 else SG
