...
def _pick_hard_scarce_best(solutions,candidates,all_tasks):
	...
	if not A:return[]
	D=sorted(A,key=lambda s:_solution_expected_cost(s,B,C))[:6]
	return min(D,key=lambda s:(_hard_scarce_shadow_cost(s,B,C),_solution_expected_cost(s,B,C)))
...
def _hard_scarce_shadow_cost(result,candidates,all_tasks):
	...
	return C+180.*(len(I)-len(B))+14.*L+N+M/5.
