...
def solve(input_text):
	...
	LE=_low_extra_column_probe(C,B,L,d,e,A)
	if LE is not _A:return LE
...
def _low_extra_column_probe(candidates,all_tasks,task_count,courier_count,avg_willingness,deadline):
	if task_count!=30 or courier_count!=60 or avg_willingness>=.20:return _A
	H=[r for r in candidates if len(r[1])==1]
	if not H:return _A
	R=_search_column_window(H,all_tasks,min(deadline,time.monotonic()+2.2),top_riders_per_task_key=18,max_k=4,option_limit=180)
	return R if R else _A
