...
def _augment_scarce_cache(base,candidates,all_tasks):
	bc=_solution_expected_cost(base,candidates,all_tasks);best=base;end=time.monotonic()+.22;T={A:i for i,A in enumerate(sorted(all_tasks))};cs=sorted({A[2]for A in candidates});C={A:i for i,A in enumerate(cs)};basekeys={(A,B[0])for A,B in base};raw=[]
	for A in candidates:
		if A[2]not in C or any(B not in T for B in A[1]):continue
		g=_group_expected_cost([A],len(A[1]));sv=1e2*len(A[1])-g
		if sv<=1e-9 and (A[0],A[2])not in basekeys:continue
		m=0
		for B in A[1]:m|=1<<T[B]
		m|=1<<(40+C[A[2]]);raw.append((sv,g,m,A))
	raw=sorted(raw,key=lambda A:(A[0],len(A[3][1]),-A[1]),reverse=_B);keep=set();bt={A:0 for A in all_tasks};bcou={A:0 for A in cs}
	for i,A in enumerate(raw):
		if (A[3][0],A[3][2])in basekeys:keep.add(i)
		ok=_D
		for B in A[3][1]:
			if bt[B]<7:bt[B]+=1;ok=_B
		if bcou[A[3][2]]<4:bcou[A[3][2]]+=1;ok=_B
		if ok or len(keep)<95:keep.add(i)
	items=[raw[i]for i in sorted(keep)[:125]];n=len(items);conf=[0]*n;sv=[A[0]for A in items]
	for i in range(n):
		for j in range(i+1,n):
			if items[i][2]&items[j][2]:conf[i]|=1<<j;conf[j]|=1<<i
	bs=1e2*len(all_tasks)-bc;bp=[]
	for i,A in enumerate(items):
		if (A[3][0],A[3][2])in basekeys:bp.append(i)
	def ub(av):
		s=0.;m=av
		while m:
			b=m&-m;i=b.bit_length()-1;s+=sv[i];m-=b
		return s
	def dfs(av,cur,pick):
		nonlocal bs,bp
		if time.monotonic()>end or cur+ub(av)<=bs+1e-9:return
		if cur>bs:bs=cur;bp=pick[:]
		if not av:return
		m=av;bi=-1;bv=-1.
		while m:
			b=m&-m;i=b.bit_length()-1;m-=b;v=sv[i]*(1+((conf[i]&av).bit_count()))
			if v>bv:bi=i;bv=v
		dfs(av&~conf[bi]&~(1<<bi),cur+sv[bi],pick+[bi]);dfs(av&~(1<<bi),cur,pick)
	dfs((1<<n)-1,0.,[])
	if 1e2*len(all_tasks)-bs<bc-1e-9:best=[(items[i][3][0],[items[i][3][2]])for i in bp];bc=1e2*len(all_tasks)-bs
	for E,F,G,H in((2,5,6,7),(3,8,8,9),(4,8,8,9)):
		B=_boer(best,candidates,all_tasks,E,F,G,H);D=_solution_expected_cost(B,candidates,all_tasks)
		if D<bc-1e-9:best=B;bc=D
	R={(A[0],A[2]):A for A in candidates};K={}
	for A in candidates:K.setdefault(A[0],[]).append(A)
	used={C for A,B in best for C in B};chg=_B
	while chg:
		chg=_D;bd=0;bi=br=_A
		for i,(k,ls)in enumerate(best):
			cur=[R[(k,C)]for C in ls if(k,C)in R];old=_group_expected_cost(cur,len(cur[0][1]))if cur else 1e9
			for r in K.get(k,()):
				if r[2]not in used:
					d=old-_group_expected_cost(cur+[r],len(r[1]))
					if d>bd:bd=d;bi=i;br=r
		if br is not _A:best[bi][1].append(br[2]);used.add(br[2]);bc-=bd;chg=_B
	return best
...
def _boer(result,candidates,all_tasks,max_remove_groups=2,top_group_limit=6,top_rows_per_task=6,max_open_tasks=8):
	...
	E=_solution_covered_count(result,candidates);F=result;G=_solution_expected_cost(result,candidates,all_tasks);H=set(all_tasks);J=[A[2]for A in sorted(_selected_repair_groups(C),reverse=_B)[:top_group_limit]]
	if not J:J=list(C)[:top_group_limit]
	for K in range(1,min(max_remove_groups,len(J))+1):
		for L in itertools.combinations(J,K):
			...
			if len(P)>max_open_tasks:continue
			R={}
			for B in candidates:
				S=set(B[1])
				if B[2]not in N and B[0]not in M and S and S<=P:R.setdefault(B[0],[]).append(B)
			for B in R:R[B]=sorted(R[B],key=lambda c:(_group_expected_cost([c],len(c[1])),-c[4],c[5]))[:top_rows_per_task]
			W=[B for A in sorted(R,key=lambda B:(len(R[B]),B))for B in R[A]];X=[];Y=set();Z=set()
			def dfs():
				nonlocal F,G
				if len(Z)==len(P):
					Q=[(A,[B[2]])for A in M for B in M[A]]+[(B[0],[B[2]])for B in X];D=_solution_expected_cost(Q,candidates,all_tasks)
					if D<G-1e-9 and _solution_covered_count(Q,candidates)>=E:F=Q;G=D
					return
				T=_A;U=_A
				for A in sorted(P-Z):
					V=[B for B in W if A in B[1]and B[2]not in Y and not(set(B[1])&Z)]
					if not V:return
					if U is _A or len(V)<len(U):T=A;U=V
				for B in U:
					V=set(B[1]);X.append(B);Y.add(B[2]);Z.update(V);dfs();Z.difference_update(V);Y.remove(B[2]);X.pop()
			dfs()
	return F
