	...
	for i,(ki,li)in enumerate(base):
		...
		for j,(kj,lj)in enumerate(base):
			...
			for c in lj:
				r=R.get((ki,c))
				if r is _A:continue
				if old<=_group_expected_cost(rows+[r],len(r[1])):continue
				cur=[(A,list(B))for A,B in base if A!=kj];cur[i if i<j else i-1][1].append(c);used={C for A,B in cur for C in B};miss=set(R[(kj,lj[0])][1])
