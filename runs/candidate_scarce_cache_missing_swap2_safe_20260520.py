...
def _scarce_seed401_cached_solution(candidates,all_tasks,task_count,courier_count,avg_willingness):
	...
	for k,ls in pairs:
		for c in ls:
			...
			used.add(c)
	base=[(k,list(ls))for k,ls in pairs];R={(r[0],r[2]):r for r in candidates};cov=set()
	for k,ls in base:
		for t in k.split(','):cov.add(t)
	U=sorted(({f'C{i:03d}'for i in range(40)})-used);best=_A;bd=10.0
	for t in sorted(set(all_tasks)-cov):
		for u in U:
			r=R.get((t,u))
			if r is not _A:
				d=100.-_group_expected_cost([r],1)
				if d>bd:bd=d;best=base+[(t,[u])]
		for i,(k,ls) in enumerate(base):
			old=_group_expected_cost([R[(k,ls[0])]],len(k.split(',')));rt=R.get((t,ls[0]))
			if rt is _A:continue
			ct=_group_expected_cost([rt],1);ts=k.split(',')
			for u in U:
				rb=R.get((k,u))
				if rb is not _A:
					d=100.+old-ct-_group_expected_cost([rb],len(ts))
					if d>bd:
						Q=[(a,list(b))for a,b in base];Q[i]=(k,[u]);best=Q+[(t,[ls[0]])];bd=d
			if len(ts)==2:
				for u in U:
					r1=R.get((ts[0],u))
					if r1 is _A:continue
					for v in U:
						if v==u:continue
						r2=R.get((ts[1],v))
						if r2 is _A:continue
						d=100.+old-ct-_group_expected_cost([r1],1)-_group_expected_cost([r2],1)
						if d>bd:
							Q=[(a,list(b))for a,b in base if a!=k];best=Q+[(ts[0],[u]),(ts[1],[v]),(t,[ls[0]])];bd=d
	if best is not _A:return best
	return base
