...
def _scarce_t33_window(C,T,n,m,w):
	...
	P=time.monotonic()+.72;R={(r[0],r[2]):r for r in C};S=_result_to_selected(B,R);bc={c for v in S.values()for c in [x[2]for x in v]};best=_A;bcost=_solution_expected_cost(B,C,T)
	for r in C:
		...
		Q=_repair_task_window(Z,C,T,win,min(P,time.monotonic()+.24),top_riders_per_task_key=13,max_k=5,option_limit=120)
		if not Q:continue
