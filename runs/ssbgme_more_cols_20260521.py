...
def _ssbgme(candidates,all_tasks,deadline):
	...
	if not D:return[]
	D.sort(key=lambda item:(_popcount(item[2]),item[0]/max(1,len(item[3])),item[0]/max(item[1],1e-09),item[0]),reverse=_B);D=D[:140];R=[(0,frozenset(),())];Z=list(R);K=_csbgwm((),J,I);a=_solution_expected_cost(K,M,G)if K else float(_F);h=min(len(G)//2+2,18)
	for b in range(h):
		...
		if not B:break
		B=_psbgs(B,I,J,max_states=180);Z.extend(B);R=B
	e=set()
	for(C,L,S)in Z:
