...
def solve(input_text):
	...
	elif J:D=_pick_low_business_shape_best(E,C,B)
...
def _pick_low_business_shape_best(solutions,candidates,all_tasks):
	B=_pick_low_robust_best(solutions,candidates,all_tasks);C=_solution_expected_cost(B,candidates,all_tasks);R={(a[0],a[2]):a for a in candidates}
	def shape(sol):
		pen=_C
		for k,cs in sol:
			rows=[R.get((k,c)) for c in cs];rows=[r for r in rows if r is not _A]
			if not rows:pen+=100.;continue
			pen+=abs(len(rows)-2)*1.5+max(0,len(rows[0][1])-1)*3.
		return pen
	D=min([s for s in solutions if s],key=lambda s:(_solution_expected_cost(s,candidates,all_tasks)+shape(s),_solution_expected_cost(s,candidates,all_tasks)))
	if _solution_expected_cost(D,candidates,all_tasks)<=C+1e-09:return D
	return B
