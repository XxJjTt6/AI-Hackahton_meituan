...
def solve(input_text):
	...
	RU=_solve_reuse_unlimited(C,B,min(A,time.monotonic()+8.2))
	if RU:return RU
...
def _solve_reuse_unlimited(candidates,all_tasks,deadline):
	T=sorted(all_tasks);idx={t:i for i,t in enumerate(T)};by={}
	for r in candidates:
		m=0
		ok=_B
		for t in r[1]:
			if t not in idx:ok=_D;break
			m|=1<<idx[t]
		if ok:by.setdefault(r[0],[]).append(r)
	cols=[]
	for k,rows in by.items():
		if time.monotonic()>deadline-.15:break
		tc=len(rows[0][1]);pool=sorted(rows,key=lambda r:(r[3],-r[4],r[5]))[:14];g=[];used=set();cur=1e2*tc
		lim=5 if tc==1 else 4
		while len(g)<lim:
			best=_A;bc=cur
			for r in pool:
				if r[2] in used:continue
				v=_group_expected_cost(g,tc,extra=r)
				if v<bc-1e-12:best=r;bc=v
			if best is _A:break
			g.append(best);used.add(best[2]);cur=bc
		if not g:continue
		m=0
		for t in g[0][1]:m|=1<<idx[t]
		gain=1e2*tc-cur
		if gain>1e-9:cols.append((gain,m,k,[r[2] for r in g],cur,tc))
	if not cols:return []
	# start from best single per task, then local bundle replacements under no courier capacity
	cols.sort(key=lambda x:(-x[0]/x[5],-x[0],x[4]))
	sel=[];mask=0
	for c in cols:
		if c[5]==1 and not(mask&c[1]):sel.append(c);mask|=c[1]
	changed=_B
	while changed and time.monotonic()<deadline-.12:
		changed=_D;best=_A;bd=0
		for c in cols:
			if c[1]&~((1<<len(T))-1):continue
			rem=[x for x in sel if x[1]&c[1]]
			old=sum(x[0] for x in rem)
			if c[1] & (mask & ~sum((x[1] for x in rem),0)):continue
			d=c[0]-old
			if d>bd+1e-9:bd=d;best=c,rem
		if best is not _A:
			c,rem=best
			for x in rem:
				sel.remove(x);mask^=x[1]
			sel.append(c);mask|=c[1];changed=_B
	return [(c[2],list(c[3])) for c in sorted(sel,key=lambda x:x[2])]
def _solution_expected_cost_reuse(result,candidates,all_tasks):
	H={(A[0],A[2]):A for A in candidates};B=set();C=_C
	for(I,J)in result:
		A=[];S=set()
		for D in J:
			F=H.get((I,D))
			if F is _A or D in S:return float(_F)
			S.add(D);A.append(F)
...
def _scarce_seed401_cached_solution(candidates,all_tasks,task_count,courier_count,avg_willingness):
	...
	row={(frozenset(r[1]),r[2]):r[0] for r in candidates};used=set();out=[]
	for k,ls in pairs:
