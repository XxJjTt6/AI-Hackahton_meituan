	...
	while changed:
		...
		for i,a in enumerate(keys):
			for b in keys[i+1:]:
				...
				if changed:break
			if changed:break
	# Allow one courier move: one task becomes K=1, another K=3.
	keys=list(sel)
	for a in keys:
		for r in list(sel[a]):
			na=[x for x in sel[a] if x!=r];old_a=_group_expected_cost(sel[a],1);new_a=_group_expected_cost(na,1)
			for b in keys:
				if a==b:continue
				rb=row.get((b,r[2]))
				if rb is _A:continue
				old_b=_group_expected_cost(sel[b],1);new_b=_group_expected_cost(sel[b]+[rb],1)
				if new_a+new_b<old_a+old_b-1e-6:
					sel[a]=na;sel[b]=sel[b]+[rb];best+=new_a+new_b-old_a-old_b;break
