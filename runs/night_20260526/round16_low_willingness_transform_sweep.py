...
'''
# transform parsed willingness A7 for low only by changing assignment C.append(...A7...)
old='C.append((T,c,U,A6,A7,A3))'
variants={
 'baseline':[],
 'low_sqrt_w':['C.append((T,c,U,A6,A7**.5 if A7<.5 else A7,A3))'],
 'low_square_w':['C.append((T,c,U,A6,A7*A7 if A7<.5 else A7,A3))'],
 'low_floor_w':['C.append((T,c,U,A6,max(A7,.08) if A7<.5 else A7,A3))'],
 'low_softplus_w':['C.append((T,c,U,A6,min(.999,A7*1.35+.02) if A7<.5 else A7,A3))'],
 'low_harsh_w':['C.append((T,c,U,A6,max(.0001,A7*.65-.01) if A7<.5 else A7,A3))'],
}
def make(name,repls):
 s=SRC.read_text()
 for rep in repls:
 ...
 p=ROOT/'runs/night_20260526'/f'round16_{name}.py'; p.write_text(s); return p
def run(p,case,n):
 rows=[]
 for _ in range(n):
  cp=subprocess.run([sys.executable,'-c',CHILD,str(p),case],text=True,capture_output=True,timeout=45)
  rows.append(json.loads(cp.stdout) if cp.returncode==0 else {'err':cp.stderr[-600:]})
 return rows
def summ(rows):
...
(ROOT/'runs/night_20260526/round16_low_willingness_transform_sweep.json').write_text(json.dumps(report,indent=2,sort_keys=True))
