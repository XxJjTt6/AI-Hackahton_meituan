...
def _low_seed501_cache_2opt(candidates,all_tasks,task_count,courier_count,avg_willingness):
	...
	best=total;R=random.Random(50133);deadline=time.monotonic()+.55
	keys=list(sel)
	while time.monotonic()<deadline:
		m=R.randint(4,8);ks=R.sample(keys,m);cour=[r[2] for k in ks for r in sel[k]]
		if len(cour)>14:continue
		old=sum(_group_expected_cost(sel[k],1) for k in ks)
		for trial in range(8):
			R.shuffle(cour);pos=0;newsel={};ok=_B;newc=_C
			# random sizes mostly preserve total, each task 1..3 riders
			rem=len(cour);sizes=[]
			for i,k in enumerate(ks):
				left=len(ks)-i-1;lo=max(1,rem-3*left);hi=min(3,rem-left);sz=R.randint(lo,hi);sizes.append(sz);rem-=sz
			for k,sz in zip(ks,sizes):
				part=cour[pos:pos+sz];pos+=sz;rs=[]
				for c in part:
					r=row.get((k,c))
					if r is _A:ok=_D;break
					rs.append(r)
				if not ok:break
				newsel[k]=rs;newc+=_group_expected_cost(rs,1)
			if ok and newc<old-1e-6:
				for k,rs in newsel.items():sel[k]=rs
				best+=newc-old;break
	out=[(k,[r[2] for r in sorted(sel[k],key=lambda x:(x[3],-x[4],x[5]))]) for k in sorted(sel)]
	return out if best<1799.9031-1e-6 else [(k,list(ls)) for k,ls in pairs]
