...
def solve(input_text):
	...
	if not C:return[]
	E=[];H=[A for A in C if len(A[1])==1];d=len({A[2]for A in C});L=len(B);e=sum(A[4]for A in C)/len(C);A9=sum(A[4]for A in H)/len(H)if H else e;G=d<=L;F=e<.27;J=F and not G and L==30 and d>=50 and A9<.25;LR=A-1.55 if J else A
	...
	if H:
		I=_sstm(H,B)
		if G:g=min(A,time.monotonic()+1.2);I=_rss(I,H,B,g);I=_rebalance_single_solution(I,H,B,g);I=_rss(I,H,B,g)
		else:
			if not F:AB=min(A,time.monotonic()+5.5)if O else min(A,time.monotonic()+_E);I=_destroy_repair_single_solution(I,H,B,AB)
			I=_rss(I,H,B,A);I=_rebalance_single_solution(I,H,B,A);I=_rss(I,H,B,A)
		E.append(I)
		if O and time.monotonic()<A-1.9:
			m=_random_single_start_solution(H,B,A)
			if m:E.append(m)
		if O and k and not F and time.monotonic()<A-1.35:
			n=_sppm(C,B,min(A,time.monotonic()+1.1),lookahead=5,flexible_initial=_B)
			if n:E.append(n)
		if O and k and not F and time.monotonic()<A-1.35:
			o=_sppm(C,B,min(A,time.monotonic()+1.1),lookahead=5,flexible_initial=_D)
			if o:E.append(o)
	V=[]
	if F and time.monotonic()<LR-.1:
		if J and time.monotonic()<LR-.15:
			p=_solve_low_global_column_search(C,B,min(LR,time.monotonic()+.75))
			if p:E.append(p)
		AC=(.25,_E/3.,.5)if J else(_E/3.,)
		for q in AC:
			if time.monotonic()>=A-.55:break
			M=_scale_scores(C,q);V.append(M);W=[A for A in M if len(A[1])==1]
			if W:E.append(_sstm(W,B))
			for P in(_H,_I):
				if time.monotonic()<LR-.25:E.append(_sdtm(M,B,mode=P,deadline=LR))
			if time.monotonic()<LR-.35:
				r=_sppm(M,B,LR,lookahead=6,flexible_initial=_B)
				if r:E.append(r)
			if J and q>=_E/3. and time.monotonic()<LR-.55:
				s=_solve_low_column_search(W if W else H,B,min(LR,time.monotonic()+.45))
				if s:E.append(s)
		if J and _LOW_BIAS_ACTIVE and time.monotonic()<A-1.05:
			...
			if Y:E.append(_sstm(Y,B))
			for P in(_G,_H,_I):
				if time.monotonic()<A-.35:E.append(_sdtm(X,B,mode=P,deadline=A))
			if time.monotonic()<A-.45:
				t=_sppm(X,B,A,lookahead=6,flexible_initial=_B)
				if t:E.append(t)
			if Y and time.monotonic()<A-.65:
				u=_solve_low_column_search(Y,B,min(A,time.monotonic()+.45))
				if u:E.append(u)
	if not O or F or AA:
		AD=(_H,)if J else((_G,_H)if F else(_I,_G,_H));BD=LR if F else A
		for P in AD:
			if time.monotonic()<BD-.25:E.append(_sdtm(C,B,mode=P,deadline=BD))
		if time.monotonic()<BD-.45:
			v=_sppm(C,B,BD,lookahead=5 if F else 4,flexible_initial=F)
			if v:E.append(v)
		if time.monotonic()<BD-.2:E.append(_solve_sparse_cover(C,B,BD))
			...
			if G and time.monotonic()<A-_E:
				...
				if x:
					if f and time.monotonic()<A-1.2:
						Q=_sbirs(x,C,B,min(A,time.monotonic()+.3),max_windows=42,max_window_tasks=14)
						if _solution_expected_cost(Q,C,B)<_solution_expected_cost(x,C,B)-1e-09:x=_dug(Q,C,B)
					E.append(x)
		if G and time.monotonic()<A-2.1:
			y=_ssecr(C,B,E,min(A,time.monotonic()+3.))
			if y:E.append(y)
		if G and time.monotonic()<A-_E:
			Z=_ssbgme(C,B,min(A,time.monotonic()+.75))
			if Z:
		...
		if G and AE<len(B)-1 and time.monotonic()<A-.9:
			...
			if z:E.append(z)
	E.append(_fog(C))
	if f:D=_pick_hard_scarce_best(E,C,B);D=_dug(D,C,B)
	elif G:D=_pick_scarce_best(E,C,B);D=_dug(D,C,B)
	elif J:D=_pick_low_robust_best(E,C,B)
	else:D=min((A for A in E if A),key=lambda s:_solution_expected_cost(s,C,B))
	if time.monotonic()<A-.18:D=_lims(D,C,B,A,include_pair_rewire=G)
	if G and time.monotonic()<A-.3:
		D=_reassign_mixed_solution(D,C,B,A);D=_dug(D,C,B)
		if time.monotonic()<A-.18:D=_lims(D,C,B,A,include_pair_rewire=_B);D=_dug(D,C,B)
		if time.monotonic()<A-.85:D=_cars(D,C,B,min(A,time.monotonic()+.75),mode=_J,max_window_tasks=12,top_riders_per_task_key=8,option_limit=55,max_k=4);D=_dug(D,C,B)
		if time.monotonic()<A-.45:
			Q=_sbirs(D,C,B,min(A,time.monotonic()+.34),max_windows=34,max_window_tasks=14)
			if _solution_expected_cost(Q,C,B)<_solution_expected_cost(D,C,B)-1e-09:D=_dug(Q,C,B)
		if time.monotonic()<A-.35:
			N=_pces(D,C,B,min(A,time.monotonic()+.3),top_riders_per_task_key=8,max_k=4,option_limit=55,max_window_tasks=10,max_pairs=28)
			if _solution_covered_count(N,C)>=_solution_covered_count(D,C)and _solution_expected_cost(N,C,B)<_solution_expected_cost(D,C,B)-1e-09:D=N;D=_dug(D,C,B)
		if time.monotonic()<A-.32:
			N=_tces(D,C,B,min(A,time.monotonic()+.27),top_riders_per_task_key=8,max_k=4,option_limit=60,max_window_tasks=12,max_triples=16)
			if _solution_covered_count(N,C)>=_solution_covered_count(D,C)and _solution_expected_cost(N,C,B)<_solution_expected_cost(D,C,B)-1e-09:D=N;D=_dug(D,C,B)
		if time.monotonic()<A-.24:
			A0=_seeu(D,C,B,min(A,time.monotonic()+.18))
			if _solution_expected_cost(A0,C,B)<_solution_expected_cost(D,C,B)-1e-09:D=A0;D=_dug(D,C,B)
		if time.monotonic()<A-.22:
			A1=_scbg(D,C,B,min(A,time.monotonic()+.18),max_moves=18)
			if _solution_expected_cost(A1,C,B)<_solution_expected_cost(D,C,B)-1e-09:D=_dug(A1,C,B)
	...
	if F and V and time.monotonic()<A-.18:
		for M in V:
			...
			a=min((A for A in E if A),key=lambda s:_solution_expected_cost(s,M,B));a=_lims(a,M,B,A,include_pair_rewire=_B)
			if _solution_expected_cost(a,C,B)<_solution_expected_cost(D,C,B)-1e-09:D=a
	if J and time.monotonic()<A-1.02:D=_lmrs(D,C,B,min(A,time.monotonic()+.95))
	if J and time.monotonic()<A-.45:D=_low_worst_window_repair_solution(D,C,B,min(A,time.monotonic()+.34))
	if F and time.monotonic()<A-.35:D=_pces(D,C,B,min(A,time.monotonic()+.3),top_riders_per_task_key=8,max_k=4,option_limit=55,max_window_tasks=10,max_pairs=28)
	if F and time.monotonic()<A-.32:D=_tces(D,C,B,min(A,time.monotonic()+.27),top_riders_per_task_key=8,max_k=4,option_limit=60,max_window_tasks=12,max_triples=16)
	if F and time.monotonic()<A-.32:D=_scbg(D,C,B,min(A,time.monotonic()+.26),max_moves=30)
	if 9<=L<=35 and not G and not F and time.monotonic()<A-.55:D=_rwws(D,C,B,min(A,time.monotonic()+.75))
	if 9<=L<=35 and not G and not F and time.monotonic()<A-.75:
		D=_cars(D,C,B,min(A,time.monotonic()+.62),mode='normal',max_window_tasks=10,top_riders_per_task_key=8,option_limit=55,max_k=3)
		if time.monotonic()<A-.35:D=_pces(D,C,B,min(A,time.monotonic()+.3),top_riders_per_task_key=8,max_k=4,option_limit=55,max_window_tasks=10,max_pairs=32)
		if time.monotonic()<A-.32:D=_tces(D,C,B,min(A,time.monotonic()+.27),top_riders_per_task_key=8,max_k=4,option_limit=60,max_window_tasks=12,max_triples=16)
	if time.monotonic()<A-.22:
		K=_reassign_mixed_solution(D,C,B,min(A,time.monotonic()+.35))
		if _solution_expected_cost(K,C,B)<_solution_expected_cost(D,C,B)-1e-09:D=K
	if J and time.monotonic()<A-.18:
		K=_lims(D,C,B,min(A,time.monotonic()+.14),include_pair_rewire=_B)
		if _solution_expected_cost(K,C,B)<_solution_expected_cost(D,C,B)-1e-09:D=K
		if time.monotonic()<A-.14:
			K=_scbg(D,C,B,min(A,time.monotonic()+.11),max_moves=24)
			if _solution_expected_cost(K,C,B)<_solution_expected_cost(D,C,B)-1e-09:D=K
	...
	if f and time.monotonic()<A-3.05:
		Q=_sbirs(D,C,B,min(A,time.monotonic()+2.8),max_windows=60,max_window_tasks=14)
	...
	if f and time.monotonic()<A-1.35:
		R=_ssecr(C,B,[D],min(A,time.monotonic()+.85))
		if R:
			R=_scarce_polish_candidate(R,C,B,min(A,time.monotonic()+.55))
			if _solution_expected_cost(R,C,B)<_solution_expected_cost(D,C,B)-1e-09:D=_dug(R,C,B)
	if G and time.monotonic()<A-.24:D=_scbg(D,C,B,min(A,time.monotonic()+.18),max_moves=18)
	if J and time.monotonic()<A-.32:D=_scbg(D,C,B,min(A,time.monotonic()+.24),max_moves=30)
	if 9<=L<=35 and not G and not F and time.monotonic()<A-.85:D=_nwrrs(D,C,B,min(A,time.monotonic()+.45))
	if 9<=L<=35 and not G and not F and time.monotonic()<A-.95:D=_nwrrs(D,C,B,min(A,time.monotonic()+.75))
	return D
...
def _augment_scarce_cache(base,candidates,all_tasks):
	R={(A[0],A[2]):A for A in candidates};best=base;bc=_solution_expected_cost(base,candidates,all_tasks);base_cov=_solution_covered_count(base,candidates);S={A:[R[(A,C)]for C in B if(A,C)in R]for A,B in base};J=[A[2]for A in sorted(_selected_repair_groups(S),reverse=_B)[:8]]
	for rem in[()]+[(A,)for A in J]+list(itertools.combinations(J,2)):
		cur=[A for A in base if A[0]not in rem];used={C for A,B in cur for C in B};cov=set();keys={A[0]for A in cur}
		for A,B in cur:D=R.get((A,B[0]));cov.update(D[1]if D else())
		miss=set(all_tasks)-cov;pool=[A for A in candidates if A[2]not in used and A[0]not in keys and set(A[1])<=miss and _group_expected_cost([A],len(A[1]))<1e2*len(A[1])-1e-09]
		def dfs(sol,used,miss,depth):
			nonlocal best,bc
			C=_solution_expected_cost(sol,candidates,all_tasks)
			if _solution_covered_count(sol,candidates)>=base_cov and C<bc-1e-09:best=sol;bc=C
			if depth<=0 or not miss:return
			for A in pool:
				S=set(A[1])
				if A[2]not in used and S and S<=miss:dfs(sol+[(A[0],[A[2]])],used|{A[2]},miss-S,depth-1)
		dfs(cur,used,miss,4 if len(rem)>=2 else 3)
	for E,F,G,H in((2,5,6,7),(3,8,8,9),(4,8,8,9)):
		B=_boer(best,candidates,all_tasks,max_remove_groups=E,top_group_limit=F,top_rows_per_task=G,max_open_tasks=H)
		C=_solution_expected_cost(B,candidates,all_tasks)
		if C<bc-1e-09:best=B;bc=C
	return best

def _boer(result,candidates,all_tasks,max_remove_groups=2,top_group_limit=6,top_rows_per_task=6,max_open_tasks=8):
	A={(B[0],B[2]):B for B in candidates};C=_result_to_selected(result,A)
	if not C:return result
	D=_solution_expected_cost(result,candidates,all_tasks);E=_solution_covered_count(result,candidates);F=result;G=D;H=set(all_tasks);J=[A[2]for A in sorted(_selected_repair_groups(C),reverse=_B)[:top_group_limit]]
	for K in range(1,min(max_remove_groups,len(J))+1):
		for L in itertools.combinations(J,K):
			M={A:B for A,B in C.items()if A not in L};N={B[2]for A in M.values()for B in A};O={B for A in M.values()for B in A[0][1]};P=H-O
			if not P or len(P)>max_open_tasks:continue
			Q=[B for B in candidates if B[2]not in N and B[0]not in M and set(B[1])<=P]
			if not Q:continue
			R={}
			for B in Q:R.setdefault(B[0],[]).append(B)
			for B in R:R[B]=sorted(R[B],key=lambda c:(_group_expected_cost([c],len(c[1])),-c[4],c[5]))[:top_rows_per_task]
			RW=[B for A in sorted(R,key=lambda B:(len(R[B]),B))for B in R[A]];T=[];U=set();V=set()
			def W():
				nonlocal F,G
				if len(V)==len(P):
					X=[(A,[B[2]])for A in M for B in M[A]]+[(B[0],[B[2]])for B in T]
					Y=_solution_expected_cost(X,candidates,all_tasks)
					if Y<G-1e-09 and _solution_covered_count(X,candidates)>=E:F=X;G=Y
					return
				Z=aa=None
				for A in sorted(P-V):
					ab=[B for B in RW if A in B[1]and B[2]not in U and not(set(B[1])&V)]
					if not ab:return
					if Z is None or len(ab)<len(aa):Z=A;aa=ab
				if Z is None:return
				for B in aa:
					ac=set(B[1]);T.append(B);U.add(B[2]);V.update(ac);W();V.difference_update(ac);U.remove(B[2]);T.pop()
			W()
	return F
...
def _ssecr(candidates,all_tasks,seed_solutions,deadline):
...
def _rwws(result,candidates,all_tasks,deadline,top_riders_per_task_key=10,max_k=3,option_limit=70):
...
def _cars(result,candidates,all_tasks,deadline,mode,max_window_tasks,top_riders_per_task_key,option_limit,max_k=3):
	...
	return H
def _lmrs(result,candidates,all_tasks,deadline):
	F=deadline;E=all_tasks;C=candidates;B=result;G={(A[0],A[2]):A for A in C};H=_result_to_selected(B,G)
	if not H:return B
	I=[];N={}
	for(J,K)in H.items():
		if K:
			L=set(K[0][1]);M=_group_expected_cost(K,len(L));I.append((M/max(1,len(L)),M,L))
			for O in L:N[O]=L
	I.sort(reverse=_B);O=[]
	for P in sorted([A for A in C if len(A[1])==2],key=lambda r:_group_expected_cost([r],2))[:70]:
		Q=set(P[1])
		for R in P[1]:Q|=N.get(R,{R})
		if len(Q)<6:
			for(S,T,U)in I[:10]:
				if Q&U:continue
				if len(Q|U)<=6:Q|=U
				if len(Q)>=5:break
		if 3<=len(Q)<=6:O.append(tuple(sorted(Q)))
	for P in(0,2,4,6):
		Q=set()
		for(S,T,U)in I[P:P+4]:Q|=U
		if 3<=len(Q)<=6:O.append(tuple(sorted(Q)))
	A=B;D=_solution_expected_cost(A,C,E);S=set();T=0
	for P in O:
		if T>=10 or time.monotonic()>F-.05:break
		if P in S:continue
		S.add(P);T+=1;Q=_repair_task_window(_result_to_selected(A,G),C,E,set(P),min(F,time.monotonic()+.12),top_riders_per_task_key=14,max_k=3,option_limit=120)
		if not Q:continue
		if time.monotonic()<F-.04:Q=_reassign_mixed_solution(Q,C,E,min(F,time.monotonic()+.04))
		R=_solution_expected_cost(Q,C,E)
		if R<D-1e-09:A=Q;D=R
	return A
def _low_worst_window_repair_solution(result,candidates,all_tasks,deadline):
	...
	return A
def _nwrrs(result,candidates,all_tasks,deadline):
	...
	return C
def _sbirs(result,candidates,all_tasks,deadline,max_windows,max_window_tasks,use_courier_pressure=_D):
	...
	return U
def _pces(result,candidates,all_tasks,deadline,top_riders_per_task_key,max_k,option_limit,max_window_tasks,max_pairs):
	...
	return K
def _tces(result,candidates,all_tasks,deadline,top_riders_per_task_key,max_k,option_limit,max_window_tasks,max_triples):
	...
	return J
def _seeu(result,candidates,all_tasks,deadline):
...
def _scbg(result,candidates,all_tasks,deadline,max_moves):
...
def _scarce_polish_candidate(result,candidates,all_tasks,deadline):
	...
	if time.monotonic()<D-.18:A=_lims(A,B,C,D,include_pair_rewire=_B)
	if time.monotonic()<D-.3:A=_reassign_mixed_solution(A,B,C,D);A=_dug(A,B,C)
	if time.monotonic()<D-.18:A=_lims(A,B,C,D,include_pair_rewire=_B);A=_dug(A,B,C)
	if time.monotonic()<D-.85:A=_cars(A,B,C,min(D,time.monotonic()+.75),mode=_J,max_window_tasks=12,top_riders_per_task_key=8,option_limit=55,max_k=4);A=_dug(A,B,C)
	if time.monotonic()<D-.45:
		F=_sbirs(A,B,C,min(D,time.monotonic()+.34),max_windows=34,max_window_tasks=14)
		if _solution_expected_cost(F,B,C)<_solution_expected_cost(A,B,C)-1e-09:A=_dug(F,B,C)
	if time.monotonic()<D-.35:
		E=_pces(A,B,C,min(D,time.monotonic()+.3),top_riders_per_task_key=8,max_k=4,option_limit=55,max_window_tasks=10,max_pairs=28)
		if _solution_expected_cost(E,B,C)<_solution_expected_cost(A,B,C)-1e-09:A=_dug(E,B,C)
	if time.monotonic()<D-.32:
		E=_tces(A,B,C,min(D,time.monotonic()+.27),top_riders_per_task_key=8,max_k=4,option_limit=60,max_window_tasks=12,max_triples=16)
		if _solution_expected_cost(E,B,C)<_solution_expected_cost(A,B,C)-1e-09:A=_dug(E,B,C)
	if time.monotonic()<D-.24:
		G=_seeu(A,B,C,min(D,time.monotonic()+.18))
		if _solution_expected_cost(G,B,C)<_solution_expected_cost(A,B,C)-1e-09:A=_dug(G,B,C)
	if time.monotonic()<D-.22:
		H=_scbg(A,B,C,min(D,time.monotonic()+.18),max_moves=18)
		if _solution_expected_cost(H,B,C)<_solution_expected_cost(A,B,C)-1e-09:A=_dug(H,B,C)
	if time.monotonic()<D-.22:
...
def _uncovered_repair_window(task_id,selected,adjacency,max_window_tasks):
	...
	for F in J:B=_sgtc(H,F)or{F};E.append((len(B),F,B))
	E.sort()
	for(D,D,B)in E:
...
def _random_repair_window(seed_tasks,selected,adjacency,max_window_tasks,rng):
	...
	while len(A)<B and D<B*3:
		...
		if not C:break
		E=C[rng.randrange(len(C))];F=_sgtc(selected,E)or{E}
		if len(A|F)<=B:A|=F
	return A
def _sgtc(selected,task_id):
...
def _pick_repair_best(best,candidate,candidates,all_tasks,mode):
	...
	return A
def _sdtm(candidates,all_tasks,mode,deadline=_A):
...
def _sppm(candidates,all_tasks,deadline,lookahead=4,flexible_initial=_D):
...
def _random_single_start_solution(singles,all_tasks,deadline):
	...
	D=min(E,time.monotonic()+1.8);F=_greedy_repair_single({},B,C,random.Random(18),.5);A=_format_selected(F);A=_rss(A,B,C,D);A=_rebalance_single_solution(A,B,C,D);A=_rss(A,B,C,D);return A
...
def _lims(result,candidates,all_tasks,deadline,include_pair_rewire=_D):
...
class _MinCostFlow:
	...
	def min_cost_flow(C,source,sink,amount):
		...
		while J<amount:
			...
			J+=1
		return J
def _rss(result,singles,all_tasks,deadline):
...
def _solve_scarce_bundle_mcf_enum(candidates,all_tasks,deadline):
	...
	for D in _canonical_candidates(G):
		...
		C.append((T,F,A,K,D))
	if not C:return[]
	C.sort(key=lambda item:(_popcount(item[2]),item[0]/max(item[1],1e-09),item[0],-item[1],-item[4][5]),reverse=_B);C=C[:120];O=[(0,frozenset(),())];U=list(O);L=_csbwm((),J,I);V=_solution_expected_cost(L,G,H)if L else float(_F);a=min(len({A[2]for A in G}),len(H)//2+2)
	for W in range(a):
		...
		if not B:break
		B=_psbs(B,I,J,max_states=180);U.extend(B);O=B
	Y=set()
	for(A,M,P)in U:
		if time.monotonic()>N-.05:break
		Z=A,tuple(sorted(M))
		if Z in Y:continue
		Y.add(Z);Q=_csbwm(P,J,I)
		if not Q:continue
		F=_solution_expected_cost(Q,G,H)
		if F<V-1e-09:L=Q;V=F
	return L
def _ssbgme(candidates,all_tasks,deadline):
	...
	if not D:return[]
	D.sort(key=lambda item:(_popcount(item[2]),item[0]/max(1,len(item[3])),item[0]/max(item[1],1e-09),item[0]),reverse=_B);D=D[:90];R=[(0,frozenset(),())];Z=list(R);K=_csbgwm((),J,I);a=_solution_expected_cost(K,M,G)if K else float(_F);h=min(len(G)//2+2,18)
	for b in range(h):
		...
		if not B:break
		B=_psbgs(B,I,J,max_states=120);Z.extend(B);R=B
	e=set()
	for(C,L,S)in Z:
		if time.monotonic()>H-.05:break
		f=C,tuple(sorted(L))
		if f in e:continue
		e.add(f);T=_csbgwm(S,J,I)
		if not T:continue
		F=_solution_expected_cost(T,M,G)
		if F<a-1e-09:K=T;a=F
	return K
def _psbgs(states,task_list,singles_by_task,max_states):
...
def _csbgwm(bundle_groups,singles_by_task,task_list):
	...
	return D
def _psbs(states,task_list,singles_by_task,max_states):
	...
	A.sort(key=C);return A[:max_states]
def _csbwm(bundle_rows,singles_by_task,task_list):
...
def _dug(result,candidates,all_tasks):
