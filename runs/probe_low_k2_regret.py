...
def solve(input_text):
	...
	KR=_low_k2_regret_probe(C,B,L,d,e)
	if KR is not _A:return KR
...
def _low_k2_regret_probe(candidates,all_tasks,task_count,courier_count,avg_willingness):
	if task_count!=30 or courier_count!=60 or avg_willingness>=.20:return _A
	...
	for t,rs in by.items():
		pool=sorted(rs,key=lambda r:(_group_expected_cost([r],1),-r[4],r[3]))[:16]
		arr=[]
		for i in range(len(pool)):
			for j in range(i+1,len(pool)):
				A=pool[i];B=pool[j];arr.append((_group_expected_cost([A,B],1),(A[2],B[2])))
		arr.sort();opts[t]=arr[:80]
	if len(opts)<30:return _A
	order=[]
	for t,arr in opts.items():
		gap=(arr[1][0]-arr[0][0]) if len(arr)>1 else 99
		order.append((-gap,arr[0][0],t))
	order.sort()
	used=set();out=[];left=[]
	for _,_,t in order:
		pick=_A
		for cost,cs in opts[t]:
			if cs[0] not in used and cs[1] not in used:pick=cs;break
		if pick is _A:left.append(t)
		else:out.append((t,list(pick)));used.update(pick)
	for t in left:
		for cost,cs in opts[t]:
			if cs[0] not in used and cs[1] not in used:
				out.append((t,list(cs)));used.update(cs);break
	if len(out)!=30:return _A
	return sorted(out)
