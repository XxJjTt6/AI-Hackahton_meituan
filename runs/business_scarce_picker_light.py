...
def _hard_scarce_business_cost(result,candidates,all_tasks):
	...
	for key,cs in result:
		...
		p=1-p
		risk+=max(_C,gc-100.*D)*.2+max(_C,.68-p)*10.*D
...
def _pick_hard_scarce_best(solutions,candidates,all_tasks):
	...
	for A in D:
		...
		if G:E.append(G)
	return min(E,key=lambda s:(_hard_scarce_business_cost(s,B,C),_hard_scarce_shadow_cost(s,B,C),_solution_expected_cost(s,B,C)))
