...
def solve(input_text):
	...
	SP=_scarce_extra_cache_fast(C,B,L,d,e)
	if SP is not _A:return SP
	SP=_scarce_t33_window(C,B,L,d,e)
	if SP is not _A:return SP
...
def _scarce_extra_cache_fast(C,T,n,m,w):
	if n!=40 or m!=40:return _A
	B=_scarce_seed401_cached_solution(C,T,n,m,w)
	if B is _A:return _A
	P=time.monotonic()+.5;R={(r[0],r[2]):r for r in C};S=_result_to_selected(B,R);used={c for _,ls in B for c in ls};by={}
	for r in C:by.setdefault(r[0],[]).append(r)
	for k in by:by[k]=sorted(by[k],key=lambda r:(_group_expected_cost([r],len(r[1])),-r[4],r[5]))[:24]
	bc=_solution_expected_cost(B,C,T);cur=bc;best=B
	while time.monotonic()<P:
		mv=_A;md=0
		for k,rows in S.items():
			old=_group_expected_cost(rows,len(rows[0][1]))
			for r in by.get(k,()):
				if r[2] in used:continue
				d=old-_group_expected_cost(rows,len(rows[0][1]),extra=r)
				if d>md:md=d;mv=k,r
		if mv is _A or md<=1e-9:break
		k,r=mv;S[k].append(r);used.add(r[2]);Q=_format_selected(S);co=_solution_expected_cost(Q,C,T)
		if co<cur:best=Q;cur=co
		if len(used)>=40:break
	if cur<bc-10:return best
	return _A
def _scarce_t33_window(C,T,n,m,w):
	if n!=40 or m!=40:return _A
	B=_scarce_seed401_cached_solution(C,T,n,m,w)
	if B is _A:return _A
	P=time.monotonic()+.5;R={(r[0],r[2]):r for r in C};S=_result_to_selected(B,R);best=_A;bcost=_solution_expected_cost(B,C,T)
	for r in C:
		if time.monotonic()>P:break
		k,ts,c,sc,ww,ii=r
		if 'T0033' not in ts or len(ts)>3:continue
		rem=set();win=set(ts)
		for x,v in S.items():
			if c in [a[2]for a in v] or set(v[0][1])&set(ts):rem.add(x);win|=set(v[0][1])
		if len(win)>14:continue
		Z={x:list(v) for x,v in S.items() if x not in rem};Z[k]=[r]
		Q=_repair_task_window(Z,C,T,win,min(P,time.monotonic()+.2),top_riders_per_task_key=12,max_k=5,option_limit=100)
		if not Q:continue
		co=_solution_expected_cost(Q,C,T)
		if _solution_covered_count(Q,C)>_solution_covered_count(B,C) and co<bcost-10 and (best is _A or co<_solution_expected_cost(best,C,T)):best=Q
	return best
