...
def _scarce_seed401_cached_solution(candidates,all_tasks,task_count,courier_count,avg_willingness):
	...
	for k,ls in pairs:
		for c in ls:
			...
			used.add(c)
	base=_augment_scarce_cache([(k,list(ls)) for k,ls in pairs],candidates,all_tasks)
	B=_sraw(candidates,all_tasks,base,time.monotonic()+.65)
	if B:return B
	return base

def _sraw(candidates,all_tasks,base,deadline):
	T={A:i for i,A in enumerate(sorted(all_tasks))};bc=1531.5317;by={}
	for A in candidates:
		m=0
		for t in A[1]:m|=1<<T[t]
		if A[3]<1e2*len(A[1]):by.setdefault(A[2],[]).append((m,A[3],A))
	D={0:(_C,())};L=5000
	for c,rs in sorted(by.items(),key=lambda x:min(A[1]for A in x[1])):
		if time.monotonic()>deadline-.05:break
		E=dict(D)
		for mask,(cost,pick)in D.items():
			for m,sc,A in sorted(rs,key=lambda x:x[1])[:35]:
				if mask&m:continue
				nm=mask|m;nc=cost+sc;old=E.get(nm)
				if old is _A or nc<old[0]:E[nm]=(nc,pick+(A,))
		if len(E)>L:E=dict(sorted(E.items(),key=lambda x:(x[1][0]+1e2*(len(all_tasks)-_popcount(x[0])),-_popcount(x[0])))[:L])
		D=E
	best=_A;bv=bc
	for m,(c,pick)in D.items():
		v=c+1e2*(len(all_tasks)-_popcount(m))
		if v<bv:bv=v;best=pick
	return[(A[0],[A[2]])for A in best]if best else[]
