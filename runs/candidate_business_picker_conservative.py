...
def _pick_scarce_best(solutions,candidates,all_tasks):
	...
	if not A:return[]
	# portfolio/business view: prefer lower expected loss, but break ties by fewer groups/couriers to reduce runtime-sensitive over-dispatch risk
	return min(A,key=lambda s:(_solution_expected_cost(s,candidates,all_tasks),len(s),sum(len(cs) for _,cs in s)))
