...
def solve(input_text):
	...
	if G and time.monotonic()<A-.3:
		...
		if not F and time.monotonic()<A-.55:D=_scarce_force_uncovered_window(D,C,B,min(A,time.monotonic()+.42));D=_drop_unprofitable_groups(D,C,B)
		if time.monotonic()<A-.45:
...
def _scarce_bundle_insertion_repair_solution(result,candidates,all_tasks,deadline,max_windows,max_window_tasks,use_courier_pressure=_D):
	...
	return U
def _scarce_force_uncovered_window(result,candidates,all_tasks,deadline):
	M=deadline;H=candidates;L=all_tasks;R={(A[0],A[2]):A for A in H};B=_result_to_selected(result,R)
	if not B:return result
	base_cov=_solution_covered_count(result,H);base_cost=_solution_expected_cost(result,H,L);best=result;J=_task_adjacency(H)
	covered={T for rows in B.values() for T in rows[0][1]};missing=sorted(set(L)-covered)
	if not missing:return result
	for u in missing:
		if time.monotonic()>M-.06:break
		windows=[]
		windows.append(_uncovered_repair_window(u,B,J,16))
		for row in sorted([A for A in H if u in A[1]],key=lambda c:(_group_expected_cost([c],len(c[1])),-len(c[1]),c[5]))[:30]:
			W=set(row[1])
			for t in row[1]:W|=_selected_group_tasks_containing(B,t)or{t}
			if len(W)<=18:windows.append(W)
		seen=set()
		for W in windows:
			if time.monotonic()>M-.05:break
			if not W or u not in W or len(W)>18:continue
			K=tuple(sorted(W))
			if K in seen:continue
			seen.add(K);S=_result_to_selected(best,R);U=_repair_task_window(S,H,L,W,min(M,time.monotonic()+.16),top_riders_per_task_key=12,max_k=4,option_limit=95)
			if not U:continue
			if time.monotonic()<M-.05:U=_reassign_mixed_solution(U,H,L,min(M,time.monotonic()+.05))
			cov=_solution_covered_count(U,H);cost=_solution_expected_cost(U,H,L)
			if cov>=base_cov and cost<base_cost-1e-09:best=U;base_cost=cost;base_cov=cov
	return best
...
def _scarce_polish_candidate(result,candidates,all_tasks,deadline):
	...
	if time.monotonic()<D-.55:A=_scarce_force_uncovered_window(A,B,C,min(D,time.monotonic()+.42));A=_drop_unprofitable_groups(A,B,C)
