...
def _group_expected_cost(rows,task_count,extra=_A):
	...
	if J>12:B=_group_expected_cost_dp(A,C)
		...
		for L in range(1<<J):
			D=_E;K=_C;E=0
			for(M,F)in enumerate(sorted(A,key=lambda c:(c[3],-c[4],c[5]))):
...
def _pick_low_robust_best(solutions,candidates,all_tasks):
	...
	def G(solution):E=solution;D=_solution_expected_cost(E,A,B);F=_solution_expected_cost_by_model(E,A,B,_L);G=_solution_expected_cost_by_model(E,A,B,_M);H=.45*D+.45*F+.1*G;I=max(D-C,F-C,G-C);return H+.05*max(_C,I),max(D,F,G),D
