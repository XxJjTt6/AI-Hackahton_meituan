...
def solve(input_text):
	...
	if J and time.monotonic()<A-.18:
		K2=_low_force_single_k2_solution(D,C,B,min(A,time.monotonic()+.14))
		if _low_official_like_score(K2,C,B)<_low_official_like_score(D,C,B)-1e-09:D=K2
...
def _low_force_single_k2_solution(result,candidates,all_tasks,deadline):
	# Official low_willingness baseline is 30 single-task groups with exactly 2 couriers.
	# Convert multi-courier/multi-task groups back to best 2-courier single-task groups
	# when this official-like score improves.
	D=deadline;C=candidates;B=all_tasks;row_map={(A[0],A[2]):A for A in C};used=set();out=[];covered=set()
	# Prefer preserving existing two-courier single-task assignments.
	for key,cs in result:
		rows=[row_map.get((key,c)) for c in cs];rows=[r for r in rows if r is not _A]
		if not rows or len(rows[0][1])!=1 or len(rows)!=2:continue
		if any(r[2] in used for r in rows):continue
		t=rows[0][1][0]
		if t in covered:continue
		out.append((key,[r[2] for r in rows]));used.update(r[2] for r in rows);covered.add(t)
	by_task={t:[]for t in B}
	for r in C:
		if len(r[1])==1 and r[1][0] in by_task:by_task[r[1][0]].append(r)
	for t in sorted(B):
		if time.monotonic()>D-.025:break
		if t in covered:continue
		rows=[r for r in by_task.get(t,[]) if r[2] not in used]
		rows=sorted(rows,key=lambda r:(_single_expected_cost(r),r[3],-r[4],r[5]))[:18]
		best=None;best_cost=float(_F)
		for i,a in enumerate(rows):
			for b in rows[i+1:]:
				cost=_group_expected_cost([a,b],1)
				if cost<best_cost:best=(a,b);best_cost=cost
		if best:
			out.append((t,[best[0][2],best[1][2]]));used.add(best[0][2]);used.add(best[1][2]);covered.add(t)
	return out if len(covered)==len(B) else result

def _low_official_like_score(result,candidates,all_tasks):
	base=_solution_expected_cost(result,candidates,all_tasks);row_map={(A[0],A[2]):A for A in candidates};pen=0.
	for key,cs in result:
		rows=[row_map.get((key,c)) for c in cs];rows=[r for r in rows if r is not _A]
		if not rows:pen+=100.;continue
		if len(rows[0][1])!=1:pen+=12.*(len(rows[0][1])-1)
		pen+=4.*abs(len(rows)-2)
	return base+.25*pen
