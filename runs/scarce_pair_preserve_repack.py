...
def _scarce_pair_preserve_repack(result,candidates,all_tasks):
	...
	# Only consider replacing two-task/single groups with one-rider rows, preserving courier uniqueness and coverage >=39.
	...
	for _,old_key,old_tasks in groups:
		...
		if 'T0033' in all_tasks:task_win.add('T0033')
		cols=[]
		for r in candidates:
		...
		for cost,ts,rs in cols:
			...
			c=_solution_expected_cost(out,candidates,all_tasks)
			if c<best_cost-1.0:best=out;best_cost=c
	return best
