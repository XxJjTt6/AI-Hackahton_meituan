...
def solve(input_text: str) -> list:
    ...
    avg_single_willingness = sum(c[4] for c in singles) / len(singles) if singles else avg_willingness
    scarce = courier_count <= task_count
    low_willingness = avg_willingness < 0.27 or avg_single_willingness < 0.35
    ...
    if singles:
        ...
        if abundant and not low_willingness and time.monotonic() < deadline - 1.9:
    ...
    if not abundant or low_willingness:
        ...
        if time.monotonic() < deadline - 0.55:
            ...
            if pair_solution:
                ...
                if (
                    low_willingness
                    and avg_single_willingness < 0.32
                    and _solution_covered_count(pair_solution, candidates) == len(all_tasks)
                    and any("," in task_key for task_key, _ in pair_solution)
                ):
                    preferred_low_solution = pair_solution
    ...
    if preferred_low_solution is not None:
        normal_best = min((s for s in solutions if s), key=lambda s: _solution_expected_cost(s, candidates, all_tasks))
        preferred_cost = _solution_expected_cost(preferred_low_solution, candidates, all_tasks)
        normal_cost = _solution_expected_cost(normal_best, candidates, all_tasks)
        if preferred_cost <= normal_cost * 1.18:
            best = preferred_low_solution
        else:
            best = normal_best
    elif scarce:
        best = max(
            ...
            key=lambda s: (
                _solution_covered_count(s, candidates),
                -_solution_expected_cost(s, candidates, all_tasks),
            ),
        )
    else:
...
def _solve_sparse_cover(candidates, all_tasks, deadline):
    ...
    should_beam = (
        len(all_tasks) <= 60
        and len(candidates) <= 45000
        and len({c[2] for c in candidates}) <= 60
        and time.monotonic() < deadline - 1.0
    )
    if should_beam:
        beam = _sparse_beam_search(candidates, all_tasks, deadline, prefer_coverage=True)
        if beam and _simple_result_score(beam, candidates, all_tasks) < _simple_result_score(best, candidates, all_tasks):
            best = beam
    return best


def _sparse_beam_search(candidates, all_tasks, deadline, prefer_coverage=False):
    ...
    beam = {0: (0.0, ())}
    beam_width = 5000 if small_sparse else (650 if len(courier_items) <= 30 else 420)
    for _, rows in courier_items:
        ...
        if len(next_beam) > beam_width:
            if prefer_coverage:
                ranked = sorted(
                    next_beam.items(),
                    key=lambda item: (_popcount(item[0]), item[1][0]),
                    reverse=True,
                )[:beam_width]
                beam = dict(ranked)
                continue
