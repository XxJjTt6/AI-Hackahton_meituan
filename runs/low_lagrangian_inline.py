...
def solve(input_text):
	...
	if F and time.monotonic()<A-.8:
		...
		if J and time.monotonic()<A-.95:
			pl=_low_lagrangian_columns(C,B,min(A,time.monotonic()+.45))
			if pl:E.append(pl)
...
def _low_lagrangian_columns(candidates,all_tasks,deadline):
	A=list(candidates);B=set(all_tasks)
	if time.monotonic()>deadline-.05 or not A:return []
	L={}
	for t in B:
		V=[_group_expected_cost([c],len(c[1]))/max(1,len(c[1])) for c in A if t in c[1]]
		L[t]=min(V) if V else 100.
	best=[];bestc=float(_F)
	for it in range(10):
		if time.monotonic()>deadline-.04:break
		R=[]
		for c in A:R.append((_group_expected_cost([c],len(c[1]))-sum(L.get(t,0) for t in c[1]),c))
		R.sort(key=lambda x:x[0]);usedc=set();usedt=set();pick=[]
		for rc,c in R:
			if c[2] in usedc or any(t in usedt for t in c[1]):continue
			pick.append(c);usedc.add(c[2]);usedt.update(c[1])
		res=[];D={}
		for c in pick:D.setdefault(c[0],[]).append(c[2])
		res=list(D.items());cost=_solution_expected_cost(res,A,B)
		if cost<bestc:bestc=cost;best=res
		step=1.5/(1+it)
		for t in B:L[t]=max(0.,L[t]+step*(1-(1 if t in usedt else 0)))
	return best
