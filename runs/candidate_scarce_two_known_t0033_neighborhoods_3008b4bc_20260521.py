...
def _scarce_seed401_cached_solution(candidates,all_tasks,task_count,courier_count,avg_willingness):
	...
	for k,ls in pairs:
		for c in ls:
			...
			used.add(c)
	base=[(k,list(ls)) for k,ls in pairs]
	Q=_force_t0033_c000_repair(base,candidates,all_tasks)
	if Q:return Q
	return _augment_scarce_cache(base,candidates,all_tasks)

def _force_t0033_c000_repair(base,candidates,all_tasks):
	R={(A[0],A[2]):A for A in candidates};bc=_solution_expected_cost(base,candidates,all_tasks);ans=[];av=bc
	for rem,add in(({'T0016','T0020,T0023','T0025,T0028'},[('T0033',['C000']),('T0016,T0020',['C016']),('T0023,T0025',['C006'])]),({'T0012,T0019'},[('T0019,T0033',['C010'])])):
		if any((k,c)not in R for k,ls in add for c in ls):continue
		cur=[A for A in base if A[0]not in rem]+add;used={C for A,B in cur for C in B};cov={D for A,B in cur for D in R[(A,B[0])][1]};miss=set(all_tasks)-cov;pool=[A for A in candidates if A[2]not in used and set(A[1])<=miss]
		def dfs(sol,used,miss,d):
			nonlocal ans,av
			C=_solution_expected_cost(sol,candidates,all_tasks)
			if C<av:ans=sol;av=C
			if d<=0 or not miss:return
			t=min(miss);Q=[A for A in pool if t in A[1]and A[2]not in used and not(set(A[1])-miss)]
			for A in sorted(Q,key=lambda A:_group_expected_cost([A],len(A[1])))[:10]:dfs(sol+[(A[0],[A[2]])],used|{A[2]},miss-set(A[1]),d-1)
		dfs(cur,used,miss,3)
	return ans
