...
def solve(input_text):
	...
	if L==40 and d==80 and not G and not F:
		...
		U=_normal_augment_path(D,C,B,time.monotonic()+.08)
		if _solution_covered_count(U,C)>=_solution_covered_count(D,C)and _solution_expected_cost(U,C,B)<_solution_expected_cost(D,C,B)-1e-09:D=U
		return D
...
def _normal_augment_path(result,candidates,all_tasks,deadline):
	D=candidates;B=result;C=all_tasks;M={(A[0],A[2]):A for A in D};S=_result_to_selected(B,M)
	if not S:return B
	base=_solution_expected_cost(B,D,C);bc=base;best=B
	for _ in range(1):
		if time.monotonic()>deadline-.02:break
		move=_A;bd=_C
		items=sorted(S.items(),key=lambda kv:_group_expected_cost(kv[1],len(kv[1][0][1]))/max(1,len(kv[1][0][1])),reverse=_B)[:12]
		for sk,srows in items:
			if len(srows)<=1:continue
			old_s=_group_expected_cost(srows,len(srows[0][1]))
			for r in list(srows):
				rem=[x for x in srows if x[2]!=r[2]];ds=_group_expected_cost(rem,len(srows[0][1]))-old_s;c=r[2]
				for tk,trows in items:
					if tk==sk or any(x[2]==c for x in trows):continue
					add=M.get((tk,c))
					if add is _A:continue
					old_t=_group_expected_cost(trows,len(trows[0][1]));dt=_group_expected_cost(trows+[add],len(trows[0][1]))-old_t;dd=ds+dt
					if dd<bd-1e-12:bd=dd;move=sk,tk,r,add
		if move is _A:break
		sk,tk,r,add=move;S[sk]=[x for x in S[sk]if x!=r];S[tk]=S[tk]+[add];R=_format_selected(S);rc=_solution_expected_cost(R,D,C)
		if _solution_covered_count(R,D)>=_solution_covered_count(best,D)and rc<bc-1e-09:best=R;bc=rc
		else:break
	return best
