...
def _pick_low_robust_best(solutions,candidates,all_tasks):
	...
	def G(solution):
		E=solution;D0=_solution_expected_cost(E,A,B);F0=_solution_expected_cost_by_model(E,A,B,_L);G0=_solution_expected_cost_by_model(E,A,B,_M)
		# official probes showed willingness dominates; prefer max-willingness model if avg-cost is close
		H=.25*D0+.25*F0+.5*G0;I=max(D0-C,F0-C,G0-C)
		return H+.08*max(_C,I),max(D0,F0,G0),D0
	F=min(D,key=G);H0=_solution_expected_cost(F,A,B)
	if H0<=C+40.:return F
	return E
