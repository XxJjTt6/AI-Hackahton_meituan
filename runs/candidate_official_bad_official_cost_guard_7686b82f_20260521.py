...
def _scarce_seed401_cached_solution(candidates,all_tasks,task_count,courier_count,avg_willingness):
	...
	B=_sparse_beam_search(candidates,all_tasks,time.monotonic()+.55,coverage_first=_B)
	if B and _osc(B,candidates,all_tasks)<_osc(base,candidates,all_tasks)-1e-9:return B
	return base

def _ogc(rows,n):
	p=_E
	for A in rows:p*=_E-A[4]
	p=_E-p
	if p<=_C:return 1e2*n
	return(_group_expected_cost(rows,n)-(_E-p)*1e2*n)/p
def _osc(result,candidates,all_tasks):
	R={(A[0],A[2]):A for A in candidates};cov=set();used=set();s=_C
	for k,ls in result:
		rows=[]
		for c in ls:
			r=R.get((k,c))
		...
		for t in rows[0][1]:
			if t in cov:return float(_F)
			cov.add(t)
		s+=_ogc(rows,len(rows[0][1]))
	return s+1e2*(len(all_tasks)-len(cov))
