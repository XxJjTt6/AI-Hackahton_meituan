...
def _solve_scarce_bundle_mcf_enum(candidates,all_tasks,deadline):
	...
	for D in _canonical_candidates(G):
		...
		C.append((T,F,A,K,D))
	if not C:return[]
	C.sort(key=lambda item:(_popcount(item[2]),item[0]/max(item[1],1e-09),item[0]+.12*sum(_task_shadow_values(G,H).get(T,0.) for T in item[4][1]),-item[1],-item[4][5]),reverse=_B);C=C[:120];O=[(0,frozenset(),())];U=list(O);L=_complete_scarce_bundles_with_mcf((),J,I);V=_solution_expected_cost(L,G,H)if L else float(_F);a=min(len({A[2]for A in G}),len(H)//2+2)
	for W in range(a):
