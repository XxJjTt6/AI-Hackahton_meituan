	...
	if F and time.monotonic()<A-.8:
		...
		if J and _LOW_BIAS_ACTIVE and time.monotonic()<A-1.05:
			...
			if Y and time.monotonic()<A-.65:
				...
				if u:E.append(u)
	if G and time.monotonic()<A-.62:
		SB=_ssbc1(C,B,min(A,time.monotonic()+.5))
		if SB:E.append(SB)
	if not O or F or AA:
...
def _augment_scarce_cache(base,candidates,all_tasks):
	...
	SB=_ssbc1(candidates,all_tasks,time.monotonic()+.45);SC=_solution_expected_cost(SB,candidates,all_tasks)
	if SB and _solution_covered_count(SB,candidates)>=base_cov and SC<bc-1e-09:best=SB;bc=SC
...
def _ssbgme(candidates,all_tasks,deadline):
	...
	return K
def _ssbc1(candidates,all_tasks,deadline):
	M=candidates;G=all_tasks;H=deadline;I=sorted(G);J={};V=[]
	for A in _canonical_candidates(M):
		if len(A[1])==1:J.setdefault(A[1][0],[]).append(A)
		elif len(A[1])==2:
			C=_group_expected_cost([A],2)
			if C<200-1e-09:V.append((C,A))
	if not V:return[]
	K=_csbgwm((),J,I);a=_solution_expected_cost(K,M,G)if K else float(_F)
	for(C,A)in sorted(V)[:12]:
