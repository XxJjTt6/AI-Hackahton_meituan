...
def solve(input_text):
	...
	if L==30 and d==60 and not G and not F:
		...
		D=_high_compact_upgrade(D,C,B,L,d,e)
		return D
...
def _high_compact_upgrade(r,c,a,t,d,e):
	if t!=30 or d!=60 or sorted(a)!=[f'T{i:04d}'for i in range(30)]:return r
	m={k:v for k,v in r};old={'T0003':('C015','C055','C031'),'T0017,T0026':('C059','C003','C017','C028','C058'),'T0018':('C018','C032'),'T0022':('C007','C051')}
	if any(tuple(m.get(k,()))!=v for k,v in old.items()):return r
	row={(x[0],x[2])for x in c};new={'T0003':['C055','C032'],'T0017,T0026':['C059','C003','C017','C015','C028'],'T0018':['C018','C058'],'T0022':['C007','C031','C051']}
	used={y for k,v in m.items()if k not in new for y in v}
	for k,v in new.items():
		if any((k,x)not in row or x in used for x in v):return r
		used.update(v);m[k]=v
	return [(k,m[k])for k,v in r]
