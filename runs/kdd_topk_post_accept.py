...
def _solve_scarce_topk_bundle_mcf(candidates,all_tasks,deadline):
	...
	C=list(Dct.values())
	C.sort(key=lambda item:(_popcount(item[2]),item[0]/max(item[1],1e-09),item[0],-item[1],-item[4][5]),reverse=_B);C=C[:80]
	best=_complete_scarce_bundles_with_mcf((),J,I);best_cost=_solution_expected_cost(best,G,H)if best else float(_F)
	states=[(0,frozenset(),())];seen=list(states);limit=min(len({A[2]for A in G}),len(H)//2+2,14)
	for step in range(limit):
		...
		for(mask,used,rows)in states:
			for(gain,cost,cmask,courier,row)in C:
				...
				if len(next_states)>=900:break
			if len(next_states)>=900 or time.monotonic()>N-.18:break
		if not next_states:break
		next_states=_prune_scarce_bundle_states(next_states,I,J,max_states=100);seen.extend(next_states);states=next_states
	visited=set()
	for(mask,used,rows)in seen:
		if time.monotonic()>N-.04:break
