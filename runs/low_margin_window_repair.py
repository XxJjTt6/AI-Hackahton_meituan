...
def solve(input_text):
	...
	if time.monotonic()<A-.75:D=_low_margin_group_repair_solution(D,C,B,min(A,time.monotonic()+.45))
...
def _normal_worst_related_repair_solution(result,candidates,all_tasks,deadline):
	...
	return C
def _low_margin_group_repair_solution(result,candidates,all_tasks,deadline):
	D=candidates;A=all_tasks;B=result;row={(r[0],r[2]):r for r in D};sel=_result_to_selected(B,row)
	if not sel:return B
	groups=[]
	for k,rows in sel.items():
		margin=100*len(rows[0][1])-_group_expected_cost(rows,len(rows[0][1]));groups.append((margin,k,set(rows[0][1])))
	best=B;best_cost=_solution_expected_cost(B,D,A)
	for _,k,tasks in sorted(groups)[:6]:
		if time.monotonic()>deadline-.08:break
		cand=_repair_task_window(sel,D,A,tasks,min(deadline,time.monotonic()+.12),top_riders_per_task_key=10,max_k=4,option_limit=60)
		if not cand:continue
		cost=_solution_expected_cost(cand,D,A)
		if cost<best_cost-1e-09:best=cand;best_cost=cost;sel=_result_to_selected(best,row)
	return best
