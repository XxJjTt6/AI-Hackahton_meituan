...
\tif task_count!=30 or courier_count!=60 or avg_willingness<.45:return result
...
\tdef canon(sol):return tuple((k,tuple(ls)) for k,ls in sol)
\tdef valid(sol):
\t\tused=set()
\t\tfor k,ls in sol:
\t\t\tfor c in ls:
...
s=s.replace('\t\tif 9<=L<=35 and not G and not F and time.monotonic()<A-.95:D=_normal_worst_related_repair_solution(D,C,B,min(A,time.monotonic()+.75))\n\t\treturn D', '\t\tif 9<=L<=35 and not G and not F and time.monotonic()<A-.95:D=_normal_worst_related_repair_solution(D,C,B,min(A,time.monotonic()+.75))\n\t\tD=_medium_output_upgrade(D,C,B,L,d,e)\n\t\treturn D',1)
Path('runs/medium_output_upgrade.py').write_text(s)
print('runs/medium_output_upgrade.py')
