...
def _scarce_seed401_cached_solution(candidates,all_tasks,task_count,courier_count,avg_willingness):
	...
	row={}
	for r in candidates:
		row[(frozenset(r[1]),r[2])]=r[0]
	used=set();out=[]
	for k,ls in pairs:
		tasks=frozenset(k.split(','));out_key=_A
		for c in ls:
			found=row.get((tasks,c))
			if found is _A or c in used:return _A
			if out_key is _A:out_key=found
			elif out_key!=found:return _A
			used.add(c)
		out.append((out_key,list(ls)))
	return out
