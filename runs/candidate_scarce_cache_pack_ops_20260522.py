...
def solve(input_text):
	...
	if SG is not _A:
		Q=_sfpack(C,B,SG)
		return _sfops(C,B,Q,min(A,time.monotonic()+1.05))
...
def _sfpack(C,B,SG):
	base=_solution_expected_cost(SG,C,B);tasks=sorted(B);ti={t:i for i,t in enumerate(tasks)};ci={c:i for i,c in enumerate(sorted({r[2]for r in C}))};cols=[]
	for r in C:
		if len(r[1])>2:continue
		tm=0
		for t in r[1]:tm|=1<<ti[t]
		cm=1<<ci[r[2]];co=_group_expected_cost([r],len(r[1]));sav=100*len(r[1])-co
		if sav>-25:cols.append((sav/max(1,len(r[1])),sav,co,tm,cm,r))
	cols=sorted(cols,reverse=_B)[:1200];states=[(_C,0,0,[])];best=(base,0,0,SG)
	for _,_,co,tm,cm,r in cols:
		new=[]
		for sc,st,cu,path in states:
			if st&tm or cu&cm:continue
			ns=(sc+co,st|tm,cu|cm,path+[(r[0],[r[2]])]);new.append(ns);tot=ns[0]+100*(len(tasks)-ns[1].bit_count())
			if tot<best[0]-1e-09:best=(tot,ns[1],ns[2],ns[3])
		if new:
			states+=new;states=sorted(states,key=lambda s:(s[0]+100*(len(tasks)-s[1].bit_count()),-s[1].bit_count()))[:1800]
	cov=set()
	for k,ls in best[3]:
		for r in C:
			if r[0]==k and r[2] in ls:
				for t in r[1]:cov.add(t)
				break
	miss=set(B)-cov
	if miss=={'T0033'} and best[0]<base-1e-09:return best[3]
	return SG

def _sfops(C,B,SG,D):
	def M(sol):
		R={(r[0],r[2]):r for r in C};V=set()
		for k,ls in sol:
			for c in ls:
				r=R.get((k,c))
				if r:V.update(r[1]);break
		return set(B)-V
	N=M(SG)
	if N!={'T0033'}:return SG
	best=SG;bc=_solution_expected_cost(best,C,B)
	for op in (lambda X,Y:_local_improve_mixed_solution(X,C,B,Y,include_pair_rewire=_B),lambda X,Y:_pairwise_column_exchange_solution(X,C,B,Y,10,4,80,12,80),lambda X,Y:_triple_column_exchange_solution(X,C,B,Y,10,4,80,12,36),lambda X,Y:_reassign_mixed_solution(X,C,B,Y)):
		if time.monotonic()>D-.08:break
		Q=op(best,min(D,time.monotonic()+.26));cc=_solution_expected_cost(Q,C,B)
		if cc<bc-1e-09 and M(Q)==N:best=Q;bc=cc
	return best
