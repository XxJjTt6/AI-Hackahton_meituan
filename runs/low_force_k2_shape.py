...
def solve(input_text):
	...
	if J and time.monotonic()<A-.28:D=_low_force_k2_if_close(D,C,B)
...
def _low_force_k2_if_close(result,candidates,all_tasks):
	A=result;B=candidates;C=all_tasks;D={(A[0],A[2]):A for A in B if len(A[1])==1};E=_result_to_selected(A,D)
	if not E or any(len(F[0][1])!=1 for F in E.values() if F):return A
	G=set();H={}
	for K in sorted(C):
		R=[A for A in D.values() if A[1][0]==K and A[2] not in G]
		if len(R)<2:return A
		I=[]
		for L in range(min(18,len(R))):
			for M in range(L+1,min(18,len(R))):
				N=[R[L],R[M]];I.append((_group_expected_cost(N,1),N))
		if not I:return A
		I.sort(key=lambda x:x[0])
		Q=None
		for P,N in I[:25]:
			if N[0][2] not in G and N[1][2] not in G:Q=N;break
		if Q is None:return A
		H[K]=Q;G.add(Q[0][2]);G.add(Q[1][2])
	J=_format_selected(H)
	return J if _solution_expected_cost(J,B,C)<=_solution_expected_cost(A,B,C)+3.0 else A
