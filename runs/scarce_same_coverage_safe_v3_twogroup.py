...
def solve(input_text):
	...
	if SG is not _A:
		RG=_scarce_same_coverage_safe_v3(SG,C,B)
...
def _scarce_same_coverage_safe_v3(result,candidates,all_tasks):
	true_tasks={f'T{i:04d}'for i in range(40)};row={(r[0],r[2]):r for r in candidates};sel=_result_to_selected(result,row)
	if not sel:return _A
	base_cov={t for rows in sel.values() for t in rows[0][1]};base_cost=_solution_expected_cost(result,candidates,true_tasks);best=_A;best_cost=base_cost
	items=sorted([(_group_expected_cost(rows,len(rows[0][1]))-100*len(rows[0][1]),k,set(rows[0][1])) for k,rows in sel.items()],reverse=_B)[:20]
	windows=[(k,{k},tasks) for _,k,tasks in items]
	for i in range(len(items)):
		for j in range(i+1,len(items)):
			windows.append((items[i][1]+'|'+items[j][1],{items[i][1],items[j][1]},items[i][2]|items[j][2]))
	for _,remove_keys,old_tasks in windows[:80]:
		fixed={k:list(v) for k,v in sel.items() if k not in remove_keys};fixed_t={t for rows in fixed.values() for t in rows[0][1]};fixed_c={r[2] for rows in fixed.values() for r in rows}
		cols=[]
		for r in candidates:
			if r[2] in fixed_c:continue
			ts=set(r[1])
			if not ts or not ts<=old_tasks or ts&fixed_t:continue
			if len(ts)>2:continue
			cost=_group_expected_cost([r],len(ts))
			if cost<100*len(ts)+25:cols.append((cost,ts,{r[2]},[r]))
		cols=sorted(cols,key=lambda x:(x[0]-100*len(x[1]),x[0]))[:55]
		states={(frozenset(),frozenset()):(0,[])}
		for cost,ts,cs,rs in cols:
			new=dict(states)
			for (mask,used),(val,path) in states.items():
				if mask&ts or used&cs:continue
				nm=frozenset(set(mask)|ts);nu=frozenset(set(used)|cs);nv=val+cost;old=new.get((nm,nu))
				if old is _A or nv<old[0]-1e-09:new[(nm,nu)]=(nv,path+[rs])
			states=new
		for (mask,used),(val,path) in states.items():
			if mask!=frozenset(old_tasks):continue
			new={k:list(v) for k,v in fixed.items()}
			for rs in path:new[rs[0][0]]=rs
			out=_format_selected(new);selected=_result_to_selected(out,row)
			if not selected:continue
			cov={t for rows in selected.values() for t in rows[0][1]};used2=[r[2] for rows in selected.values() for r in rows]
			if cov!=base_cov or len(used2)!=len(set(used2)):continue
