...
def _solve_low_column_search(singles,all_tasks,deadline):
	A=singles
	if not A:return[]
	return _search_column_window(A,all_tasks,deadline,top_riders_per_task_key=16,max_k=4,option_limit=55)
def _solve_low_global_column_search(candidates,all_tasks,deadline):
	A=candidates
	if not A:return[]
	return _search_column_window(A,all_tasks,deadline,top_riders_per_task_key=14,max_k=4,option_limit=50)
...
def _low_worst_window_repair_solution(result,candidates,all_tasks,deadline):
	...
	for(N,O)in((8,range(0,8)),(10,range(0,8)),(12,range(0,8))):
		for P in O:
			...
			K=tuple(sorted(D))
			if K in J:continue
			J.add(K);Q=_result_to_selected(A,G);L=_repair_task_window(Q,C,E,D,min(F,time.monotonic()+.22),top_riders_per_task_key=16,max_k=5,option_limit=130)
			if not L:continue
...
def _low_deep_window_repair_solution(result,candidates,all_tasks,deadline):
	...
	for N in(8,10,12):
		for O in range(10):
			...
			K=tuple(sorted(D))
			if K in J:continue
			J.add(K);P=_result_to_selected(A,G);L=_repair_task_window(P,C,E,D,min(F,time.monotonic()+.2),top_riders_per_task_key=16,max_k=5,option_limit=140)
			if not L:continue
			A=_pick_low_robust_best([A,L],C,E)
	return A
def _low_late_acceptance_repair_solution(result,candidates,all_tasks,deadline):
	...
	while time.monotonic()<P-.12:
		...
		L=_repair_task_window(J,D,I,C,min(P,time.monotonic()+.12),top_riders_per_task_key=16,max_k=5,option_limit=140)
