...
def solve(input_text):
	...
	if G and time.monotonic()<A-.3:
		...
		if time.monotonic()<A-.28:
			A2=_scarce_one_swap_uncovered_repair(D,C,B,min(A,time.monotonic()+.22))
			if _solution_covered_count(A2,C)>=_solution_covered_count(D,C)and _solution_expected_cost(A2,C,B)<_solution_expected_cost(D,C,B)-1e-09:D=_drop_unprofitable_groups(A2,C,B)
...
def _triple_column_exchange_solution(result,candidates,all_tasks,deadline,top_riders_per_task_key,max_k,option_limit,max_window_tasks,max_triples):
	...
	return J
def _scarce_one_swap_uncovered_repair(result,candidates,all_tasks,deadline):
	M=deadline;L=all_tasks;H=candidates;G=result;R={(A[0],A[2]):A for A in H};B=_result_to_selected(G,R)
	if not B:return G
	by_task={};by_key={};by_courier={}
	for A in H:
		by_key.setdefault(A[0],[]).append(A);by_courier.setdefault(A[2],[]).append(A)
		for T in A[1]:by_task.setdefault(T,[]).append(A)
	base=_solution_expected_cost(G,H,L);base_cov=_solution_covered_count(G,H);best=G;best_cost=base
	while time.monotonic()<M-.04:
		covered={T for rows in B.values() for T in rows[0][1]};missing=sorted(set(L)-covered)
		if not missing:break
		used={A[2] for rows in B.values() for A in rows};improved=_D
		for u in missing:
			if time.monotonic()>M-.04:break
			for cand in sorted(by_task.get(u,()),key=lambda c:(_group_expected_cost([c],len(c[1])),-len(c[1]),c[5]))[:80]:
				cid=cand[2]
				if cid not in used:continue
				old_key=_A;old_rows=_A
				for K,V in B.items():
					if any(A[2]==cid for A in V):old_key=K;old_rows=V;break
				if old_key is _A:continue
				new_tasks=set(cand[1]);old_tasks=set(old_rows[0][1]);lost=old_tasks-new_tasks
				if not lost:continue
				S={K:list(V) for K,V in B.items() if K!=old_key};S[cand[0]]=[cand]
				used2={A[2] for rows in S.values() for A in rows};covered2={T for rows in S.values() for T in rows[0][1]}
				# Repair lost tasks with currently unused single/bundle rows, preserving disjoint coverage.
				for t in sorted(lost):
					if t in covered2:continue
					opts=[]
					for row in by_task.get(t,()):
						if row[2] in used2:continue
						if any(x in covered2 for x in row[1] if x!=t):continue
						opts.append(row)
					if not opts:continue
					row=min(opts,key=lambda c:(_group_expected_cost([c],len(c[1])),-c[4],c[5]));S[row[0]]=[row];used2.add(row[2]);covered2.update(row[1])
				U=_format_selected(S);cov=_solution_covered_count(U,H)
				if cov<base_cov:continue
				cost=_solution_expected_cost(U,H,L)
				if cost<best_cost-1e-09:best=U;best_cost=cost;B=S;improved=_B;break
			if improved:break
...
def _scarce_polish_candidate(result,candidates,all_tasks,deadline):
	...
	if time.monotonic()<D-.28:
		J=_scarce_one_swap_uncovered_repair(A,B,C,min(D,time.monotonic()+.22))
		if _solution_covered_count(J,B)>=_solution_covered_count(A,B)and _solution_expected_cost(J,B,C)<_solution_expected_cost(A,B,C)-1e-09:A=_drop_unprofitable_groups(J,B,C)
