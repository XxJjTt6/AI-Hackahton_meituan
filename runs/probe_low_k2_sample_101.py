...
def solve(input_text):
	...
	KP=_low_k2_sample_probe(C,B,L,d,e)
...
def _low_k2_sample_probe(candidates,all_tasks,task_count,courier_count,avg_willingness):
	...
	used=set();out=[];rng=random.Random(101)
	order=sorted(all_tasks)
	# deterministic task rotation for diversity
	shift=101%30; order=order[shift:]+order[:shift]
	for pos,t in enumerate(order):
		...
		pick=[]; start=(101+pos*7)%max(1,min(9,len(opts)))
