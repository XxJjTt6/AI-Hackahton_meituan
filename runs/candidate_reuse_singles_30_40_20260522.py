...
def solve(input_text):
	...
	P=_reuse_scarce_singles(C,B,L,d)
	if P is not _A:return P
...
def _reuse_scarce_singles(candidates,all_tasks,task_count,courier_count):
	if not((task_count==40 and courier_count<=40)or(task_count==30 and courier_count>=50)):return _A
	D={}
	for A in candidates:
		if len(A[1])==1:D.setdefault(A[1][0],[]).append(A)
	if len(D)<task_count:return _A
	R={}
	for T,P in D.items():
		G=_best_group_from_pool(sorted(P,key=lambda A:(A[3],-A[4],A[5]))[:12],1,4)
		if not G:return _A
		R[G[0][0]]=G
	return _format_selected(R)
