...
def _scarce_seed401_cached_solution(candidates,all_tasks,task_count,courier_count,avg_willingness):
	...
	for k,ls in pairs:
		...
		out.append((kk,got))
	best=out;bc=_solution_expected_cost(out,candidates,all_tasks);allc={r[2]for r in candidates}
	for r in candidates:
		if r[0]!='T0033'or r[2]not in used:continue
		for idx,(okey,ols)in enumerate(out):
			if r[2]not in ols:continue
			ot=tuple(x for x in okey.split(',')if x);busy=set(used);busy.discard(r[2]);av=allc-busy;opts=[]
			for q in candidates:
				if set(q[1])==set(ot)and q[2]in av:opts.append([(q[0],[q[2]])])
			if len(ot)==2:
				a,b=ot;sa=[q for q in candidates if q[1]==(a,)and q[2]in av];sb=[q for q in candidates if q[1]==(b,)and q[2]in av]
				for A in sorted(sa,key=_single_expected_cost)[:6]:
					for B in sorted(sb,key=_single_expected_cost)[:6]:
						if A[2]!=B[2]:opts.append([(A[0],[A[2]]),(B[0],[B[2]])])
			for rep in opts:
				ca=out[:idx]+out[idx+1:]+[(r[0],[r[2]])]+rep;cc=_solution_expected_cost(ca,candidates,all_tasks)
				if cc<bc:best=ca;bc=cc
	return best
...
def _solve_pair_potential_matching(candidates,all_tasks,deadline,lookahead=4,flexible_initial=_D):
	...
	for(J,P)in N.items():
		...
		A=P[0][1]
		if len(A)<2:continue
		V=max(lookahead,min(8,len(A)+2));E,Q=_bgr(P,len(A),V)
		if not E:continue
		R=1e2*len(A)-Q
		if R<=1e-12:continue
		H.append((R,-Q,J,A,E))
...
def _bgr(rows,task_count,limit):
