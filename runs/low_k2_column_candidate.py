...
def solve(input_text):
	...
	if F and time.monotonic()<A-.8:
		...
		if J and H and time.monotonic()<A-1.05:
			p=_solve_low_k2_only_column_search(H,B,min(A,time.monotonic()+.55))
			if p:E.append(p)
...
def _solve_low_k2_only_column_search(singles,all_tasks,deadline):
	A=singles
	if not A:return[]
	return _search_column_window(A,all_tasks,deadline,top_riders_per_task_key=16,max_k=2,option_limit=80)
