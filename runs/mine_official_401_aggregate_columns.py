...
items.sort(key=lambda x:x[0],reverse=True);n=len(items)
conf=[0]*n
for i in range(n):
...
dfs((1<<n)-1,0,[])
print('aggregate cols',len(cols),'items',n,'nodes',nodes,'best',round(4000-best_s,4),'covered',sum(len(items[i][3]) for i in best),'cols',len(best))
