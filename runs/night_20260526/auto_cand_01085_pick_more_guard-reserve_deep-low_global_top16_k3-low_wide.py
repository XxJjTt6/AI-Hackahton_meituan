...
def solve(input_text):
	...
	if J and time.monotonic()<A-1.65:D=_low_deep_window_repair_solution(D,C,B,min(A,time.monotonic()+1.5))
...
def _solve_low_global_column_search(candidates,all_tasks,deadline):
	A=candidates
	if not A:return[]
	return _search_column_window(A,all_tasks,deadline,top_riders_per_task_key=16,max_k=3,option_limit=80)
