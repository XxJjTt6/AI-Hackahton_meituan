...
def solve(input_text):
	...
	if G and time.monotonic()<A-.55:
		AR=_elite_route_pool_pack_repair(C,B,E,min(A,time.monotonic()+.45))
		if AR:E.append(AR)
	if f:D=_pick_hard_scarce_best(E,C,B);D=_drop_unprofitable_groups(D,C,B)
	elif G:D=_pick_scarce_coverage_guard_best(E,C,B);D=_drop_unprofitable_groups(D,C,B)
...
def _elite_route_pool_pack_repair(candidates,all_tasks,seed_solutions,deadline):
	P=deadline;O=all_tasks;G=candidates;Q=sorted(O)
	if not Q:return[]
	a={B:A for(A,B)in enumerate(Q)};b={B:A for(A,B)in enumerate(sorted({A[2]for A in G}))};R={(A[0],A[2]):A for A in G};I={}
	def S(rows,rank):
		if not rows:return
		K=rows[0][0];T=rows[0][1]
		if any(A[0]!=K or A[1]!=T for A in rows):return
		if len({A[2]for A in rows})!=len(rows):return
		M=0
		for C in T:
			D=a.get(C)
			if D is _A:return
		...
		for C in rows:
			...
			if N&E:return
			N|=E
		L=len(T);D=_group_expected_cost(rows,L);F=1e2*L-D
		if F<=1e-09:return
		U=M,N;V=F,D,M,N,tuple(rows),rank;W=I.get(U)
		if W is _A or D<W[1]-1e-09 or abs(D-W[1])<=1e-09 and rank>W[5]:I[U]=V
	J=[]
	for D in seed_solutions:
		if not D:continue
		E=_solution_expected_cost(D,G,O)
	...
	for H,(E,D) in enumerate(J[:10]):
		Y=_result_to_selected(D,R)
		for A in Y.values():S(tuple(A),10-H)
	U={}
	for A in _canonical_candidates(G):U.setdefault(A[0],[]).append(A)
	for A in U.values():
		if time.monotonic()>P-.12:break
		A=sorted(A,key=lambda c:(_group_expected_cost([c],len(c[1])),-c[4],c[5]))[:7]
		if not A:continue
		H=len(A[0][1]);K=2 if H==1 else min(4,len(A))
		for L in range(1,K+1):
			for D in itertools.combinations(A,L):
				if len({C[2] for C in D})!=len(D):continue
				S(tuple(D),0)
	D=list(I.values())
	if not D:return[]
	D=_scarce_prune_elite_columns(D,900)
	E=_scarce_beam_pack_columns(D,min(P,time.monotonic()+.18),beam_width=90)
	if not E:return[]
	Z=[]
	for K in E:
		A=D[K];B=sorted(A[4],key=lambda c:(c[3],-c[4],c[5]));Z.append((B[0][0],[C[2]for C in B]))
	if time.monotonic()<P-.18:Z=_column_alns_repair_solution(Z,G,O,min(P,time.monotonic()+.16),mode=_J,max_window_tasks=12,top_riders_per_task_key=8,option_limit=60,max_k=4)
	if time.monotonic()<P-.08:Z=_reassign_mixed_solution(Z,G,O,min(P,time.monotonic()+.06))
	return Z
...
def _pick_scarce_coverage_guard_best(solutions,candidates,all_tasks):
	D=[A for A in solutions if A]
	if not D:return[]
	E=max(_solution_covered_count(A,candidates) for A in D)
	F=[A for A in D if _solution_covered_count(A,candidates)>=E]
	return min(F,key=lambda s:_solution_expected_cost(s,candidates,all_tasks))
