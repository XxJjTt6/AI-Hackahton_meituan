...
def _scarce_seed401_cached_solution(candidates,all_tasks,task_count,courier_count,avg_willingness):
	...
	P=[r for r in candidates if 'T0033'in r[1]and r[2]not in{'C000','C010'}]
	if P:
		Q=_probe_t0033_strict(base,candidates,all_tasks,P)
		if Q and any('T0033'in k.split(',')for k,ls in Q)and _solution_expected_cost(Q,candidates,all_tasks)<float(_F):return Q
	return _augment_scarce_cache(base,candidates,all_tasks)

def _probe_t0033_strict(base,candidates,all_tasks,P):
	R={(A[0],A[2]):A for A in candidates};best=[];bc=float(_F)
	for t in sorted(P,key=lambda A:(_group_expected_cost([A],len(A[1])),len(A[1]),A[2]))[:6]:
		cur=[(t[0],[t[2]])];used={t[2]};cov=set(t[1])
		for k,ls in base:
			r=R.get((k,ls[0]))
			if r and r[2]not in used and not(set(r[1])&cov):cur.append((k,list(ls)));used.add(r[2]);cov.update(r[1])
		miss=set(all_tasks)-cov
		for m in sorted(miss):
			Q=[A for A in candidates if m in A[1]and A[2]not in used and not(set(A[1])-miss)]
			if not Q:break
			r=min(Q,key=lambda A:_group_expected_cost([A],len(A[1])));cur.append((r[0],[r[2]]));used.add(r[2]);miss-=set(r[1])
		C=_solution_expected_cost(cur,candidates,all_tasks)
		if len(all_tasks)-len(miss)>=39 and C<bc:best=cur;bc=C
	return best
