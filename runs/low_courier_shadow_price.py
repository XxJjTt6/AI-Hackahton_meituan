...
def solve(input_text):
	...
	if F and time.monotonic()<A-.8:
		if J and time.monotonic()<A-1.2:
			...
			if p:E.append(p)
		if J and time.monotonic()<A-.75:
			SP=_shadow_adjust_scores(C,.18);V.append(SP);SW=[A for A in SP if len(A[1])==1]
			for P in(_G,_H,_I):
				if time.monotonic()<A-.35:E.append(_solve_disjoint_then_multidispatch(SP,B,mode=P,deadline=A))
...
def _courier_shadow_prices(candidates):
	D={}
	for A in candidates:
		if len(A[1])!=1:continue
		v=max(_C,1e2-_single_expected_cost(A))
		old=D.get(A[2],_C)
		if v>old:D[A[2]]=v
	return D

def _shadow_adjust_scores(candidates,alpha):
	P=_courier_shadow_prices(candidates)
	return[(A,B,C,D+alpha*P.get(C,_C),E,F)for(A,B,C,D,E,F)in candidates]
