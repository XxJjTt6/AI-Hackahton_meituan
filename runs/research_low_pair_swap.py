...
def solve(input_text):
	...
	if J and time.monotonic()<A-.35:D=_low_pair_swap_improve(D,C,B,min(A,time.monotonic()+.28))
...
def _low_pair_swap_improve(result,candidates,all_tasks,deadline):
	M=deadline;H=candidates;L=all_tasks;R={(A[0],A[2]):A for A in H};B=_result_to_selected(result,R)
	if not B:return result
	by_task={}
	for A in H:
		if len(A[1])==1:by_task.setdefault(A[1][0],[]).append(A)
	# Only touch the official-like low shape: one group per task, mostly 2 couriers.
	if len(B)<len(L)-1:return result
	cur=_solution_expected_cost(result,H,L);changed=_B
	while changed and time.monotonic()<M-.04:
		changed=_D;best=_A;best_delta=_C;keys=list(B.keys())
		used={A[2] for rows in B.values() for A in rows}
		for i,K1 in enumerate(keys):
			if time.monotonic()>M-.04:break
			A1=B.get(K1)
			if not A1 or len(A1[0][1])!=1:continue
			for K2 in keys[i+1:]:
				A2=B.get(K2)
				if not A2 or len(A2[0][1])!=1:continue
				old=_group_expected_cost(A1,1)+_group_expected_cost(A2,1);t1=A1[0][1][0];t2=A2[0][1][0]
				for c1 in A1:
					for c2 in A2:
						if c1[2]==c2[2]:continue
						# Swap couriers between two singleton tasks if both rows exist.
						n1=[A for A in by_task.get(t1,()) if A[2]==c2[2]];n2=[A for A in by_task.get(t2,()) if A[2]==c1[2]]
						if not n1 or not n2:continue
						B1=[n1[0] if A is c1 else A for A in A1];B2=[n2[0] if A is c2 else A for A in A2]
						if len({A[2] for A in B1+B2})!=len(B1)+len(B2):continue
						new=_group_expected_cost(B1,1)+_group_expected_cost(B2,1);delta=new-old
						if delta<best_delta-1e-09:best_delta=delta;best=K1,B1,K2,B2
		if best is not _A:
			K1,B1,K2,B2=best;B[K1]=B1;B[K2]=B2;cur+=best_delta;changed=_B
