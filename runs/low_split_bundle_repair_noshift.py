...
def solve(input_text):
	...
	if J and time.monotonic()<A-.55:D=_pick_low_robust_best([D,_low_split_bundle_repair_solution(D,C,B,min(A,time.monotonic()+.42))],C,B)
	if _D and J and time.monotonic()<A-.32:D=_shift_couriers_between_groups(D,C,B,min(A,time.monotonic()+.24),max_moves=30)
...
def _low_split_bundle_repair_solution(result,candidates,all_tasks,deadline):
	H={(A[0],A[2]):A for A in candidates};singles={}
	for A in candidates:
		if len(A[1])==1:singles.setdefault(A[0],[]).append(A)
	best=result;bestc=_solution_expected_cost(result,candidates,all_tasks)
	for idx,(k,cs) in enumerate(result):
		if time.monotonic()>deadline-.05:break
		ts=k.split(',')
		if len(ts)!=2:continue
		fixed=result[:idx]+result[idx+1:];used={c for kk,cc in fixed for c in cc};opts=[];ok=_B
		for t in ts:
			pool=sorted([A for A in singles.get(t,[]) if A[2] not in used],key=lambda r:_single_expected_cost(r))[:12];o=[]
			for m in(1,2,3):
				for comb in itertools.combinations(pool,m):
					if len({A[2] for A in comb})<m:continue
					o.append((_group_expected_cost(comb,1),t,tuple(A[2] for A in sorted(comb,key=lambda r:(r[3],-r[4],r[5])))))
			if not o:ok=_D;break
			opts.append(sorted(o)[:8])
		if not ok:continue
		for a in opts[0]:
			for b in opts[1]:
				if set(a[2])&set(b[2]):continue
				cand=fixed+[(a[1],list(a[2])),(b[1],list(b[2]))];c=_solution_expected_cost(cand,candidates,all_tasks)
				if c<bestc-1e-09:bestc=c;best=cand
	return best
