	...
	if not O or F or AA:
		...
		if G and time.monotonic()<A-_E:
			...
			if f and time.monotonic()<A-1.05:
				w2=_solve_scarce_topk_seedpair_mcf(C,B,min(A,time.monotonic()+.24))
				if w2:E.append(w2)
			if G and time.monotonic()<A-_E:
...
def _solve_scarce_topk_seedpair_mcf(candidates,all_tasks,deadline):
	...
	seeds=[()]
	for item in items[:16]:seeds.append((item[4],))
	for idx,a in enumerate(items[:10]):
		for b in items[idx+1:16]:
