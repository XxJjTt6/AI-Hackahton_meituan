...
def solve(input_text):
	...
	if J and time.monotonic()<A-.95:
		L2=_solve_low_exact_k2_matching(C,B,min(A,time.monotonic()+.75))
		if L2 and _pick_low_robust_best([D,L2],C,B) is L2:D=L2
...
def _solve_low_exact_k2_matching(candidates,all_tasks,deadline):
	# ETA/RL-style action filter for low willingness: official low baseline uses exactly
	# two couriers per task. Keep top filtered pair choices per task, then solve exact
	# disjoint task-pair assignment by branch-and-bound.
	D=deadline;B=sorted(all_tasks);A=candidates;singles={t:[]for t in B}
	for c in A:
		if len(c[1])==1 and c[1][0] in singles:singles[c[1][0]].append(c)
	if any(not singles[t] for t in B):return[]
	opts=[]
	for t in B:
		rows=sorted(singles[t],key=lambda c:(_single_expected_cost(c),c[3],-c[4],c[5]))[:14]
		pairs=[]
		for i,x in enumerate(rows):
			for y in rows[i+1:]:
				if x[2]==y[2]:continue
				cost=_group_expected_cost([x,y],1)
				pairs.append((cost,(x,y),frozenset((x[2],y[2]))))
		pairs.sort(key=lambda z:(z[0],z[1][0][5],z[1][1][5]))
		if not pairs:return[]
		opts.append((t,pairs[:22]))
	opts.sort(key=lambda x:(len(x[1]),x[0]))
	best=[];best_cost=float(_F);cur=[];used=set()
	mins=[min(p[0] for p in pairs) for t,pairs in opts]
	suf=[0]*(len(opts)+1)
	for i in range(len(opts)-1,-1,-1):suf[i]=suf[i+1]+mins[i]
	def dfs(idx,cost):
		nonlocal best,best_cost
		if time.monotonic()>D-.035:return
		if cost+suf[idx]>=best_cost-1e-09:return
		if idx==len(opts):
			best_cost=cost;best=list(cur);return
		t,pairs=opts[idx]
		for pcost,rows,cs in pairs:
			if used&cs:continue
			if cost+pcost+suf[idx+1]>=best_cost-1e-09:continue
			used.update(cs);cur.append((t,[rows[0][2],rows[1][2]]));dfs(idx+1,cost+pcost);cur.pop();used.difference_update(cs)
	dfs(0,_C)
	return sorted(best)
