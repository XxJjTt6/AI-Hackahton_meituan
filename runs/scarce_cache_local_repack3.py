...
def solve(input_text):
	...
	if SG is not _A:
		RG=_scarce_local_repack3_probe(SG,C,B)
		return RG if RG is not _A else SG
...
def _scarce_local_repack3_probe(result,candidates,all_tasks):
	...
	groups=[]
	for k,rows in sel.items():groups.append((_group_expected_cost(rows,len(rows[0][1]))-100*len(rows[0][1]),k,set(rows[0][1])))
	groups=sorted(groups,reverse=_B)[:8]
	remove_sets=[]
	for i in range(len(groups)):remove_sets.append({groups[i][1]})
	for i in range(len(groups)):
		for j in range(i+1,len(groups)):remove_sets.append({groups[i][1],groups[j][1]})
	for i in range(min(6,len(groups))):
		for j in range(i+1,min(7,len(groups))):
			for k in range(j+1,min(8,len(groups))):remove_sets.append({groups[i][1],groups[j][1],groups[k][1]})
	for rem in remove_sets[:80]:
		fixed={k:list(v) for k,v in sel.items() if k not in rem};fixed_tasks={t for rows in fixed.values() for t in rows[0][1]};fixed_cour={r[2] for rows in fixed.values() for r in rows};task_win={t for k in rem for t in sel[k][0][1]}
		# optionally include T0033 only if it can improve; otherwise repack same covered tasks.
		if 'T0033' in all_tasks:task_win2=task_win|{'T0033'}
		else:task_win2=task_win
		cols=[]
		for r in candidates:
			if r[2] in fixed_cour:continue
			ts=set(r[1])
			if not ts<=task_win2 or ts&fixed_tasks:continue
			cost=_group_expected_cost([r],len(r[1]));red=cost-100*len(ts)
			if red<40:cols.append((cost,ts,{r[2]},[r]))
		bykey={}
		for cost,ts,cs,rs in cols:bykey.setdefault(rs[0][0],[]).append(rs[0])
		for rs in bykey.values():
			rs=sorted(rs,key=lambda r:_group_expected_cost([r],len(r[1])))[:7]
			for a in range(len(rs)):
				for b in range(a+1,len(rs)):
					if rs[a][2]==rs[b][2]:continue
					cost=_group_expected_cost([rs[a],rs[b]],len(rs[a][1]));red=cost-100*len(rs[a][1])
					if red<40:cols.append((cost,set(rs[a][1]),{rs[a][2],rs[b][2]},[rs[a],rs[b]]))
		cols=sorted(cols,key=lambda x:(x[0]-100*len(x[1]),x[0]))[:120]
		if not cols:continue
		tl=sorted(task_win2);ti={t:i for i,t in enumerate(tl)};cl=sorted({c for _,_,cs,_ in cols for c in cs});ci={c:i for i,c in enumerate(cl)};states={(0,0):(0,[])}
		enc=[]
		...
		for (tm,um),(val,path) in states.items():
			...
			out=_format_selected(new);covered={t for k,cs in out for t in k.split(',')}
			if len(covered)<39:continue
			cost=_solution_expected_cost(out,candidates,all_tasks)
			if cost<best_cost-1.0:best=out;best_cost=cost
	return best
