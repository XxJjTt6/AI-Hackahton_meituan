...
def solve(input_text):
	...
	if L==30 and d==60 and not G and not F:
		...
		if time.monotonic()<A-.2:D=_exact_group_descent(D,C,min(A,time.monotonic()+.6))
		return D
...
def _exact_grp(rows,k):
	if not rows:return 1e2*k
	pr=_E;num=_C;den=_C
	for s,w in rows:pr*=_E-w;num+=w*s;den+=w
	p=_E-pr;return(num/den if den>_C else _C)*p+(_E-p)*1e2*k
def _exact_group_descent(result,candidates,deadline):
	R={};P={}
	for A in candidates:R[A[0],A[2]]=(A[3],A[4]);P.setdefault(A[0],[]).append(A[2])
	sol={A:list(B)for(A,B)in result};U=set()
	for B in sol.values():U.update(B)
	def gc(t,cs):return _exact_grp([R[t,c]for c in cs if(t,c)in R],t.count(',')+1)
	I=_B
	while I and time.monotonic()<deadline:
		I=_D
		for t in list(sol):
			cur=sol[t];G0=gc(t,cur);pl=[c for c in P.get(t,())if c not in U or c in cur];bd=-.1;bn=_A
			if len(cur)>1:
				for c in cur:
					nw=[x for x in cur if x!=c];d=gc(t,nw)-G0
					if d<bd:bd=d;bn=nw
			for c in pl:
				if c in cur:continue
				nw=cur+[c];d=gc(t,nw)-G0
				if d<bd:bd=d;bn=nw
			for ci in cur:
				for c in pl:
					if c in cur:continue
					nw=[x for x in cur if x!=ci]+[c];d=gc(t,nw)-G0
					if d<bd:bd=d;bn=nw
			if bn is not _A:
				for c in cur:U.discard(c)
				U.update(bn);sol[t]=bn;I=_B
			if time.monotonic()>=deadline:break
	return[(A,B)for(A,B)in sol.items()]
