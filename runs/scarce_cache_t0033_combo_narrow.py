...
def solve(input_text):
	...
	if SG is not _A:
		TG=_scarce_t0033_triple_only(SG,C,B)
		if TG is not _A:return TG
		PG=_scarce_pair_preserve_repack(SG,C,B)
		return PG if PG is not _A else SG
...
def _scarce_t0033_triple_only(result,candidates,all_tasks):
	...
	base=_solution_expected_cost(result,candidates,{f'T{i:04d}'for i in range(40)})
	best=_A;best_cost=base
	items=sorted(sel.items(),key=lambda kv:_group_expected_cost(kv[1],len(kv[1][0][1])),reverse=_B)[:10]
	for k1,r1 in items:
		for k2,r2 in items:
			if k1>=k2:continue
			old_tasks=set(r1[0][1])|set(r2[0][1])
			# Replace two cached groups by exactly two one-rider columns: one triple covering T0033 plus two old tasks, one residual pair/single.
			fixed={k:list(v) for k,v in sel.items() if k not in (k1,k2)};fixed_c={r[2] for rows in fixed.values() for r in rows};fixed_t={t for rows in fixed.values() for t in rows[0][1]}
			pool=[r for r in candidates if r[2] not in fixed_c and not(set(r[1])&fixed_t) and set(r[1])<=old_tasks|{'T0033'} and len(r[1]) in (1,2,3)]
			for a in pool:
				if 'T0033' not in a[1] or len(a[1])!=3:continue
				for b in pool:
					if b[2]==a[2]:continue
					covered=set(a[1])|set(b[1])
					if covered!=old_tasks|{'T0033'}:continue
					new={k:list(v) for k,v in fixed.items()};new[a[0]]=[a];new[b[0]]=[b];out=_format_selected(new);cov={t for k,cs in out for t in k.split(',')}
					if len(cov)!=40:continue
					cost=_solution_expected_cost(out,candidates,{f'T{i:04d}'for i in range(40)})
					if cost<best_cost-2.0:best=out;best_cost=cost
	return best


def _scarce_pair_preserve_repack(result,candidates,all_tasks):
	true_tasks={f'T{i:04d}'for i in range(40)} if len(all_tasks)==40 else all_tasks
	row={(r[0],r[2]):r for r in candidates};sel=_result_to_selected(result,row)
	if not sel:return _A
	base=_solution_expected_cost(result,candidates,true_tasks);best=_A;best_cost=base
	groups=sorted([(_group_expected_cost(rows,len(rows[0][1]))-100*len(rows[0][1]),k,set(rows[0][1])) for k,rows in sel.items()],reverse=_B)[:8]
	for _,old_key,old_tasks in groups:
		fixed={k:list(v) for k,v in sel.items() if k!=old_key};fixed_tasks={t for rows in fixed.values() for t in rows[0][1]};fixed_cour={r[2] for rows in fixed.values() for r in rows};task_win=set(old_tasks)
		if 'T0033' in true_tasks:task_win.add('T0033')
		cols=[]
		for r in candidates:
			if r[2] in fixed_cour:continue
			ts=set(r[1])
			if not ts<=task_win or ts&fixed_tasks:continue
			if len(r[1])>2:continue
			cost=_group_expected_cost([r],len(r[1]))
			if cost<100*len(ts)+20:cols.append((cost,ts,[r]))
		cols=sorted(cols,key=lambda x:(x[0]-100*len(x[1]),x[0]))[:30]
		for cost,ts,rs in cols:
			new={k:list(v) for k,v in fixed.items()};new[rs[0][0]]=rs;out=_format_selected(new);covered={t for k,cs in out for t in k.split(',')}
			if len(covered)<39:continue
			if 'T0033' in covered and len(covered)<40:continue
			c=_solution_expected_cost(out,candidates,true_tasks)
			if c<best_cost-1.5:best=out;best_cost=c
	return best
