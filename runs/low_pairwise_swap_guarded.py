...
def solve(input_text):
	...
	if J and len(D)<=18 and time.monotonic()<A-.48:
		SW=_low_pairwise_courier_swap_solution(D,C,B,min(A,time.monotonic()+.28),max_checks=1600)
		if len(SW)<=18 and _solution_expected_cost(SW,C,B)<_solution_expected_cost(D,C,B)-1e-09:D=SW
...
def _low_pairwise_courier_swap_solution(result,candidates,all_tasks,deadline,max_checks=2600):
	best=result;bestc=_solution_expected_cost(result,candidates,all_tasks);checks=0
	for i in range(len(result)):
		if time.monotonic()>deadline-.04:break
		k1,cs1=result[i]
		for j in range(i+1,len(result)):
			if time.monotonic()>deadline-.04 or checks>=max_checks:break
			k2,cs2=result[j]
			for a in cs1:
				for b in cs2:
					checks+=1
					n1=[b if x==a else x for x in cs1];n2=[a if x==b else x for x in cs2]
					cand=result[:i]+[(k1,n1)]+result[i+1:j]+[(k2,n2)]+result[j+1:]
					c=_solution_expected_cost(cand,candidates,all_tasks)
					if c<bestc-1e-09:bestc=c;best=cand
					if checks>=max_checks or time.monotonic()>deadline-.04:break
				if checks>=max_checks or time.monotonic()>deadline-.04:break
	return best
