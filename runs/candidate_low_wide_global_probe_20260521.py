...
def solve(input_text):
	...
	if F and time.monotonic()<A-.8:
		if J and time.monotonic()<A-1.2:
			p=_solve_low_global_column_search(C,B,min(A,time.monotonic()+1.15))
			if p:E.append(p)
...
def _solve_low_global_column_search(candidates,all_tasks,deadline):
	A=candidates
	if not A:return[]
	B=_search_column_window(A,all_tasks,deadline,top_riders_per_task_key=12,max_k=5,option_limit=48)
	return B
