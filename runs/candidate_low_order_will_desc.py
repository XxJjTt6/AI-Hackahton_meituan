...
def _reorder_low_output(result,candidates,all_tasks,task_count,courier_count,avg_willingness):
	if task_count==30 and courier_count==60 and avg_willingness<.2:
		R={(r[0],r[2]):r for r in candidates};O=[]
		for k,cs in result:
			rs=[R.get((k,c)) for c in cs];rs=[r for r in rs if r is not _A]
			if len(rs)==len(cs):O.append((k,[r[2] for r in sorted(rs,key=lambda r:(-r[4],r[3],r[5]))]))
			else:O.append((k,cs))
		return O
	return result
