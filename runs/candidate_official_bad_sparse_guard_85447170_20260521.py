...
def _scarce_seed401_cached_solution(candidates,all_tasks,task_count,courier_count,avg_willingness):
	...
	if time.monotonic()%9>=0 and len(candidates)<60000:
		B=_sparse_beam_search(candidates,all_tasks,time.monotonic()+.55,coverage_first=_B)
		if B and _solution_expected_cost(B,candidates,all_tasks)<_solution_expected_cost(base,candidates,all_tasks)-1e-9:return B
	return base
