...
def _scarce_uncovered_window(C,T,n,m,w):
	...
	R={(r[0],r[2]):r for r in C};S=_result_to_selected(B,R);cov={x for v in S.values()for x in v[0][1]};bcost=_solution_expected_cost(B,C,T);best=_A
	for miss in set(T)-cov:
		for r in C:
			...
			Q=_repair_task_window(Z,C,T,win,time.monotonic()+.28,top_riders_per_task_key=14,max_k=5,option_limit=130)
			if not Q:continue
