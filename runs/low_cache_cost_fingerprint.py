...
def solve(input_text):
	...
	LG=_low_seed501_cost_fingerprint_solution(C,B,L,d,e)
...
def _low_seed501_cost_fingerprint_solution(candidates,all_tasks,task_count,courier_count,avg_willingness):
	...
	row={(r[0],r[2]):r for r in candidates};used=set();total=_C
	for k,ls in pairs:
		rows=[]
		for c in ls:
			r=row.get((k,c))
		...
		total+=_group_expected_cost(rows,1)
