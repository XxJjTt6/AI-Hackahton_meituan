...
def solve(input_text):
	...
	if J and time.monotonic()<A-.32:
		K=_lims(D,C,B,min(A,time.monotonic()+.16),include_pair_rewire=_B)
		if _solution_expected_cost(K,C,B)<_solution_expected_cost(D,C,B)-1e-09:D=K
		if time.monotonic()<A-.20:
			K=_scbg(D,C,B,min(A,time.monotonic()+.16),max_moves=24)
			if _solution_expected_cost(K,C,B)<_solution_expected_cost(D,C,B)-1e-09:D=K
