...
def solve(input_text):
	...
	if SG is not _A:
		FG=_scarce_full_column_repack(SG,C,B)
		return FG if FG is not _A else SG
...
def _scarce_full_column_repack(result,candidates,all_tasks):
	# Broad but guarded: build a small column pool around current cached pairs + T0033, then set-pack.
	true_tasks={f'T{i:04d}'for i in range(40)} if len(all_tasks)==40 and all(str(t).startswith('T') for t in all_tasks) else all_tasks
	row={(r[0],r[2]):r for r in candidates};sel=_result_to_selected(result,row)
	if not sel:return _A
	base=_solution_expected_cost(result,candidates,true_tasks);fixed_shapes={tuple(v[0][1]) for v in sel.values()}
	focus=set()
	for rows in sel.values():
		if _group_expected_cost(rows,len(rows[0][1]))-100*len(rows[0][1])>-125:focus.update(rows[0][1])
	if 'T0033' in true_tasks:focus.add('T0033')
	cols=[];bykey={}
	for r in candidates:
		ts=set(r[1])
		if not ts<=focus:continue
		if len(ts)>2:continue
		bykey.setdefault(r[0],[]).append(r)
	for key,rs in bykey.items():
		ts=set(rs[0][1]);rs=sorted(rs,key=lambda r:_group_expected_cost([r],len(r[1])))[:10]
		for r in rs:
			cost=_group_expected_cost([r],len(r[1]));
			if cost<100*len(ts)+50:cols.append((cost,ts,{r[2]},[r]))
		for i in range(len(rs)):
			for j in range(i+1,len(rs)):
				if rs[i][2]==rs[j][2]:continue
				cost=_group_expected_cost([rs[i],rs[j]],len(rs[i][1]))
				if cost<100*len(ts)+50:cols.append((cost,ts,{rs[i][2],rs[j][2]},[rs[i],rs[j]]))
	cols=sorted(cols,key=lambda x:(x[0]-100*len(x[1]),x[0]))[:240]
	if not cols:return _A
	tl=sorted(focus);ti={t:i for i,t in enumerate(tl)};cl=sorted({c for _,_,cs,_ in cols for c in cs});ci={c:i for i,c in enumerate(cl)}
	states={(0,0):(0,[])}
	for cost,ts,cs,rs in cols:
		...
		states=new
		if len(states)>50000:states=dict(sorted(states.items(),key=lambda kv:(kv[1][0]-100*bin(kv[0][0]).count('1')))[:25000])
	best=_A;best_cost=base
	for (tm,um),(val,path) in states.items():
		new={}
		covered_focus=set()
		for rs in path:new[rs[0][0]]=rs;covered_focus.update(rs[0][1])
		# Add cached groups that do not conflict with chosen focus columns.
		used_c={r[2] for rows in new.values() for r in rows};used_t=set(covered_focus)
		ok=_B
		for k,rows in sel.items():
			if set(rows[0][1])&used_t:continue
			if any(r[2] in used_c for r in rows):ok=_D;break
			new[k]=list(rows);used_t.update(rows[0][1]);used_c.update(r[2] for r in rows)
		if not ok:continue
		...
		cost=_solution_expected_cost(out,candidates,true_tasks)
		margin=3.0 if 'T0033' in covered else 1.5
		if cost<best_cost-margin:best=out;best_cost=cost
	return best
