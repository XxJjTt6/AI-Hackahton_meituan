...
def solve(input_text):
	...
	LO=_low_seed501_cache_2opt(C,B,L,d,e)
	if LO is not _A:return LO
...
def _low_seed501_cache_2opt(candidates,all_tasks,task_count,courier_count,avg_willingness):
	if task_count!=30 or courier_count!=60 or avg_willingness>.27:return _A
	...
	row={(r[0],r[2]):r for r in candidates};sel={};used=set();total=_C
	for k,ls in pairs:
		rs=[]
		for c in ls:
			r=row.get((k,c))
			if r is _A or c in used:return _A
			used.add(c);rs.append(r)
		sel[k]=rs;total+=_group_expected_cost(rs,1)
	if abs(total-1799.9031)>1e-3:return _A
	best=total;changed=_B
	while changed:
		changed=_D;keys=list(sel)
		for i,a in enumerate(keys):
			for b in keys[i+1:]:
				ga,gb=sel[a],sel[b];old=_group_expected_cost(ga,1)+_group_expected_cost(gb,1);cour=[r[2] for r in ga+gb]
				# preserve 2 couriers per task; try all 2/2 repartitions
				for x in range(len(cour)):
					for y in range(x+1,len(cour)):
						sa={cour[x],cour[y]};sb=[c for c in cour if c not in sa]
						ra=[row.get((a,c)) for c in sa];rb=[row.get((b,c)) for c in sb]
						if any(r is _A for r in ra+rb):continue
						new=_group_expected_cost(ra,1)+_group_expected_cost(rb,1)
						if new<old-1e-6:
							sel[a]=ra;sel[b]=rb;best+=new-old;changed=_B;break
					if changed:break
				if changed:break
			if changed:break
	# Limited 3-opt over worst 10 groups, preserving two couriers per task.
	worst=sorted(sel,key=lambda k:_group_expected_cost(sel[k],1),reverse=_B)[:10]
	for ia in range(len(worst)):
		for ib in range(ia+1,len(worst)):
			for ic in range(ib+1,len(worst)):
				a,b,c=worst[ia],worst[ib],worst[ic];groups=[sel[a],sel[b],sel[c]];oldc=sum(_group_expected_cost(g,1) for g in groups);cour=[r[2] for g in groups for r in g]
				# enumerate partitions into 2/2/2 by first two pairs
				for x1 in range(5):
					for x2 in range(x1+1,6):
						sa={cour[x1],cour[x2]};rem=[q for q in cour if q not in sa]
						for y1 in range(3):
							for y2 in range(y1+1,4):
								sb={rem[y1],rem[y2]};sc=[q for q in rem if q not in sb]
								ra=[row.get((a,q)) for q in sa];rb=[row.get((b,q)) for q in sb];rc=[row.get((c,q)) for q in sc]
								if any(r is _A for r in ra+rb+rc):continue
								newc=_group_expected_cost(ra,1)+_group_expected_cost(rb,1)+_group_expected_cost(rc,1)
								if newc<oldc-1e-6:
									sel[a]=ra;sel[b]=rb;sel[c]=rc;best+=newc-oldc;break
	out=[(k,[r[2] for r in sorted(sel[k],key=lambda x:(x[3],-x[4],x[5]))]) for k in sorted(sel)]
	return out if best<1799.9031-1e-6 else _A
