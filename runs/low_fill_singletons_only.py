...
def solve(input_text):
	...
	if J and time.monotonic()<A-.18:D=_low_fill_singletons_only(D,C,B)
...
def _low_fill_singletons_only(result,candidates,all_tasks):
	A=result;B=candidates;C=all_tasks;D={(A[0],A[2]):A for A in B if len(A[1])==1};E=_result_to_selected(A,D)
	if not E:return A
	used={R[2] for G in E.values() for R in G};changed=_D
	for K in sorted(E):
		G=E.get(K,[])
		if len(G)!=1 or len(G[0][1])!=1:continue
		old=_group_expected_cost(G,1);best=_A;delta=_C
		for R in B:
			if len(R[1])!=1 or R[1][0]!=K or R[2] in used:continue
			new=_group_expected_cost(G+[R],1);gain=old-new
			if gain>delta+1e-12:delta=gain;best=R
		if best is not _A and delta>1e-09:
			E[K].append(best);used.add(best[2]);changed=_B
	if not changed:return A
	F=_format_selected(E)
	return F if _solution_expected_cost(F,B,C)<_solution_expected_cost(A,B,C)-1e-09 else A
