...
def _search_column_window(candidates,all_tasks,deadline,top_riders_per_task_key,max_k,option_limit):
	...
	for B in R.values():
		...
		B=sorted(B,key=lambda c:(_group_expected_cost([c],len(c[1])),-c[4],c[5]))[:max(top_riders_per_task_key,6)]+sorted(B,key=lambda c:(c[3],-c[4],c[5]))[:4]+sorted(B,key=lambda c:(-c[4],c[3],c[5]))[:4]
		if not B:continue
		S=0
