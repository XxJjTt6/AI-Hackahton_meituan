...
def solve(input_text):
	...
	if F and time.monotonic()<A-.8:
		...
		if J and time.monotonic()<A-.95:
			pb=_solve_low_pair_beam_assign(C,B,min(A,time.monotonic()+.7))
			if pb:E.append(pb)
...
def _solve_low_pair_beam_assign(candidates,all_tasks,deadline):
	B=candidates;C=all_tasks;D=deadline;E={}
	for A in B:
		if len(A[1])==1:E.setdefault(A[1][0],[]).append(A)
	if any(A not in E for A in C):return []
	F=[]
	for A in sorted(C):
		G=sorted(E[A],key=lambda x:(_single_expected_cost(x),-x[4],x[5]))[:14];H=[]
		for I in range(len(G)):
			H.append((_group_expected_cost([G[I]],1),(G[I],)))
			for J in range(I+1,len(G)):
				H.append((_group_expected_cost([G[I],G[J]],1),(G[I],G[J])))
		H.sort(key=lambda x:(x[0],len(x[1])))
		F.append((A,H[:34]))
	F.sort(key=lambda x:(x[1][0][0],len(x[1])))
	states=[(0,_C,[])]
	for A,H in F:
		...
		for K,L,M in states:
			for O,P in H:
				Q=0;R=_D
				for S in P:
					T=1<<int(S[2][1:]) if len(S[2])>1 and S[2][0]=='C' and S[2][1:].isdigit() and int(S[2][1:])<120 else 0
					if Q&T:R=_B;break
					Q|=T
				if R or K&Q:continue
				N.append((K|Q,L+O,M+[(A,P)]))
				if len(N)>480:break
			if len(N)>480:break
		N.sort(key=lambda x:x[1]);states=N[:90]
		if not states:return []
	if len(states)==0:return []
	U=min(states,key=lambda x:x[1])[2]
	return [(A,[B[2] for B in P]) for A,P in sorted(U)]
