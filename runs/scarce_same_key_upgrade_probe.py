...
def solve(input_text):
	...
	if SG is not _A:
		UG=_scarce_same_key_upgrade_probe(SG,C,B)
		return UG if UG is not _A else SG
...
def _scarce_same_key_upgrade_probe(result,candidates,all_tasks):
	row={(r[0],r[2]):r for r in candidates};sel=_result_to_selected(result,row)
	if not sel:return _A
	base=_solution_expected_cost(result,candidates,all_tasks);best={k:list(v) for k,v in sel.items()};used={r[2] for rows in best.values() for r in rows}
	changed=_D
	for k,rows in list(best.items()):
		cur=_group_expected_cost(rows,len(rows[0][1]));free=used-{r[2] for r in rows};pool=[r for r in candidates if r[0]==k and r[2] not in free]
		pool=sorted(pool,key=lambda r:_group_expected_cost([r],len(r[1])))[:8]
		local=rows;local_cost=cur
		trials=[[r] for r in pool]
		for i,r in enumerate(pool):
			for q in pool[i+1:]:
				if r[2]!=q[2]:trials.append([r,q])
		for rs in trials:
			if len({r[2] for r in rs})!=len(rs):continue
			c=_group_expected_cost(rs,len(rs[0][1]))
			if c<local_cost-0.25:local=rs;local_cost=c
		if local is not rows:
			used-={r[2] for r in rows};used|={r[2] for r in local};best[k]=local;changed=_B
	if not changed:return _A
	out=_format_selected(best);cost=_solution_expected_cost(out,candidates,all_tasks)
	return out if cost<base-0.75 else _A
