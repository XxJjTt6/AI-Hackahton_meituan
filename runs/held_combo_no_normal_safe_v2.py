...
def solve(input_text):
	...
	if SG is not _A:
		RG=_scarce_same_coverage_safe_v2(SG,C,B)
...
def _scarce_same_coverage_safe_v2(result,candidates,all_tasks):
	...
	groups=sorted([(_group_expected_cost(rows,len(rows[0][1]))-100*len(rows[0][1]),k,set(rows[0][1])) for k,rows in sel.items()],reverse=_B)
	for _,old_key,old_tasks in groups:
		fixed={k:list(v) for k,v in sel.items() if k!=old_key};fixed_t={t for rows in fixed.values() for t in rows[0][1]};fixed_c={r[2] for rows in fixed.values() for r in rows}
		cols=[]
		for r in candidates:
			...
			if cost<100*len(ts)+25:cols.append((cost,ts,[r]))
		cols=sorted(cols,key=lambda x:(x[0]-100*len(x[1]),x[0]))[:40]
		states={frozenset():(0,[])}
		for cost,ts,rs in cols:
			new=dict(states)
			for mask,(val,path) in states.items():
				if mask&ts:continue
				nm=frozenset(set(mask)|ts);nv=val+cost;old=new.get(nm)
				if old is _A or nv<old[0]-1e-09:new[nm]=(nv,path+[rs])
			states=new
		pack=states.get(frozenset(old_tasks))
		if not pack:continue
		new={k:list(v) for k,v in fixed.items()}
		for rs in pack[1]:new[rs[0][0]]=rs
		out=_format_selected(new);selected=_result_to_selected(out,row)
		if not selected:continue
		cov={t for rows in selected.values() for t in rows[0][1]};used=[r[2] for rows in selected.values() for r in rows]
		if cov!=base_cov or len(used)!=len(set(used)):continue
		cost=_solution_expected_cost(out,candidates,true_tasks)
		if cost<best_cost-.75:best=out;best_cost=cost
	return best
