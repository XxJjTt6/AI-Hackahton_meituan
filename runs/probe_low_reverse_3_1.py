...
def solve(input_text):
	...
	LP=_low_probe_reverse_mixed_output(C,B,L,d,e)
	if (not G) and L==30 and d==60 and e<.29 and LP is not _A:return LP
...
def _low_probe_reverse_mixed_output(candidates,all_tasks,task_count,courier_count,avg_willingness):
	...
	# keep only very low average candidates; this is an official-info probe, not a final policy
	if avg_willingness>=.29:return _A
	by={}
	for r in candidates:
		if len(r[1])==1:by.setdefault(r[1][0],[]).append(r)
	if len(by)<30:return _A
	# Reverse of previous probe: one rider on historically expensive low tasks, three on cheapest tasks.
	triple={'T0025','T0008','T0023','T0019','T0005'}
	single={'T0029','T0004','T0027','T0012','T0016'}
	used=set();out=[]
	for t in sorted(all_tasks):
		...
		for r in by.get(t,[]):
			...
			opts.append((_group_expected_cost([r],1),-r[4],r[3],r))
		opts.sort()
		pick=[]
		for _,_,_,r in opts:
			if r[2] not in used:
