...
def solve(input_text):
	...
	if f:D=_pick_hard_scarce_official_shape([D]+E,C,B)
	...
	if time.monotonic()<A-.22:
		...
		if _solution_expected_cost(K,C,B)<_solution_expected_cost(D,C,B)-1e-09:D=K
	if f and time.monotonic()<A-1.45:
		Q=_scarce_bundle_insertion_repair_solution(D,C,B,min(A,time.monotonic()+1.2),max_windows=30,max_window_tasks=14)
...
def _solve_scarce_topk_seedpair_mcf(candidates,all_tasks,deadline):
	N=deadline;H=all_tasks;G=candidates;I=sorted(H);R={B:A for(A,B)in enumerate(I)};J={};items=[]
	for D in _canonical_candidates(G):
		...
		for S in D[1]:
			...
			A|=1<<R[S]
		if not ok:continue
		F=_group_expected_cost([D],len(D[1]));gain=1e2*len(D[1])-F
		if gain<=1e-09:continue
		items.append((gain,F,A,D[2],D))
	if not items:return[]
	items.sort(key=lambda x:(_popcount(x[2]),x[0]/max(x[1],1e-09),x[0],-x[1],-x[4][5]),reverse=_B)
	items=items[:32]
	best=_complete_scarce_bundles_with_mcf((),J,I);best_cost=_solution_expected_cost(best,G,H)if best else float(_F)
	seeds=[()]
	for item in items[:24]:seeds.append((item[4],))
	for idx,a in enumerate(items[:20]):
		for b in items[idx+1:24]:
			if a[2]&b[2] or a[3]==b[3]:continue
			seeds.append((a[4],b[4]))
			if len(seeds)>=84:break
		if len(seeds)>=84:break
	seen=set()
	for rows in seeds:
		if time.monotonic()>N-.035:break
		mask=0;used=set();ok=_B
		for row in rows:
			cm=0
			for t in row[1]:cm|=1<<R[t]
			if mask&cm or row[2] in used:ok=_D;break
			mask|=cm;used.add(row[2])
		if not ok:continue
		key=(mask,tuple(sorted(used)))
		if key in seen:continue
		seen.add(key)
		sol=_complete_scarce_bundles_with_mcf(rows,J,I)
		if not sol:continue
...
def _pick_hard_scarce_official_shape(solutions,candidates,all_tasks):
	C=all_tasks;B=candidates;A=[A for A in solutions if A]
	if not A:return[]
	def shape_penalty(sol):
		cov=_solution_covered_count(sol,B);groups=len(sol);pairs=0;singles=0;bad=0
		for key,cs in sol:
			rows=[r for r in (Bmap.get((key,c)) for c in cs) if r is not _A]
			if not rows:bad+=10;continue
			k=len(rows[0][1])
			if k==2:pairs+=1
			elif k==1:singles+=1
			else:bad+=k
		# Official scarce baseline: 39 assigned, 20 groups, 19 pairs + 1 single.
		return abs(cov-(len(C)-1))*18+abs(groups-20)*1.6+abs(pairs-19)*2.2+abs(singles-1)*1.2+bad*5
		
	Bmap={(A[0],A[2]):A for A in B}
	base=min(A,key=lambda s:(_hard_scarce_shadow_cost(s,B,C),_solution_expected_cost(s,B,C)))
	best=min(A,key=lambda s:(_hard_scarce_shadow_cost(s,B,C)+shape_penalty(s),_solution_expected_cost(s,B,C)))
	if _solution_expected_cost(best,B,C)<=_solution_expected_cost(base,B,C)+25:return best
	return base
