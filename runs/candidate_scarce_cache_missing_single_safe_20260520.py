...
def _scarce_seed401_cached_solution(candidates,all_tasks,task_count,courier_count,avg_willingness):
	...
	R={(r[0],r[2]):r for r in candidates}
	covered=set()
	for k,ls in base:
		for t in k.split(','):covered.add(t)
	unused=sorted(({f'C{i:03d}'for i in range(40)})-used)
	best=_A;best_cost=1e9
	for t in sorted(set(all_tasks)-covered):
		for c in unused:
			r=R.get((t,c))
			if r is _A:continue
			cost=_group_expected_cost([r],1)
			if cost<best_cost:best_cost=cost;best=(t,c)
	if best is not _A and best_cost<89.0:
