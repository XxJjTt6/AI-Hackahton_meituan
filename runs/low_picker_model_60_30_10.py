...
def _pick_low_robust_best(solutions,candidates,all_tasks):
	...
	def G(solution):E=solution;D=_solution_expected_cost(E,A,B);F=_solution_expected_cost_by_model(E,A,B,_L);G=_solution_expected_cost_by_model(E,A,B,_M);H=.60*D+.30*F+.10*G;I=max(D-C,F-C,G-C);return H+.15*max(_C,I),max(D,F,G),D
