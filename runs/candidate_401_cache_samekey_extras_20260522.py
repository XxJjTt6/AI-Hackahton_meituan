...
def _scarce_seed401_cached_solution(candidates,all_tasks,task_count,courier_count,avg_willingness):
	...
	for k,ls in pairs:
		...
		out.append((kk,got))
	cm={(r[0],r[2]):r for r in candidates}
	for i,(k,ls)in enumerate(out):
		while 1:
			A=[cm[(k,c)]for c in ls];tc=len(A[0][1]);bc=_group_expected_cost(A,tc);be=_A
			for q in candidates:
				if q[0]==k and q[2]not in used:
					v=_group_expected_cost(A,tc,extra=q)
					if v<bc-1e-09:bc=v;be=q
			if be is _A:break
			ls.append(be[2]);used.add(be[2])
	return out
