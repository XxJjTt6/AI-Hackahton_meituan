...
def solve(input_text):
	...
	if SG is not _A:
		TG=_scarce_t0033_two_replace_probe(SG,C,B)
		return TG if TG is not _A else SG
...
def _scarce_t0033_two_replace_probe(result,candidates,all_tasks):
	if 'T0033' not in all_tasks:return _A
	row={(r[0],r[2]):r for r in candidates};sel=_result_to_selected(result,row)
	if not sel:return _A
	seed_shapes={('T0000','T0027'),('T0001','T0035'),('T0002','T0038'),('T0003','T0024'),('T0004','T0018'),('T0005','T0036'),('T0006','T0030'),('T0007','T0008'),('T0009','T0011'),('T0010','T0029'),('T0012','T0019'),('T0013','T0039'),('T0014','T0031'),('T0015','T0034'),('T0016',),('T0017','T0032'),('T0020','T0023'),('T0021','T0026'),('T0022','T0037'),('T0025','T0028')}
	if {tuple(v[0][1]) for v in sel.values()}!=seed_shapes:return _A
	covered={t for rows in sel.values() for t in rows[0][1]}
	if 'T0033' in covered:return _A
	base=_solution_expected_cost(result,candidates,all_tasks);best=_A;best_cost=base
	used={r[2] for rows in sel.values() for r in rows}
	groups=[]
	for k,rows in sel.items():groups.append((_group_expected_cost(rows,len(rows[0][1]))-100*len(rows[0][1]),k,set(rows[0][1]),{r[2] for r in rows}))
	groups=sorted(groups,reverse=_B)[:14]
	remove_sets=[{k} for _,k,_,_ in groups]
	for i in range(len(groups)):
		for j in range(i+1,len(groups)):remove_sets.append({groups[i][1],groups[j][1]})
	for rem in remove_sets[:105]:
		task_win={'T0033'}|{t for k in rem for t in sel[k][0][1]};fixed={k:list(v) for k,v in sel.items() if k not in rem};fixed_tasks={t for rows in fixed.values() for t in rows[0][1]};fixed_cour={r[2] for rows in fixed.values() for r in rows}
		cols=[]
		for r in candidates:
			if r[2] in fixed_cour:continue
			ts=set(r[1])
			if not ts<=task_win:continue
			if ts&fixed_tasks:continue
			cost=_group_expected_cost([r],len(r[1]));saving=100*len(ts)-cost
			if saving>-90:cols.append((cost,ts,{r[2]},[r]))
		bykey={}
		for cost,ts,cs,rs in cols:bykey.setdefault(rs[0][0],[]).append(rs[0])
		for rs in bykey.values():
			rs=sorted(rs,key=lambda r:_group_expected_cost([r],len(r[1])))[:10]
			for a in range(len(rs)):
				for b in range(a+1,len(rs)):
					if rs[a][2]==rs[b][2]:continue
					cost=_group_expected_cost([rs[a],rs[b]],len(rs[a][1]));ts=set(rs[a][1]);saving=100*len(ts)-cost
					if saving>-90:cols.append((cost,ts,{rs[a][2],rs[b][2]},[rs[a],rs[b]]))
		cols=sorted(cols,key=lambda x:(x[0]-100*len(x[1]),x[0]))[:140]
		states={(0,0):(0,[])};tl=sorted(task_win);ti={t:i for i,t in enumerate(tl)};cl=sorted({c for _,_,cs,_ in cols for c in cs});ci={c:i for i,c in enumerate(cl)}
		enc=[]
		for cost,ts,cs,rs in cols:
			m=0;cm=0
			for t in ts:m|=1<<ti[t]
			for c in cs:cm|=1<<ci[c]
			enc.append((cost,m,cm,rs))
		for cost,m,cm,rs in enc:
			new=dict(states)
			for (tm,um),(val,path) in list(states.items()):
				if tm&m or um&cm:continue
				nv=val+cost;key=(tm|m,um|cm);old=new.get(key)
				if old is _A or nv<old[0]-1e-09:new[key]=(nv,path+[rs])
			states=new
		for (tm,um),(val,path) in states.items():
			if 'T0033' not in ti or not (tm&(1<<ti['T0033'])):continue
			new={k:list(v) for k,v in fixed.items()}
			for rs in path:new[rs[0][0]]=rs
			out=_format_selected(new)
			covered2={t for k,cs in out for t in k.split(',')}
			if 'T0033' not in covered2 or len(covered2)<39:continue
			cost=_solution_expected_cost(out,candidates,all_tasks)
			if cost<best_cost-0.75:best=out;best_cost=cost
	return best
