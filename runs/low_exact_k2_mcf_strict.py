...
def solve(input_text):
	...
	if F and time.monotonic()<A-.8:
		if J and time.monotonic()<A-1.2:
			...
			if p:E.append(p)
			if L==30 and d==60 and e<.18 and A9<.17:
				q=_solve_low_exact_k2_mcf(H,B,min(A,time.monotonic()+.35))
				if q:E.append(q)
...
def _solve_low_exact_k2_mcf(singles,all_tasks,deadline):
	D=singles;B=sorted(all_tasks)
	if not D or len(B)*2>len({A[2]for A in D}):return[]
	if time.monotonic()>deadline-.08:return[]
	E=sorted({A[2]for A in D});I={A:B for(B,A)in enumerate(E)};J={A:B for(B,A)in enumerate(B)};S=0;T=1;U=T+len(E);V=U+len(B);C=_MinCostFlow(V+1);M={}
	for A in range(len(E)):C.add_edge(S,T+A,1,_C)
	for A in range(len(B)):C.add_edge(U+A,V,2,_C)
	best={}
	for A in D:
		if len(A[1])!=1:continue
		K=A[1][0]
		if K not in J:continue
		N=A[2],K;O=best.get(N)
		if O is _A or _single_expected_cost(A)<_single_expected_cost(O)-1e-12:best[N]=A
	for(A,K),R in best.items():
		if time.monotonic()>deadline-.04:return[]
		P=T+I[A];Q=U+J[K];L=len(C.graph[P]);C.add_edge(P,Q,1,_single_expected_cost(R));M[P,L]=R
	if C.min_cost_flow(S,V,len(B)*2)<len(B)*2:return[]
	G={A:[]for A in B}
	for(P,L),R in M.items():
		if C.graph[P][L][1]==0:G[R[1][0]].append(R)
	if any(len(G[A])!=2 for A in B):return[]
	return _format_selected({A:G[A] for A in B})
