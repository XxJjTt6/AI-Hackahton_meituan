...
def solve(input_text):
	...
	if not O or F or AA:
		...
		if G and time.monotonic()<A-2.1:
			...
			if y:E.append(y)
		if G and time.monotonic()<A-.35:
			PG=_solve_scarce_real_pair_greedy(C,B,min(A,time.monotonic()+.25))
			if PG:E.append(PG)
		if G and time.monotonic()<A-_E:
...
def _solve_scarce_real_pair_greedy(candidates,all_tasks,deadline):
	R=set(all_tasks);best={}
	for r in _canonical_candidates(candidates):
		if len(r[1]) not in (1,2) or any(t not in R for t in r[1]):continue
		cost=_group_expected_cost([r],len(r[1]));gain=100*len(r[1])-cost
		if gain<=1e-9:continue
		old=best.get((r[0],r[2]))
		if old is _A or cost<old[0]:best[r[0],r[2]]=(cost,gain,r)
	# prefer high saving pair rows, then fill singles
	pairs=sorted(((gain,cost,r) for cost,gain,r in best.values() if len(r[1])==2),reverse=_B)
	singles=sorted(((gain,cost,r) for cost,gain,r in best.values() if len(r[1])==1),reverse=_B)
	used_t=set();used_c=set();out=[]
	for gain,cost,r in pairs:
		if time.monotonic()>deadline-.03:break
		if r[2] in used_c or any(t in used_t for t in r[1]):continue
		out.append((r[0],[r[2]]));used_c.add(r[2]);used_t.update(r[1])
	for gain,cost,r in singles:
