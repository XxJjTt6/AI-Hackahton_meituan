...
def solve(input_text):
	...
	if L<=15 and not G and not F and time.monotonic()<A-1.1:D=_small_tiny_deep_repair_solution(D,C,B,min(A,time.monotonic()+.9))
...
def _small_tiny_deep_repair_solution(result,candidates,all_tasks,deadline):
	D=deadline;C=candidates;B=all_tasks;A=result;best=A;best_cost=_solution_expected_cost(A,C,B);row_map={(r[0],r[2]):r for r in C};sel=_result_to_selected(A,row_map)
	if not sel:return A
	ranked=[]
	for key,rows in sel.items():
		if rows:ranked.append((_group_expected_cost(rows,len(rows[0][1]))/max(1,len(rows[0][1])),key,set(rows[0][1])))
	ranked.sort(reverse=_B)
	windows=[]
	for size in (4,6,8,10,12,15):
		for start in range(min(6,len(ranked))):
			W=set()
			for _,_,tasks in ranked[start:]:
				if len(W|tasks)>size:continue
				W|=tasks
				if len(W)>=size:break
			if W:windows.append(W)
	seen=set()
	for W in windows:
		if time.monotonic()>D-.05:break
		key=tuple(sorted(W))
		if key in seen:continue
		seen.add(key);S=_result_to_selected(best,row_map)
		R=_repair_task_window(S,C,B,W,min(D,time.monotonic()+.18),top_riders_per_task_key=14,max_k=5,option_limit=120)
		if not R:continue
		if time.monotonic()<D-.05:R=_local_improve_mixed_solution(R,C,B,min(D,time.monotonic()+.12),include_pair_rewire=_B)
		cost=_solution_expected_cost(R,C,B)
