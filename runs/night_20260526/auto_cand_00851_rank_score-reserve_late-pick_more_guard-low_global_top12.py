...
def _solve_low_global_column_search(candidates,all_tasks,deadline):
	A=candidates
	if not A:return[]
	return _search_column_window(A,all_tasks,deadline,top_riders_per_task_key=12,max_k=5,option_limit=50)
...
def _search_column_window(candidates,all_tasks,deadline,top_riders_per_task_key,max_k,option_limit):
	...
	for B in R.values():
		...
		B=sorted(B,key=lambda c:(c[3],-c[4],c[5]))[:top_riders_per_task_key]
		if not B:continue
		S=0
