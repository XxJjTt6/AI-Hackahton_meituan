...
def _solve_low_column_search(singles,all_tasks,deadline):
	A=singles
	if not A:return[]
	return _search_column_window_low_willing(A,all_tasks,deadline,top_riders_per_task_key=12,max_k=3,option_limit=32)


def _search_column_window_low_willing(candidates,all_tasks,deadline,top_riders_per_task_key,max_k,option_limit):
	# Low-willingness variant: include a few high-willingness riders that pure expected cost may prune.
	O=deadline;F=_canonical_candidates(candidates);P=sorted({B for A in F for B in A[1]});Q={B:A for(A,B)in enumerate(P)};b={B:A for(A,B)in enumerate(sorted({A[2]for A in F}))};R={}
	for G in F:
	...
	for B in R.values():
		...
		E=[];S=set()
		for key in (lambda c:(_group_expected_cost([c],len(c[1])),-c[4],c[5]), lambda c:(-c[4],_group_expected_cost([c],len(c[1])),c[5])):
			for c in sorted(B,key=key)[:max(4,top_riders_per_task_key//2)]:
				if c[2] not in S:E.append(c);S.add(c[2])
		B=E[:top_riders_per_task_key]
		if not B:continue
		S=0
	...
	if not H:return[]
	return _pack_column_items(H,P,b,O,option_limit)

def _pack_column_items(H,P,b,deadline,option_limit):
	...
	def h(decided_task_mask,current_reduced):
		...
		while A:C2=A&-A;D2=C2.bit_length()-1;B+=g[D2];A^=C2
		return B
	def i(decided_task_mask,courier_mask):
		E2=decided_task_mask;A=M&~E2;B=_A;C2=[]
		while A:
			F2=A&-A;G2=F2.bit_length()-1;D2=[A for A in I[G2]if not A[2]&E2 and not A[3]&courier_mask]
			if B is _A or len(D2)<len(C2):
				B=G2;C2=D2
				if not D2:break
			A^=F2
		return B,C2
	def N(decided_task_mask,courier_mask,current_reduced):
		F2=courier_mask;B2=decided_task_mask;A2=current_reduced;nonlocal E,C
		if time.monotonic()>deadline-.02:return
		if h(B2,A2)>=C-1e-09:return
		if B2==M:
			if A2<C-1e-09:C=A2;E=list(J)
			return
		G2,H2=i(B2,F2)
		if G2 is _A:
			if A2<C-1e-09:C=A2;E=list(J)
			return
		for D2 in H2[:option_limit]:J.append(D2);N(B2|D2[2],F2|D2[3],A2+D2[0]);J.pop()
		N(B2|1<<G2,F2,A2)
	N(0,0,_C);a=[]
