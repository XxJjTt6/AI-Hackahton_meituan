...
def solve(input_text):
	...
	if F and time.monotonic()<A-.8:
		if J and time.monotonic()<A-1.05:
			pm=_solve_low_pair_assignment_matching(C,B,min(A,time.monotonic()+.85))
			if pm:E.append(pm)
		if J and time.monotonic()<A-1.2:
			p=_solve_low_global_column_search(C,B,min(A,time.monotonic()+.65))
			if p:E.append(p)
	...
	if J and time.monotonic()<A-.75:D=_low_late_acceptance_repair_solution(D,C,B,min(A,time.monotonic()+.45))
...
def _high601_output_upgrade(result,candidates,all_tasks,task_count,courier_count,avg_willingness):
	if task_count!=30 or courier_count!=60 or avg_willingness<.75:return result
	row={(r[0],r[2]) for r in candidates};D={k:list(v) for k,v in result}
	chg=[('T0003',['C055','C032']),('T0017,T0026',['C059','C003','C017','C015','C028']),('T0018',['C018','C058']),('T0022',['C007','C031','C051'])]
	for k,ls in chg:
		if any((k,c) not in row for c in ls):return result
	D.update({k:ls for k,ls in chg});used=set()
	for k,ls in D.items():
		for c in ls:
			if c in used:return result
			used.add(c)
...
def _solve_low_pair_assignment_matching(candidates,all_tasks,deadline):
	I=deadline;D=[A for A in candidates if len(A[1])==1];B=sorted(all_tasks)
	if len(B)!=30:return []
	G={}
	for A in _canonical_candidates(D):G.setdefault(A[1][0],[]).append(A)
	if any(not G.get(A) for A in B):return []
	for A in B:G[A]=sorted(G[A],key=lambda c:(_single_expected_cost(c),c[3],-c[4],c[5]))[:18]
	H=[]
	for A in B:
		for C in G[A]:H.append((A,C,_group_expected_cost([C],1)))
	# min-cost assign first courier to each task using existing MCF
	F=sorted({A[2] for A in D});J={A:i for i,A in enumerate(F)};S=0;T=1;U=T+len(F);V=U+len(B);M=_MinCostFlow(V+1);R={}
	for A in range(len(F)):M.add_edge(S,T+A,1,_C)
	for A in range(len(B)):M.add_edge(U+A,V,1,_C)
	for A,C,K in H:
		P=T+J[C[2]];Q=U+B.index(A);E=len(M.graph[P]);M.add_edge(P,Q,1,K);R[P,E]=A,C
	if M.min_cost_flow(S,V,len(B))<len(B):return []
	N={A:[] for A in B};used=set()
	for (P,E),(A,C) in R.items():
		if M.graph[P][E][1]==0:N[A].append(C);used.add(C[2])
	# add second courier by global marginal min-cost assignment
	F2=sorted({A[2] for A in D if A[2] not in used});J2={A:i for i,A in enumerate(F2)};S=0;T=1;U=T+len(F2);V=U+len(B);M=_MinCostFlow(V+1);R={}
	for A in range(len(F2)):M.add_edge(S,T+A,1,_C)
	for A in range(len(B)):M.add_edge(U+A,V,1,_C)
	for ti,A in enumerate(B):
		base=_group_expected_cost(N[A],1)
		for C in G[A]:
			if C[2] in used:continue
			P=T+J2[C[2]];Q=U+ti;K=_group_expected_cost(N[A],1,extra=C)-base;E=len(M.graph[P]);M.add_edge(P,Q,1,K);R[P,E]=A,C
	if M.min_cost_flow(S,V,len(B))<len(B):return []
	for (P,E),(A,C) in R.items():
		if M.graph[P][E][1]==0:N[A].append(C);used.add(C[2])
	# local same-key improve to allow best row groups after matching
	Z=_format_selected(N)
	if time.monotonic()<I-.2:Z=_reassign_single_solution(Z,D,set(B),min(I,time.monotonic()+.45))
	if time.monotonic()<I-.2:Z=_rebalance_single_solution(Z,D,set(B),min(I,time.monotonic()+.45))
	return Z
