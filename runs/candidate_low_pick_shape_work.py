...
def _pick_low_robust_best(solutions,candidates,all_tasks):
	...
	def S(solution):
		used=sum(len(cs) for k,cs in solution);groups=len(solution);twos=sum(1 for k,cs in solution if len(cs)==2 and ',' not in k);ones=sum(1 for k,cs in solution if len(cs)==1 and ',' not in k);extra=max(0,used-60)
		return abs(used-60)*2.2+max(0,30-groups)*3.0+max(0,groups-30)*1.2+extra*4.0-0.35*twos+0.15*ones
	def G(solution):
		E=solution;D0=_solution_expected_cost(E,A,B);F=_solution_expected_cost_by_model(E,A,B,_L);G0=_solution_expected_cost_by_model(E,A,B,_M);H=.42*D0+.43*F+.15*G0;I=max(D0-C,F-C,G0-C);return H+.15*max(_C,I)+S(E),max(D0,F,G0),D0
