...
def solve(input_text):
	...
	else:D=min((A for A in E if A),key=lambda s:_solution_expected_cost_p80(s,C,B))
...
def _solution_expected_cost(result,candidates,all_tasks):
	...
	C+=1e2*(len(all_tasks)-len(B));return C
def _group_expected_cost_p80(rows,task_count,extra=_A):
	G=extra;C=task_count;A=rows
	if G is not _A:A=list(A)+[G]
	...
	if J>12:
		...
		for L,X in enumerate(A):
			...
			for M,Y in enumerate(A):
				if M==L:continue
				I=Y[4];E=[0.0]*(len(P)+1)
				for J2,V in enumerate(P):E[J2]+=V*(1.0-I);E[J2+1]+=V*I
				P=E
			K=0.0
			for O,V in enumerate(P):K+=V/(O+1)
			B+=X[3]*H*K
		return B
	B=0.0
	for mask in range(1<<J):
		...
		for idx,X in enumerate(A):
			if mask>>idx&1:D*=X[4];K+=X[3];E+=1
			else:D*=1.0-X[4]
		if E:B+=D*(K/E)
		else:B+=D*(80.*C)
	return B
def _solution_expected_cost_p80(result,candidates,all_tasks):
	H={(A[0],A[2]):A for A in candidates};B=set();E=set();C=0.0
	for(I,J)in result:
		A=[]
		for D in J:
			F=H.get((I,D))
		...
		for G in A[0][1]:
			if G in B:return float(_F)
			B.add(G)
		C+=_group_expected_cost_p80(A,len(A[0][1]))
