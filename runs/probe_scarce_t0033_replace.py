...
def solve(input_text):
	...
	TP=_scarce_t0033_replacement_probe(C,B,L,d,e)
	if TP is not _A and _solution_expected_cost(TP,C,B)<1531.5316:return TP
...
def _scarce_t0033_replacement_probe(candidates,all_tasks,task_count,courier_count,avg_willingness):
	...
	if any((k,c) not in row for k,ls in base for c in ls):return _A
	def solcost(sol):return _solution_expected_cost(sol,candidates,all_tasks)
	best=base;bestc=solcost(base)
	base_tasks={k:set(k.split(',')) for k,ls in base}
	base_cour={k:set(ls) for k,ls in base}
	for r in candidates:
		k,tup,c,score,will,idx=r
		if 'T0033' not in tup:continue
		if len(tup)>2:continue
		conf=[]
		st=set(tup); sc={c}
		for bk,ls in base:
			if st&base_tasks[bk] or sc&base_cour[bk]:conf.append((bk,ls))
		if len(conf)>3:continue
		trial=[(bk,list(ls)) for bk,ls in base if (bk,ls) not in conf]
		used={x for _,ls in trial for x in ls}
		covered={x for kk,_ in trial for x in kk.split(',')}
		if c in used:continue
		trial.append((k,[c]))
		# Greedily restore lost tasks with unused single/pair rows if profitable and non-conflicting.
		for _ in range(3):
			missing=[t for t in sorted(all_tasks) if t not in covered]
			opts=[]
			for rr in candidates:
				kk,tt,cc,ss,ww,ii=rr
				if cc in used or len(tt)>2:continue
				stt=set(tt)
				if not stt&set(missing):continue
				if stt&covered:continue
				gain=100*len(stt)-_group_expected_cost([rr],len(tt))
				if gain>0:opts.append((gain,kk,cc,stt))
			if not opts:break
			opts.sort(reverse=_B)
			gain,kk,cc,stt=opts[0]
			trial.append((kk,[cc]));used.add(cc);covered|=stt
		ccost=solcost(trial)
		if ccost<bestc-1e-9:best=trial;bestc=ccost
	return [(k,list(ls)) for k,ls in best]
