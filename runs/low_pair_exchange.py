...
def solve(input_text):
	...
	if J and time.monotonic()<A-.42:
		K=_low_pair_exchange_solution(D,C,B,min(A,time.monotonic()+.34),max_moves=18)
		if _solution_expected_cost(K,C,B)<_solution_expected_cost(D,C,B)-1e-09:D=K
...
def _low_pair_exchange_solution(result,candidates,all_tasks,deadline,max_moves=18):
	row={(r[0],r[2]):r for r in candidates};sel=_result_to_selected(result,row)
	if not sel:return result
	keys=[k for k,v in sel.items() if v and len(v[0][1])==1 and len(v)==2]
	if len(keys)<2:return result
	moves=0
	while moves<max_moves and time.monotonic()<deadline-.035:
		best=_A;best_delta=_C
		for i,k1 in enumerate(keys):
			g1=sel.get(k1)
			if not g1 or len(g1)!=2:continue
			c1=_group_expected_cost(g1,1)
			for k2 in keys[i+1:]:
				if time.monotonic()>deadline-.035:break
				g2=sel.get(k2)
				if not g2 or len(g2)!=2:continue
				c2=_group_expected_cost(g2,1);old=c1+c2
				for a in range(2):
					for b in range(2):
						r1=g1[a];r2=g2[b]
						n1=row.get((k1,r2[2]));n2=row.get((k2,r1[2]))
						if n1 is _A or n2 is _A:continue
						ng1=list(g1);ng2=list(g2);ng1[a]=n1;ng2[b]=n2
						if ng1[0][2]==ng1[1][2] or ng2[0][2]==ng2[1][2]:continue
						new=_group_expected_cost(ng1,1)+_group_expected_cost(ng2,1);d=new-old
						if d<best_delta-1e-12:best_delta=d;best=k1,k2,ng1,ng2
		if best is _A:break
		k1,k2,ng1,ng2=best;sel[k1]=ng1;sel[k2]=ng2;moves+=1
	trial=_format_selected(sel)
