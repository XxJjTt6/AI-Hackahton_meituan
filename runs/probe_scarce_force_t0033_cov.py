...
def solve(input_text):
	...
	SP=_scarce_force_t0033_probe(C,B,L,d,e)
...
def _scarce_force_t0033_probe(candidates,all_tasks,task_count,courier_count,avg_willingness):
	...
	base_tasks={k:set(k.split(',')) for k,ls in base};base_cour={k:set(ls) for k,ls in base}
	def metric(sol):
		cov=_solution_covered_count(sol,candidates)
		cost=_solution_expected_cost(sol,candidates,all_tasks)
		return cov,-cost
	best=_A;bestm=(-1,float('-inf'))
	for r in candidates:
		k,tup,c,score,will,idx=r
		if 'T0033' not in tup or len(tup)>2:continue
		st=set(tup);sc={c};conf=[]
		for bk,ls in base:
			if st&base_tasks[bk] or sc&base_cour[bk]:conf.append((bk,ls))
		if len(conf)>4:continue
		trial=[(bk,list(ls)) for bk,ls in base if (bk,ls) not in conf]
		used={x for _,ls in trial for x in ls};covered={x for kk,_ in trial for x in kk.split(',')}
		if c in used:continue
		trial.append((k,[c]));used.add(c);covered|=st
		for _ in range(6):
			missing=[t for t in sorted(all_tasks) if t not in covered]
			if not missing:break
			opts=[];miss=set(missing)
			for rr in candidates:
				kk,tt,cc,ss,ww,ii=rr
				if cc in used or len(tt)>2:continue
				stt=set(tt)
				if not stt&miss or stt&covered:continue
				gain=100*len(stt)-_group_expected_cost([rr],len(tt))
				opts.append((len(stt&miss),gain,-_group_expected_cost([rr],len(tt)),kk,cc,stt))
			if not opts:break
			opts.sort(reverse=_B);_,_,_,kk,cc,stt=opts[0]
			trial.append((kk,[cc]));used.add(cc);covered|=stt
		m=metric(trial)
		if m>bestm:bestm=m;best=trial
	if best is _A or bestm[0]<39:return _A
	return sorted(best)
