...
def solve(input_text):
	...
	if J and time.monotonic()<A-.42:D=_low_mcf_reassign_solution(D,C,B,min(A,time.monotonic()+.35))
...
def _low_mcf_reassign_solution(result,candidates,all_tasks,deadline):
	M=deadline;H=candidates;L=all_tasks;R={(A[0],A[2]):A for A in H};B=_result_to_selected(result,R)
	if not B or len(B)<len(L)-1:return result
	# Reassign singleton-task groups with fixed group sizes via min-cost flow over candidate combinations.
	items=[];couriers=sorted({A[2] for rows in B.values() for A in rows})
	if len(couriers)>70:return result
	by_key={}
	for A in H:
		if len(A[1])==1:by_key.setdefault(A[0],[]).append(A)
	for key,rows in B.items():
		if len(rows[0][1])!=1:return result
		k=len(rows);pool=sorted(by_key.get(key,[]),key=lambda c:(_group_expected_cost([c],1),-c[4],c[5]))[:16]
		opts=[]
		for comb in itertools.combinations(pool,k):
			if time.monotonic()>M-.05:break
			if len({A[2] for A in comb})!=k:continue
			opts.append((_group_expected_cost(comb,1),tuple(comb)))
		if not opts:return result
		items.append((key,k,sorted(opts,key=lambda x:x[0])[:24]))
	if time.monotonic()>M-.05:return result
	S=0;I=1;J=I+len(couriers);K=J+sum(len(x[2]) for x in items);T=K+len(items);N=T+1;flow=sum(k for _,k,_ in items);D=_MinCostFlow(N);idx={c:i for i,c in enumerate(couriers)};meta={};node=K
	for c,i in idx.items():D.add_edge(S,I+i,1,_C)
	optnode=J
	for gi,(key,k,opts) in enumerate(items):
		gnode=K+gi;D.add_edge(gnode,T,k,_C)
		for cost,comb in opts:
			cur=optnode;optnode+=1;D.add_edge(cur,gnode,k,cost/k);meta[cur]=(key,comb)
			for row in comb:D.add_edge(I+idx[row[2]],cur,1,_C)
	D.add_edge(T,N,flow,_C)
	if D.min_cost_flow(S,N,flow)<flow:return result
	selected={}
	for cnode,(key,comb) in meta.items():
		got=0
		for e in D.graph[cnode]:
			if K<=e[0]<T and e[1]==0:got=1;break
		if got:selected[key]=list(comb)
	if len(selected)!=len(items):return result
	U=_format_selected(selected)
	if _solution_expected_cost(U,H,L)<_solution_expected_cost(result,H,L)-1e-09:return U
	return result
