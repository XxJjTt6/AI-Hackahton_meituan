...
def solve(input_text):
	...
	if G and time.monotonic()<A-.62:
		T=_solve_scarce_topk_bundle_mcf(C,B,min(A,time.monotonic()+.5))
		if T and _accept_scarce_topk_candidate(D,T,C,B,f):D=_drop_unprofitable_groups(T,C,B)
...
def _solve_scarce_bundle_mcf_enum(candidates,all_tasks,deadline):
	...
	return L
def _solve_scarce_topk_bundle_mcf(candidates,all_tasks,deadline):
	# KDD-style transfer: keep only a few locally strong bundle edges per task,
	# then use the existing exact MCF completion layer for remaining singletons.
	N=deadline;H=all_tasks;G=candidates;I=sorted(H);R={B:A for(A,B)in enumerate(I)};J={};per_task={A:[]for A in I};C=[]
	for D in _canonical_candidates(G):
		if len(D[1])==1:J.setdefault(D[1][0],[]).append(D);continue
		A=0;ok=_B
		for S in D[1]:
			if S not in R:ok=_D;break
			A|=1<<R[S]
		if not ok:continue
		F=_group_expected_cost([D],len(D[1]));T=1e2*len(D[1])-F
		if T<=1e-09:continue
		item=(T,F,A,D[2],D)
		for S in D[1]:per_task[S].append(item)
	for S,L in per_task.items():
		L.sort(key=lambda item:(item[0]/max(item[1],1e-09),item[0],-item[1],-item[4][5]),reverse=_B)
		C.extend(L[:4])
	if not C:return[]
	Dct={}
	for item in C:
		key=(item[2],item[3],item[4][0])
		old=Dct.get(key)
		if old is _A or item[1]<old[1]-1e-12:Dct[key]=item
	C=list(Dct.values())
	C.sort(key=lambda item:(_popcount(item[2]),item[0]/max(item[1],1e-09),item[0],-item[1],-item[4][5]),reverse=_B);C=C[:48]
	best=_complete_scarce_bundles_with_mcf((),J,I);best_cost=_solution_expected_cost(best,G,H)if best else float(_F)
	states=[(0,frozenset(),())];seen=list(states);limit=min(len({A[2]for A in G}),len(H)//2+2,10)
	for step in range(limit):
		if time.monotonic()>N-.18:break
		next_states=[]
		for(mask,used,rows)in states:
			for(gain,cost,cmask,courier,row)in C:
				if mask&cmask or courier in used:continue
				next_states.append((mask|cmask,used|{courier},rows+(row,)))
				if len(next_states)>=360:break
			if len(next_states)>=360 or time.monotonic()>N-.18:break
		if not next_states:break
		next_states=_prune_scarce_bundle_states(next_states,I,J,max_states=48);seen.extend(next_states);states=next_states
	visited=set()
	evals=0
	for(mask,used,rows)in sorted(seen,key=lambda st:(-_popcount(st[0]),len(st[1]),tuple(A[0] for A in st[2]))):
		if time.monotonic()>N-.04 or evals>=36:break
		evals+=1
		key=(mask,tuple(sorted(used)))
		if key in visited:continue
		visited.add(key);sol=_complete_scarce_bundles_with_mcf(rows,J,I)
		if not sol:continue
		cost=_solution_expected_cost(sol,G,H)
		if cost<best_cost-1e-09:best=sol;best_cost=cost
	return best
...
def _accept_scarce_topk_candidate(current,candidate,candidates,all_tasks,hard_scarce):
	A=current;B=candidate;C=candidates;D=all_tasks
	if not B:return _D
	covA=_solution_covered_count(A,C);covB=_solution_covered_count(B,C)
	if covB<covA:return _D
	costA=_solution_expected_cost(A,C,D);costB=_solution_expected_cost(B,C,D)
	if hard_scarce:
		# Official scarce repeatedly showed that raw full coverage can be harmful.
		# Require a meaningful raw win and no shadow-score degradation.
		if costB>costA-8.:return _D
		return _hard_scarce_shadow_cost(B,C,D)<=_hard_scarce_shadow_cost(A,C,D)-1e-09
	return costB<costA-1e-09
