...
def solve(input_text):
	...
	if f:
		OS=_solve_hard_scarce_official_shape(C,B,min(A,time.monotonic()+.42))
		if OS:E.append(OS)
		D=_pick_hard_scarce_official_shape(E,C,B);D=_drop_unprofitable_groups(D,C,B)
...
def _solve_hard_scarce_official_shape(candidates,all_tasks,deadline):
	# Official-cache-calibrated action filter: hard scarce baseline is close to
	# 19 two-task bundles + 1 single, with one task intentionally left out.
	D=deadline;C=candidates;B=sorted(all_tasks);idx={t:i for i,t in enumerate(B)};singles={t:[]for t in B};pairs=[]
	for r in _canonical_candidates(C):
		if len(r[1])==1 and r[1][0] in singles:singles[r[1][0]].append(r)
		elif len(r[1])==2 and all(t in idx for t in r[1]):
			cost=_group_expected_cost([r],2);gain=200.-cost
			if gain>1e-9:pairs.append((cost,gain,r))
	if not pairs:return[]
	for t in B:singles[t].sort(key=lambda r:_group_expected_cost([r],1))
	pairs.sort(key=lambda x:(x[0],-x[1],x[2][5]))
	# Try top pair seeds greedily; complete remaining tasks with singles, intentionally
	# allowing one uncovered task if it avoids an expensive assignment.
	best=[];best_score=float(_F)
	for _,_,seed in pairs[:80]:
		if time.monotonic()>D-.035:break
		used={seed[2]};covered=set(seed[1]);out=[(seed[0],[seed[2]])]
		avail=[]
		for cost,gain,r in pairs[:260]:
			if r is seed or r[2] in used or covered&set(r[1]):continue
			avail.append((cost,r))
		for cost,r in avail:
			if len(out)>=19:break
			if r[2] in used or covered&set(r[1]):continue
			out.append((r[0],[r[2]]));used.add(r[2]);covered.update(r[1])
		# add one best single among remaining tasks/couriers
		best_single=None;best_single_cost=float(_F)
		for t in B:
			if t in covered:continue
			for r in singles.get(t,[])[:12]:
				if r[2] in used:continue
				cost=_group_expected_cost([r],1)
				if cost<best_single_cost:best_single=(r[0],[r[2]],t,r[2]);best_single_cost=cost
		if best_single:
			out.append((best_single[0],best_single[1]))
		score=_solution_expected_cost(out,C,set(B))+.35*_hard_scarce_shape_penalty(out,C,set(B))
		if score<best_score:best=out;best_score=score
	return sorted(best)

def _hard_scarce_shape_penalty(result,candidates,all_tasks):
	row_map={(A[0],A[2]):A for A in candidates};pairs=singles=bad=0;covered=set();used=set()
	for key,cs in result:
		rows=[row_map.get((key,c)) for c in cs];rows=[r for r in rows if r is not _A]
		if not rows:bad+=5;continue
		if any(r[2] in used for r in rows):bad+=20
		used.update(r[2] for r in rows)
		for t in rows[0][1]:
			if t in covered:bad+=20
			covered.add(t)
		if len(rows[0][1])==2:pairs+=1
		elif len(rows[0][1])==1:singles+=1
		else:bad+=len(rows[0][1])
	return 18*abs(len(covered)-(len(all_tasks)-1))+2*abs(pairs-19)+abs(singles-1)+bad

def _pick_hard_scarce_official_shape(solutions,candidates,all_tasks):
	A=[s for s in solutions if s]
	if not A:return[]
	return min(A,key=lambda s:(_hard_scarce_shadow_cost(s,candidates,all_tasks)+.35*_hard_scarce_shape_penalty(s,candidates,all_tasks),_solution_expected_cost(s,candidates,all_tasks)))
