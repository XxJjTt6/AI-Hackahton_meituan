...
def solve(input_text):
	...
	if f and time.monotonic()<A-.72:
		T=_solve_scarce_topk_seedpair_mcf(C,B,min(A,time.monotonic()+.38))
		if T and _accept_scarce_seedpair_candidate(D,T,C,B,f):D=_drop_unprofitable_groups(T,C,B)
	...
	if time.monotonic()<A-.22:
		...
		if _solution_expected_cost(K,C,B)<_solution_expected_cost(D,C,B)-1e-09:D=K
	if f and time.monotonic()<A-2.15:
		Q=_scarce_bundle_insertion_repair_solution(D,C,B,min(A,time.monotonic()+1.9),max_windows=42,max_window_tasks=14)
...
def _solve_scarce_topk_seedpair_mcf(candidates,all_tasks,deadline):
	...
	seeds=[()]
	for item in items[:20]:seeds.append((item[4],))
	for idx,a in enumerate(items[:16]):
		for b in items[idx+1:20]:
			if a[2]&b[2] or a[3]==b[3]:continue
			seeds.append((a[4],b[4]))
			if len(seeds)>=56:break
		if len(seeds)>=56:break
	seen=set()
	for rows in seeds:
...
def _accept_scarce_seedpair_candidate(current,candidate,candidates,all_tasks,hard_scarce):
	...
	shadowA=_hard_scarce_shadow_cost(A,C,D)if hard_scarce else costA
	shadowB=_hard_scarce_shadow_cost(B,C,D)if hard_scarce else costB
	if hard_scarce:
		# Avoid official coverage trap: require raw cost win, not only missing-penalty win.
		return costB<costA-1.5 and shadowB<shadowA-1e-09
