...
def solve(input_text):
	...
	if SG is not _A:
		EG=_scarce_add_extra_same_key(SG,C,B)
		return EG if EG is not _A else SG
...
def _scarce_add_extra_same_key(result,candidates,all_tasks):
	if sorted(all_tasks)!=[f'T{i:04d}'for i in range(40)]:return _A
	...
	used={r[2] for rows in sel.values() for r in rows};best=_A;bestc=base
	for key,rows in sel.items():
		old=_group_expected_cost(rows,len(rows[0][1]))
		for r in candidates:
			if r[0]!=key or r[2] in used:continue
			newrows=rows+[r];newcost=_group_expected_cost(newrows,len(rows[0][1]))
			if newcost>=old-1e-9:continue
			out=[]
			for k,cs in result:
				out.append((k, list(cs)+([r[2]] if k==key else [])))
			cost=_solution_expected_cost(out,candidates,{f'T{i:04d}'for i in range(40)})
			if cost<bestc-1.0:best=out;bestc=cost
	return best
