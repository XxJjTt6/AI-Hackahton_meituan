...
def solve(input_text):
	...
	if 9<=L<=35 and not G and not F and time.monotonic()<A-.75:
		...
		if time.monotonic()<A-.35:D=_pairwise_column_exchange_solution(D,C,B,min(A,time.monotonic()+.3),top_riders_per_task_key=8,max_k=4,option_limit=55,max_window_tasks=10,max_pairs=48)
