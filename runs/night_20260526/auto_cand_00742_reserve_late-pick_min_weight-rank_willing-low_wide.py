...
def _search_column_window(candidates,all_tasks,deadline,top_riders_per_task_key,max_k,option_limit):
	...
	for B in R.values():
		...
		B=sorted(B,key=lambda c:(-c[4],c[3],c[5]))[:top_riders_per_task_key]
		if not B:continue
		S=0
...
def _pick_low_robust_best(solutions,candidates,all_tasks):
	...
	def G(solution):E=solution;D=_solution_expected_cost(E,A,B);F=_solution_expected_cost_by_model(E,A,B,_L);G=_solution_expected_cost_by_model(E,A,B,_M);H=.25*D+.65*F+.1*G;I=max(D-C,F-C,G-C);return H+.15*max(_C,I),max(D,F,G),D
