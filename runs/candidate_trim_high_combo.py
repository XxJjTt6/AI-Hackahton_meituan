...
def solve(input_text):
	...
	if J and time.monotonic()<A-1.05:D=_low_deep_window_repair_solution(D,C,B,min(A,time.monotonic()+.65))
	if J and time.monotonic()<A-.7:D=_low_late_acceptance_repair_solution(D,C,B,min(A,time.monotonic()+.35))
	if J and time.monotonic()<A-.28:D=_shift_couriers_between_groups(D,C,B,min(A,time.monotonic()+.14),max_moves=16)
