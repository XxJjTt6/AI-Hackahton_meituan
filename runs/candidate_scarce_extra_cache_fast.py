...
def _scarce_extra_cache_fast(C,T,n,m,w):
	...
	P=time.monotonic()+.65;R={(r[0],r[2]):r for r in C};S=_result_to_selected(B,R);used={c for _,ls in B for c in ls};by={}
	for r in C:by.setdefault(r[0],[]).append(r)
	for k in by:by[k]=sorted(by[k],key=lambda r:(_group_expected_cost([r],len(r[1])),-r[4],r[5]))[:28]
