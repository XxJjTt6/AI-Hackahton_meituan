...
def solve(input_text):
	...
	if G and time.monotonic()<A-.3:
		...
		if time.monotonic()<A-.25:
			A2=_scarce_superset_upgrade_uncovered(D,C,B,min(A,time.monotonic()+.16))
...
def _triple_column_exchange_solution(result,candidates,all_tasks,deadline,top_riders_per_task_key,max_k,option_limit,max_window_tasks,max_triples):
	...
	return J
def _scarce_superset_upgrade_uncovered(result,candidates,all_tasks,deadline):
	M=deadline;L=all_tasks;H=candidates;G=result;R={(A[0],A[2]):A for A in H};B=_result_to_selected(G,R)
	if not B:return G
	by_courier={}
	for A in H:by_courier.setdefault(A[2],[]).append(A)
	while time.monotonic()<M-.04:
		covered={T for rows in B.values() for T in rows[0][1]};missing=sorted(set(L)-covered)
		if not missing:break
		best=_A;best_delta=_C
		for key,rows in list(B.items()):
			if time.monotonic()>M-.04:break
			old_tasks=set(rows[0][1]);old_cost=_group_expected_cost(rows,len(old_tasks));couriers=[A[2] for A in rows]
			for u in missing:
				new_tasks=old_tasks|{u}
				if len(new_tasks)>4:continue
				new_key=','.join(sorted(new_tasks))
				pool=[]
				ok=_B
				for cid in couriers:
					cand=[A for A in by_courier.get(cid,()) if set(A[1])==new_tasks]
					if not cand:ok=_D;break
					pool.append(min(cand,key=lambda c:(_group_expected_cost([c],len(c[1])),-c[4],c[5])))
				if not ok:continue
				new_cost=_group_expected_cost(pool,len(new_tasks));delta=new_cost-old_cost-1e2*len(new_tasks-old_tasks)
				if delta<best_delta-1e-09:best_delta=delta;best=key,new_key,pool
		if best is _A:break
		old_key,new_key,pool=best
		if old_key in B:del B[old_key]
		B[new_key]=pool
...
def _scarce_polish_candidate(result,candidates,all_tasks,deadline):
	...
	if time.monotonic()<D-.25:
		J=_scarce_superset_upgrade_uncovered(A,B,C,min(D,time.monotonic()+.16))
