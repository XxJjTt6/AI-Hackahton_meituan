...
def solve(input_text):
	...
	if 25<=L<=35 and not G and not F and not J:D=_normal_balance_k2_pick([D]+E,C,B)
	return D
...
def _normal_balance_k2_pick(solutions,candidates,all_tasks):
	A=[s for s in solutions if s]
	if not A:return[]
	base=min(A,key=lambda s:_solution_expected_cost(s,candidates,all_tasks));base_cost=_solution_expected_cost(base,candidates,all_tasks)
	...
	def penalty(sol):
		...
		for key,cs in sol:
			...
			# Medium official good shapes favor mostly 2-courier single-task groups.
			p+=abs(len(rows)-2)
			if len(rows[0][1])!=1:p+=2
		return p
	best=min(A,key=lambda s:(_solution_expected_cost(s,candidates,all_tasks)+.18*penalty(s),_solution_expected_cost(s,candidates,all_tasks)))
	if _solution_expected_cost(best,candidates,all_tasks)<=base_cost+1.8:return best
	return base
