...
old_line="\tif 9<=L<=35 and not G and not F and time.monotonic()<A-.95:D=_normal_worst_related_repair_solution(D,C,B,min(A,time.monotonic()+.75))\n\treturn D"
new_line="\tif 9<=L<=35 and not G and not F and time.monotonic()<A-.95:D=_normal_worst_related_repair_solution(D,C,B,min(A,time.monotonic()+.75))\n\tD=_medium_output_upgrade(D,C,B,L,d,e)\n\treturn D"
if old_line not in s:
    raise SystemExit('return marker not found')
s=s.replace(old_line,new_line,1)
Path('runs/medium_output_upgrade_v2.py').write_text(s)
print('runs/medium_output_upgrade_v2.py')
