...
def run_agent(
    ...
    trusted = evolution.trusted_strategies(regime)
    if trusted:
        emit(
            {
                "type": "evolution_recall",
                "message": f"读取到 {len(trusted)} 个适用于 {regime} 的历史生成策略，Planner 会将其作为下一轮参考。",
                "strategies": [item["strategy_id"] for item in trusted[:3]],
            }
        )
    generated_strategy = evolution.generate_strategy(regime, f"run_agent:{case_id}:best-so-far experiment")
    emit(
        {
        ...
        if not adaptive_added and best_record is not None:
            ...
            if safety.passed:
                ...
                outcome = evolution.run_generated_strategy(
                    ...
                    helpers={"fallback_greedy": module._fallback_official_greedy},
