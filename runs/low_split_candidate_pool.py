...
def solve(input_text):
	...
	if f:D=_pick_hard_scarce_best(E,C,B);D=_drop_unprofitable_groups(D,C,B)
		...
		if time.monotonic()<A-.55:
			NE=[]
			for cand in E[:6]:
				if cand and time.monotonic()<A-.20:NE.append(_low_split_bundle_repair_solution(cand,C,B,min(A,time.monotonic()+.12)))
			E.extend([A for A in NE if A])
		D=_pick_low_robust_best(E,C,B)
