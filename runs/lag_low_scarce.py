...
def _lag_lag_result_from_picked(picked):
...
def _lagrangian_columns(candidates, all_tasks, deadline,
                       base_result=None,
                       max_iter=24,
                       solver_module=None,
                       robust_pick=False):
    ...
    for i, c in enumerate(cands):
        atomic_costs[i] = _lag_cand_atomic_cost(S._group_expected_cost, c)
        for t in c[1]:
    ...
    for it in range(max_iter):
        ...
        picked, uncovered = _lag_greedy_assign(rc_list, all_tasks_set)
        result = _lag_result_from_picked(picked)
