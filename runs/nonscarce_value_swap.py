...
def solve(input_text):
	...
	if 9<=L<=35 and not G and not F and time.monotonic()<A-.28:
		K=_value_swap_selected_solution(D,C,B,min(A,time.monotonic()+.22),max_moves=12)
		if _solution_expected_cost(K,C,B)<_solution_expected_cost(D,C,B)-1e-09:D=K
	return D


def _value_swap_selected_solution(result,candidates,all_tasks,deadline,max_moves=12):
	...
	by_courier={}
	for r in candidates:by_courier.setdefault(r[2],[]).append(r)
	moves=0
	while moves<max_moves and time.monotonic()<deadline-.035:
		...
		for i,k1 in enumerate(keys):
			g1=sel.get(k1)
			if not g1:continue
			n1=len(g1[0][1]);c1=_group_expected_cost(g1,n1)
			for k2 in keys[i+1:]:
				if time.monotonic()>deadline-.035:break
				g2=sel.get(k2)
				if not g2:continue
				n2=len(g2[0][1]);c2=_group_expected_cost(g2,n2);old=c1+c2
				for r1 in g1:
					for r2 in g2:
						if r1[2]==r2[2]:continue
						a=row.get((k1,r2[2]));b=row.get((k2,r1[2]))
						if a is _A or b is _A:continue
						ng1=[a if x is r1 else x for x in g1];ng2=[b if x is r2 else x for x in g2]
						new=_group_expected_cost(ng1,n1)+_group_expected_cost(ng2,n2);d=new-old
