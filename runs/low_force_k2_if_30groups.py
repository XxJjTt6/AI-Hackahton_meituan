...
def solve(input_text):
	...
	if J and time.monotonic()<A-.62:D=_low_force_k2_same_groups(D,C,B,min(A,time.monotonic()+.45))
...
def _low_force_k2_same_groups(result,candidates,all_tasks,deadline):
	if len(result)!=len(all_tasks):return result
	keys=[A for(A,B) in result]
	if any(',' in A for A in keys):return result
	R={}
	for A in candidates:
		if len(A[1])==1:R.setdefault(A[0],[]).append(A)
	if any(k not in R for k in keys):return result
	K=keys;Cands=sorted({A[2] for A in candidates});S=0;KG=1;CG=KG+len(K);T=CG+len(Cands);F=_MinCostFlow(T+1);edge={}
	for i,k in enumerate(K):F.add_edge(S,KG+i,2,_C)
	for j,c in enumerate(Cands):F.add_edge(CG+j,T,1,_C)
	ci={c:j for j,c in enumerate(Cands)}
	for i,k in enumerate(K):
		for r in sorted(R[k],key=lambda r:_single_expected_cost(r))[:18]:
			j=ci[r[2]];idx=len(F.graph[KG+i]);F.add_edge(KG+i,CG+j,1,_group_expected_cost([r],1));edge[(KG+i,idx)]=r
	if F.min_cost_flow(S,T,2*len(K))<2*len(K):return result
	out=[]
	for i,k in enumerate(K):
		rows=[];node=KG+i
		for idx,e in enumerate(F.graph[node]):
			if (node,idx) in edge and e[1]==0:rows.append(edge[(node,idx)])
		if len(rows)!=2:return result
		rows=sorted(rows,key=lambda r:(r[3],-r[4],r[5]));out.append((k,[r[2] for r in rows]))
	if _solution_expected_cost(out,candidates,all_tasks)<_solution_expected_cost(result,candidates,all_tasks)-1e-09:return out
	return result
